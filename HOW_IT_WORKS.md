...
NANDA — экспериментальный локальный слой автопомощи. Это не облачная LLM и не
свободная генерация текста. Runtime по-прежнему строит ограниченный набор
кандидатов, а NANDA может добавить или оценить только такие кандидаты, которые
потом проходят через обычный safe replacement pipeline.
...
```text
Expert64 = 64 KB state packet
```

Текущий runtime-статус можно посмотреть так:
...
- `Автокоррекция NANDA` — переключатель внутри окна NANDA, а не дубликат
  основной автоподмены.

Клетки не печатают напрямую в приложение. Они работают как генераторы,
оценщики и защитники:

- writer-клетка может предложить layout-кандидат;
- scorer-клетки оценивают раскладочный/контекстный сигнал;
- guard-клетки гасят protected, CLI и technical risk;
- Mesh делает несколько коротких тактов согласования;
- если согласия нет, результат остаётся `KeepOriginal`.
