...
### Исправление перед Enter

Опция `enter_autocorrect` выключена по умолчанию. Если пользователь включает её
в трее, daemon при нажатии Enter пробует исправить текущий хвост тем же
консервативным typing-assist pipeline и затем досылает Enter через uinput.

Если `auto_replace = false`, для Enter остаются только layout-правила. Это
сохраняет ручной сценарий предсказуемым и не превращает Enter в агрессивный
автокорректор.

Важное ограничение текущей архитектуры: evdev listener получает копию события,
а не монопольный input-method контроль над ним. Поэтому текущая реализация —
best-effort opt-in. Полностью атомарное поведение “исправить, потом отдать
Enter приложению” должно жить в будущем IME/proxy backend.
