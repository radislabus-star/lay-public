...
- в настройках NANDA вынесена в отдельное окно: в основном меню осталась
  только кнопка `NANDA ячейки`, а переключатель `Автокоррекция NANDA` живёт
  внутри этого окна;
- окно NANDA показывает профиль Expert64, роли ячеек и волновую схему
  ансамблевой моды;
...
## NANDA / клеточный мозг

`lay` экспериментирует с маленькой локальной NANDA-архитектурой для автопомощи
при наборе. Это не облачная LLM и не свободная генерация текста. Идея другая:
собрать локальный ансамбль маленьких специализированных клеток, которые пишут
и оценивают только безопасные кандидаты исправления.

Базовая единица:

```text
Expert64 = 64 KB специализированной памяти
```

Клетка маленькая по памяти, но может видеть весь короткий хвост предложения.
Она не обязана отвечать только за одно слово. Например:

- `layout_writer_64k` предлагает кандидаты раскладки;
- `layout_signal_64k` оценивает, похож ли случай на wrong-layout;
- `space_boundary_64k` следит за пробелами и склейками;
- `technical_token_64k` защищает `api`, `vpn`, `git`, `wechat`;
- `morphology_64k` может отвечать за согласование русской фразы;
- `user_style_64k` учится на локальных ручных исправлениях;
- `undo_guard_64k` гасит кандидаты, которые пользователь обычно откатывает.
...
Это важно: `64x3` и `192 KB` имеют одинаковый сырой бюджет, но разные
топологии. Монолит хранит всё в одном пространстве, а ансамбль раскладывает
признаки по органам: layout, guard, context/space.
...
Важная часть NANDA — искать моды не только внутри одной клетки, а в ансамбле.

`Ensemble Mode` — устойчивый паттерн совместной активации Expert64-клеток,
...
Клетки не должны хаотично самообновляться во время печати. Обучение должно идти
через локальный teacher/controller:
...
Текущий статус NANDA: в коде уже есть Expert64-каркас, writer/scorer/guard
слои, Mesh trace и первый Rust-only trainer для 64 KB пакета. Это
исследовательское направление, а не обещание идеального автокорректора.

Проверка клетки перед продвижением:

```bash
cargo run --bin lay-nanda-train -- --epochs 20 --out target/nanda/layout_signal_64k.ex64
LAY_NANDA_LAYOUT_SIGNAL_64K=target/nanda/layout_signal_64k.ex64 \
  cargo run --bin lay-nanda-eval -- --cases data/neural_arbiter/holdout.tsv --show-changes
LAY_NANDA_LAYOUT_SIGNAL_64K=target/nanda/layout_signal_64k.ex64 \
  cargo run --bin lay-nanda-eval -- --cases data/neural_arbiter/holdout.tsv --ensemble-sweep
cargo run --bin lay-nanda-eval -- --compare-64x3-vs-192
```

Безопасный локальный цикл обучения:

```bash
cargo run --bin lay-nanda-loop -- --epochs 32
```

По умолчанию это `dry-run`: команда читает fixtures и локальный learning log,
обучает кандидат `Expert64`, прогоняет eval/ablation и пишет отчёт в
`target/nanda/reports/`, но не меняет runtime-клетку. Сырой пользовательский
текст из learning log в отчёт не печатается.

Продвижение в runtime разрешено только явно:

```bash
cargo run --bin lay-nanda-loop -- --epochs 32 --promote
```

Даже с `--promote` пакет включается только если кандидат не хуже текущего
runtime-пакета и eval не показывает regressions.

Клетку нельзя делать главной только потому, что она существует. Минимальный
критерий: `lay-nanda-eval` не ухудшает baseline, а ablation показывает, что
отключение клетки ухудшает её конкретный класс задач.

`--compare-64x3-vs-192` — синтетический контрольный тест, не production-proof.
Он сравнивает три специализированные 64 KB клетки (`layout`, `guard`,
`context/space`) с одним 192 KB монолитом на композиционных случаях вида
"применить только если есть сигнал исправления и нет veto-риска".
...
`lay-nanda-eval --ensemble-sweep` печатает эти признаки:
...
- `NANDA ячейки` — отдельное окно с клеточным слоем, статусом и переключателем
  `Автокоррекция NANDA`.
