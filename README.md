...
#### 6. Исправлять перед Enter

`Исправлять перед Enter` — отдельный opt-in режим. По умолчанию он выключен,
потому что обычная помощь уже работает после пробела.

Когда режим включён, daemon при Enter пробует применить тот же безопасный
pipeline к текущему хвосту и только потом дослать Enter. Если `Автоподмена`
выключена, остаются только layout-правила; typo-правки не становятся
агрессивнее.

Техническая граница текущего evdev/uinput пути: это best-effort поведение, а не
идеальный атомарный перехват клавиши во всех приложениях. Настоящий строгий
вариант требует input-method/proxy backend, где lay управляет `Enter` до того,
как приложение его получит.
...
#### 8. Жёсткая раскладка по окну: ptah_alexs
...
- `Исправлять перед Enter`: opt-in правка текущего хвоста при Enter.
