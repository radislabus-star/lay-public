...
pub(crate) fn print_json(args: &[String]) -> io::Result<()> {
    ...
    let report = report_json(&path, limit, examples)?;
    println!("{}", serde_json::to_string_pretty(&report)?);
    Ok(())
}

fn report_json(path: &Path, limit: usize, max_examples: usize) -> io::Result<Value> {
    ...
    for line in &lines[start..] {
        ...
        let Some(input) = value.get("lay_from").and_then(Value::as_str) else {
            continue;
        };
