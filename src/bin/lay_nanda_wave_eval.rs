...
mod ime_hit_rate;
#[path = "lay_nanda_wave_eval/l2_lexical_corpus.rs"]
mod l2_lexical_corpus;
#[path = "lay_nanda_wave_eval/l2_route_compare.rs"]
mod l2_route_compare;
#[path = "lay_nanda_wave_eval/learning_loop.rs"]
mod learning_loop;
#[path = "lay_nanda_wave_eval/real_suite.rs"]
mod real_suite;
#[path = "lay_nanda_wave_eval/self_teacher_l3.rs"]
mod self_teacher_l3;
...
fn main() -> io::Result<()> {
    ...
    if let Some(path) = arg_value(&args, "--l2-lexical-memory") {
        env::set_var("LAY_L2_LEXICAL_PHASE_MEMORY", path);
    }
    if args.iter().any(|arg| arg == "--l2-lexical-corpus-eval") {
        l2_lexical_corpus::print_json(&args)?;
        return Ok(());
    }
    ...
    if args.iter().any(|arg| arg == "--lay-self-teacher-l3") {
        self_teacher_l3::print_json(&args)?;
        return Ok(());
    }
        ...
        let input = arg_value(&args, "--input")
            ...
            .ok_or_else(|| {
                std::io::Error::new(
                    ...
                    "--compile-usage-feedback requires --input PATH",
                )
            })?;
        let output = arg_value(&args, "--out")
            .map(std::path::PathBuf::from)
            .ok_or_else(|| {
                std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
                    "--compile-usage-feedback requires --out PATH",
                )
            })?;
        println!(
            "{}",
            serde_json::to_string_pretty(&lay::nanda_wave::compile_usage_feedback_snapshot(
                &input, &output,
            )?)?
        );
        return Ok(());
    ...
    if args.iter().any(|arg| arg == "--l2-route-compare-report") {
        l2_route_compare::print_json(&args)?;
        return Ok(());
    }
