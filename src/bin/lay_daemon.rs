...
fn idle_wait_timeout(
    ...
    typing_assist_blocked_by_shift: bool,
) -> Duration {
    idle_wait_timeout_at(
        ...
        last_focus_ignore_poll,
        shift_window,
        typing_assist_blocked_by_shift,
    )
}
...
fn idle_wait_timeout_at(
    ...
    if !typing_assist_blocked_by_shift {
...
fn handle_typing_assist_after_space(
    ...
    let _physical_grab = physical_device.map(grab_physical_device_for_correction);
...
mod tests {
    ...
    fn idle_wait_uses_long_sleep_when_no_internal_deadlines() {
        ...
        assert_eq!(
            idle_wait_timeout_at(now, None, None, now, Duration::from_millis(120), false),
    ...
    #[test]
    fn idle_wait_does_not_spin_when_typing_assist_waits_for_shift_release() {
        let now = Instant::now();
