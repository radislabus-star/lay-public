...
fn main() -> io::Result<()> {
    ...
    #[cfg(feature = "lexical-compiler")]
    if let Some(corpus) = arg_path(&args, "--prove-l1-typed-edit-phase7a") {
        let package = arg_path(&args, "--memory")
            .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "--memory is required"))?;
        let report = lay::nanda_wave::prove_l1_typed_edit_phase7a(
            &corpus,
            &package,
            arg_usize(&args, "--max-words").unwrap_or(0),
            arg_usize(&args, "--heldout-per-class").unwrap_or(20_000),
    ...
    #[cfg(feature = "lexical-compiler")]
    if let Some(corpus) = arg_path(&args, "--prove-l1-typed-edit-phase7b") {
        let package = arg_path(&args, "--memory")
            .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "--memory is required"))?;
        let report = lay::nanda_wave::prove_l1_typed_edit_phase7b(
            &corpus,
            &package,
            arg_usize(&args, "--max-words").unwrap_or(0),
            arg_usize(&args, "--heldout-per-class").unwrap_or(20_000),
    ...
    #[cfg(feature = "lexical-compiler")]
    if let Some(corpus) = arg_path(&args, "--prove-l1-typed-edit-phase7d") {
        let package = arg_path(&args, "--memory")
            .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidInput, "--memory is required"))?;
        let report = lay::nanda_wave::prove_l1_typed_edit_phase7d(
            &corpus,
            &package,
            arg_usize(&args, "--max-words").unwrap_or(0),
            arg_usize(&args, "--heldout-per-class").unwrap_or(20_000),
