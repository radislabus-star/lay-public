...
pub(crate) struct ObservedCanonicalL2FieldReadout {
    ...
    pub(crate) authority_context: super::LexicalAuthorityEvaluationContextV1,
}
...
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub(crate) struct CanonicalL1LexicalAnchorV1 {
    terminal_id: u32,
    anchor_form_ref: u32,
    lemma_id: Option<u32>,
    direct: bool,
...
impl CanonicalL1LexicalAnchorV1 {
    pub(crate) const fn terminal_id(self) -> u32 {
        self.terminal_id
    }

    pub(crate) const fn anchor_form_ref(self) -> u32 {
        self.anchor_form_ref
    }

    pub(crate) const fn lemma_id(self) -> Option<u32> {
        self.lemma_id
    }
...
impl StandaloneL2Field {
    ...
    pub(super) fn package_identity(&self) -> Option<super::package_bytes::LoadedPackageIdentity> {
    ...
    pub(crate) fn l1_lexical_anchor_for_form_ref(
        &self,
        form_ref: u32,
    ) -> Option<CanonicalL1LexicalAnchorV1> {
        self.package.form(form_ref as usize)?;
        let lemma_ids = self
            .bindings_for_form(form_ref)
            .map(|binding| binding.lemma_center_id)
            .collect::<BTreeSet<_>>();
        let unique_lemma_id = (lemma_ids.len() == 1)
            .then(|| lemma_ids.first().copied())
            .flatten();

        if let Some(terminal_id) = self.l1_terminal_for_form_ref(form_ref) {
            return Some(CanonicalL1LexicalAnchorV1 {
                terminal_id,
                anchor_form_ref: form_ref,
                lemma_id: unique_lemma_id,
                direct: true,
            });
        }

        let lemma_id = unique_lemma_id?;
        let mut best = None::<(u8, usize, u32, String, u32, u32)>;
        for binding in self.bindings_for_lemma(lemma_id) {
            let anchor_form_ref = binding.form_center_ref;
            let Some(terminal_id) = self.l1_terminal_for_form_ref(anchor_form_ref) else {
                continue;
            };
            let surface = self.decode_form_ref(anchor_form_ref)?.into_owned();
            let candidate = (
                crate::nanda_wave::morphology_phase::productive_source_priority(
                    binding.feature_mask,
                ),
        ...
        let (_, _, _, _, anchor_form_ref, terminal_id) = best?;
        Some(CanonicalL1LexicalAnchorV1 {
            terminal_id,
            anchor_form_ref,
