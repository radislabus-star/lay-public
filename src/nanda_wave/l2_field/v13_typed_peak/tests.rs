...
use crate::nanda_wave::l2_field::productive_v1::{
    ...
    ExactPeakCandidateInputV1, PreparedFieldMaterialScopeV1, PRODUCTIVE_V90_TYPED_EXACT_SOURCE_ID,
};
...
const EXPECTED_V7_SHA256: &str = "33fded73e13f565bdc08a83bd440be67c80f6d85a196453050b09bc9f7ef28e4";
...
const PSS_HELPER_TEST: &str = "nanda_wave::l2_field::v13_typed_peak::tests::v11_mmap_pss_helper";
    ...
    for observed in [
        ...
        let expected = surfaces
            ...
            .filter_map(|(form_ref, surface)| {
                (!crate::nanda_wave::lexical_grokking::phase7d_certificate_keys(observed, surface)
                    .expect("oracle")
                    .is_empty())
...
fn generated_phase7d_matrix_matches_tiny_exhaustive_oracle() {
    ...
    surfaces.sort();
    surfaces.dedup();
    let cases = crate::nanda_wave::lexical_grokking::prepare_fixed_heldout_cases(&surfaces, 2, 0)
    ...
    for case in cases {
        let result = search_typed_peaks(&view, &package, &case.surface, SearchBudget::proof())
            .expect("typed search");
        let banded =
            search_typed_peaks_banded_oracle(&view, &package, &case.surface, SearchBudget::proof())
...
fn run_generated_gate_b() -> Result<serde_json::Value, String> {
    ...
    surfaces.sort();
    surfaces.dedup();
    let cases = crate::nanda_wave::lexical_grokking::prepare_fixed_heldout_cases(&surfaces, 4, 0)
    ...
    for case in &cases {
        ...
        let expected = surfaces
            ...
            .enumerate()
            .filter_map(|(form_ref, surface)| {
                let certificates = crate::nanda_wave::lexical_grokking::phase7d_certificate_keys(
                    &case.surface,
                    surface,
                )
                .ok()?;
    ...
    for case in cases {
        *by_availability
        ...
        for peak in &result.peaks {
            ...
            if independently_verified.is_empty() || independently_verified != peak.certificate_keys
            {
    ...
    for outcome in outcomes {
        match outcome {
            Ok((completeness, dla_build_us, intersection_us, search_us, total_us)) => {
                unresolved += usize::from(completeness != SearchCompleteness::CertifiedExhaustive);
...
fn v11_mmap_pss_helper() {
    ...
    let sync = std::path::PathBuf::from(std::env::var_os("LAY_V11_PSS_SYNC").expect("PSS sync"));
    let identity = V13Identity {
    ...
    let sidecar = std::path::PathBuf::from(std::env::var_os("LAY_V11_SIDECAR").expect("sidecar"));
...
fn run_m3_test_source_integration() -> Result<serde_json::Value, String> {
    ...
    for reverse_schedule in [false, true] {
        ...
        for case in &cases {
            ...
            let byte = byte_exact_search(&index, &lanes, SearchBudget::proof(), reverse_schedule)?;
            ...
            terminal_rank_mismatches += usize::from(byte.terminal_ranks != typed.terminal_ranks);
            ...
            typed_examined_edges = typed_examined_edges.saturating_add(typed.work.examined_edges);
            byte_expanded_states = byte_expanded_states.saturating_add(byte.work.expanded_states);
...
fn run_m3_actual_owner_proof() -> Result<serde_json::Value, String> {
    ...
    for reverse_schedule in [false, true] {
        for (case_index, case) in cases.iter().enumerate() {
            ...
            for peak in &result.peaks {
                ...
                let normalized = super::super::compositional::normalize_surface(surface.as_ref());
    ...
    let lattice_mismatches = lattice_marker_mismatches.saturating_add(emitted_surface_mismatches);
...
mod m3_v8 {
    ...
    impl ExactGeneration {
        fn from_validated(sidecar: V13DafsaView, sidecar_sha256: [u8; 32]) -> Result<Self, String> {
    ...
    fn m3_end_to_end_pss_helper() {
        ...
        let sidecar_sha256 =
            parse_hex_32(&std::env::var("LAY_M3_V8_SIDECAR_SHA256").expect("M3 V8 sidecar SHA"))
    ...
    fn run_closed_request(
        ...
        let oracle =
            Phase7dCertificateOracle::new(&case.damaged_surface).map_err(V8Failure::semantic)?;
        ...
        counters.certificate_mismatches = usize::from(actual_certificates != expected_certificates);
    ...
    fn exercise_generation_reload(
        ...
        let view_b = V13DafsaView::load(sidecar_path, identity).map_err(V8Failure::provenance)?;
    ...
    fn run_m3_end_to_end_physical_proof() -> Result<serde_json::Value, V8Failure> {
        ...
        let identity = V13Identity {
            ...
            binding_count: u32::try_from(package.binding_count()).map_err(V8Failure::provenance)?,
        };
        let sidecar_bytes = compile_sidecar(&package, identity).map_err(V8Failure::provenance)?;
        ...
        let sidecar = V13DafsaView::load(&sidecar_path, identity).map_err(V8Failure::provenance)?;
        ...
        let maximum_round_total_p99 = round_rows
            ...
            .max()
            .unwrap_or(u64::MAX);
        let reload = exercise_generation_reload(&owner, &sidecar_path, identity, sidecar_sha256)?;
    ...
    fn m3_end_to_end_physical_proof() {
        ...
        write_integration_receipt(&receipt_path, &receipt).expect("write M3 V8 end-to-end receipt");
    ...
    fn generation_owner_state_machine_is_fail_closed() {
        ...
        let readers = (0..8)
            .map(|_| {
                let owner = Arc::clone(&owner);
                std::thread::spawn(move || owner.borrow().expect("concurrent generation borrow"))
            })
