...
pub(super) fn apply_trailing_word_rule(
    ctx: &TypingRuleContext<'_>,
    rule: TextRule,
) -> Option<String> {
    let segments = split_ws_segments(ctx.core);
    ...
    let right_idx = segments
        .iter()
        .enumerate()
        .rev()
        .find_map(|(idx, (_, is_ws))| (!*is_ws).then_some(idx))?;
    let (right_leading, right_word, right_trailing) = split_word_punctuation(segments[right_idx].0);
    if right_word.is_empty() || !right_leading.is_empty() {
        return None;
    }
    let replacement = rule(right_word)?;
    if replacement == right_word {
...
pub(super) fn apply_any_word_rule(ctx: &TypingRuleContext<'_>, rule: TextRule) -> Option<String> {
    let segments = split_ws_segments(ctx.core);
    if segments.len() < 3 {
        return None;
    }

    for (target_idx, (segment, is_ws)) in segments.iter().enumerate().rev() {
        if *is_ws {
            continue;
        }
        ...
        let Some(replacement) = rule(word) else {
            continue;
        };
        if replacement == word {
            continue;
        ...
        let mut output = String::with_capacity(ctx.core.len().max(replacement.len()));
        for (idx, (part, _)) in segments.iter().enumerate() {
            if idx == target_idx {
