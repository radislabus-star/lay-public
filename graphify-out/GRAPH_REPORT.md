# Graph Report - lay  (2026-08-06)

## Corpus Check
- 903 files · ~3,977,592 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 11654 nodes · 29872 edges · 506 communities (450 shown, 56 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 2955 edges (avg confidence: 0.8)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `19892e17`
- Run `git rev-parse HEAD` and compare to check if the graph is stale.
- Run `graphify update .` after code changes (no API cost).

## Community Hubs (Navigation)
- learning_loop.rs
- default_typing_assist_pipeline
- decode_typing_assist_tail
- lay_nanda_wave_eval.rs
- static_models.rs
- UnifiedCorrectionCandidate
- Lay L1-L4 Runtime Architecture
- tests.rs
- tray_support.js
- canonical_l2_recent.rs
- text_edit_tests.rs
- gate_candidate_with_source
- eval_two_word_models.py
- lexicon.rs
- PhaseReadout
- LlmWaveMemory
- WordCandidate
- word_reader.rs
- KeyEvent
- fixture_rows
- LexicalPhaseMemory
- TypingErrorClass
- candidate_sources.rs
- l3.rs
- direct.rs
- config_runtime.rs
- fixture_rows
- surface_motif_memory
- fuzzy_known_word_candidates
- typing_assist_deferred.rs
- tests.rs
- read_entries
- ContextPhasePackage
- hot_field.rs
- TextReplacement
- verifier.rs
- ManualTriggerEventContext
- usage_prior.rs
- candidate_gate.rs
- l2_wave_peak.rs
- LayIbusEngine
- data_lines
- safe_extra_letter_candidates
- RecentActionGateTrace
- correction_core.rs
- gnome_dbus.rs
- lay_debug_actions.rs
- decide_scoped_tail_correction
- .commit_active_composition_with_suffix
- correct_wrong_layout_ascii_word
- TailContext
- prefs.js
- candidate_quality.rs
- lexical_surface_atoms.rs
- trace.rs
- surface.rs
- compiler.rs
- lay
- tail_memory.rs
- run_scenario
- completed_word.rs
- decode_completed_tail
- status.rs
- llm_tests.rs
- TypingCandidateFamily
- text_to_key_events
- PendingTypingAssist
- manual_toggle.rs
- settings.js
- String
- desktop_probe.rs
- DaemonLoopState
- TypingAssistRuleConfig
- TypingRuleContext
- mixed_script_repair.rs
- WaveOptions
- replay_layout_decision
- input_gate.rs
- format.rs
- mod.rs
- layout_controller.rs
- is_cyrillic_word
- DaemonTextContext
- runtime.rs
- typing_replacements.rs
- signal.rs
- eval.rs
- typing_context_tests.rs
- journal_record.rs
- String
- state.rs
- compiler.rs
- ShiftState
- typing_candidate_tests.rs
- ForceLayoutHotkeyContext
- typing_assist_mixed_corpus.rs
- agree_adjective_like_tail
- transition.rs
- log
- String
- lay_nanda_dataset.rs
- correction_bayes.rs
- lay-kde-tray.py
- run_command_capture
- runtime_flags.rs
- apply_prepared_typing_assist_after_space
- CandidateOrigin
- structural_relation.rs
- What You Must Do When Invoked
- assert_hyphen_case_keeps_undo
- LayIbusEngine
- pattern_wave.rs
- layout_niri.rs
- hunspell.rs
- hot.rs
- ime_hit_rate.rs
- technical.rs
- journal.rs
- typing_memory.rs
- canonical_l1_l2.rs
- focus_guard.rs
- ManualCorrectionOutputContext
- cyrillic.rs
- NANDA Lexical Attractor Model
- CellTraceRecord
- explain_typing_assist_with_pipeline
- layout_adapter.rs
- l4_goal_state.rs
- Wave Transition CPU Plan
- listen_keyboard
- Vec
- previous_token_allows_layout_tail
- L1-L4 Intelligence Route
- PhysicalInputGrab
- LayImeBridge
- lay_ngram_corpus.rs
- IME / Daemon Route Map
- x11_layout.rs
- correction_memory_runtime.rs
- main.rs
- llmwave.rs
- EvalCase
- l2_candidate_phase.rs
- text_mutation_monopoly_contract.rs
- Как работает lay
- debug_log.rs
- TextEditBackend
- apply_text_replacement_pipeline
- lay_memory_report.rs
- CharNgramModel
- TypingAssistExplanation
- composition_edit.rs
- EnterAutocorrectContext
- syntax_guard.rs
- layout_sync.rs
- .run
- fixture_row_by_id
- VisibleTailSnapshot
- LayImeBridge
- apply_layout_replay
- ManualOutputCommon
- map_original_events
- main.rs
- .from_bytes
- apply_layout_replay
- state.rs
- candidate.rs
- time.rs
- LayIbusEngine
- Linux input correction research
- replacement_tests.rs
- lay_daemon.rs
- l4_hidden_state.rs
- UsageEvent
- l3_context_metrics.rs
- TransitionAudit
- check-architecture.sh
- .from_bytes
- preedit.rs
- experiment.py
- UserLearningCorrection
- Row
- precognition.rs
- http.rs
- update
- L4PhaseWitnessBank
- l4_active_disambiguation.rs
- .forward_queued_typing
- WaveTrace
- apply_word_rule
- balanced_l2_words_by
- .process_pressed_key
- Double Shift вместо ручного удаления: локальный помощник раскладки для Linux
- Double Shift вместо ручного удаления: локальный помощник раскладки для Linux
- check-lay-audit-50.sh
- layout_niri.rs
- server.rs
- ime_candidate_readout.rs
- config.rs
- desktop.rs
- DirtyLogPair
- committed_tail.rs
- run_runtime_smoke.py
- .process_key_event
- NANDA Wave Architecture
- Форма Хабра
- install.sh
- lay-runtime-control.sh
- architecture_contract.rs
- autocorrect_edit_safety
- format.rs
- apply_layout_replay
- transition_relation.rs
- .run
- UserLearningCorrection
- README.md
- install-remote.sh
- Sync
- correct_wrong_layout_ascii_phrase
- key_decode.rs
- key_decode.rs
- EditAction
- UserLearningCorrection
- profile_tests.rs
- recognize_token
- graphify reference: extra exports and benchmark
- Optional multi-tap Shift scope
- Distribution platforms
- Lay Neural Arbiter Research Plan
- Rust Architecture Rules
- factory.rs
- tests.rs
- Command
- feedback.rs
- committed_tail.rs
- .what_to_replay
- add_group
- core_tests.rs
- Архитектурный чек-лист lay
- Русские площадки для первой волны
- runtime_warmup_plan
- choose_best_with_gap
- .forward_queued_typing
- profile_tests.rs
- tray_ui_contract.rs
- Чек-лист публикации
- choose_token_consensus_with_chooser
- ManualCorrectionOutputContext
- text_metrics.rs
- update.sh
- Development Notes
- graphify reference: query, path, explain
- Short posts
- Text Mutation Monopoly Plan
- render-publicity-demo.py
- bump-lay-version.sh
- check-gnome-extension-runtime.sh
- lay-host-vm-guard.sh
- read
- active_typing_assist_pipeline_for_auto_replace
- make_ibus_text
- lay
- attrs.rs
- L3Feedback
- install-l2-lexical-phase.sh
- replacement_tests.rs
- candidate.rs
- LLMWave Production Contour
- refresh-l2-lexical-phase.sh
- Text Correction Gate Architecture
- Two-word model evaluation
- check-lay-changed.sh
- EntryCapture
- ArchitectureReceiptFreshnessTest
- LayoutBackend
- graphify reference: add a URL and watch a folder
- graphify reference: commit hook and native CLAUDE.md integration
- graphify reference: incremental update and cluster-only
- LayExtension
- feedback.rs
- xml.rs
- WaveTrace
- DecoderEditPlan
- graphify reference: GitHub clone and cross-repo merge
- graphify reference: transcribe video and audio
- Two-word model evaluation
- Two-word model evaluation
- Two-word model evaluation
- Two-word model evaluation
- Two-word model evaluation
- Two-word model evaluation
- Two-word model evaluation
- NANDA Wave / клеточный мозг
- decode_ascii_tail
- .trace_key
- warmup.rs
- AGENTS.md
- extraction-spec.md
- README.md
- README.md
- dev-reload.sh
- tproger-pitch-ru.md
- install-extension.sh
- bump-version-from-git.sh
- check-lay-full.sh
- package-extension.sh
- prepare-llmwave-external-corpus.sh
- args.rs
- handle_enter_autocorrect
- VERSIONING.md
- apply_short_left_word_rule
- Phase Word Recovery Baseline 0.2.241
- Lay L2 Transition Phase Route
- install-l2-transition-phase.sh
- SurfaceWave4096
- ComplexBasisWave
- refresh-l2-transition-phase-gate.sh
- correction_core.rs
- decode_typing_assist_tail
- packet.rs
- learning_deltas
- Project families found
- manual_toggle.rs
- Reconciler
- phase_proof_json
- apply_word_rule
- handle_enter_autocorrect
- uninstall.sh
- lexical_phase_compile.rs
- test-public-issues.sh
- .run
- WaveTrace
- learned.rs
- russian_lexicon_tests.rs
- rebuild-l4-feedback-memory.sh
- LayImeBridge
- verify_with_retry
- correct_vowel_confusion_impl
- install-l3-context-phase.sh
- CharNgramModel
- score_typing_candidate
- correction_core.rs
- cargo-guard.sh
- packet.rs
- time.rs
- precognition.rs
- DecisionOutput
- LayImeBridge
- LayConfig
- LayConfig
- eval.rs
- .from_bytes
- .partition_frontier
- hunspell.rs
- calibration.rs
- LayoutBackend
- build-l3-context-phase.sh
- composition_commit.rs
- cases.py
- install-release-binaries.sh
- tests.rs
- read
- compare-l2-lexical-evals.py
- l3_online.rs
- l3-context-0.2.253.md
- smart_insert_memory.rs
- decode_ascii_tail
- collect_recent_text
- .what_to_replay
- LayConfig
- config.rs
- apply_phrase_case
- auto_undo.rs
- Args
- measured_precognition_stages
- is_backed_russian_form
- mod.rs
- .train
- layout_switch_policy.rs
- desktop.rs
- Lay Self Teacher L3 Plan
- ngram_tests.rs
- DecisionTransitionReceipt
- read_entries
- .what_to_replay
- .run
- LayImeBridge
- proof_chain.rs
- WordBuffer
- packet.rs
- autocorrect_edit_safety
- .manual_toggle_active_text_target
- Reconciler
- decode_ascii_tail
- runtime_flags.rs
- text_edit_gate_contract.rs
- add_mechanistic_features
- precognition.rs
- correct_wrong_layout_ascii_phrase
- live_candidate_gate_stats
- candidate_explanation.rs
- .default
- keyboard_io.rs
- flip_word_events
- dict_tests.rs
- L4PhaseWitnessBank
- dict.rs
- correct_vowel_confusion_impl
- managed_ime_session
- WordBuffer
- .ru_l2_word_attractor_candidates
- run-l11-causal-depth-ablation.sh
- run-l11-surface-depth-ablation.sh
- handle_enter_autocorrect
- ManifestWriteLock
- types.rs
- russian_lexicon_tests.rs
- l2_transition_phase_shadow_readout
- LayIbusEngine
- L2ProbePool
- text_edit_gate_contract.rs
- manual_toggle.rs
- ClassMetrics
- split_ws_segments
- gate_candidate
- typing.rs
- runtime_flags.rs
- l2-package-contract.sh
- apply_l2_phase_shadow
- convert
- .what_to_replay
- l2_transition_phase_shadow_readout
- choose_best_with_gap
- .ru_l2_word_attractor_candidates
- preedit_readout.rs
- .remember_visible_text_for_correction
- candidate_explanation.rs
- composition_commit.rs
- text_edit_gate_contract.rs
- desktop.rs
- precognition.rs
- ime.py
- 13. 2026-08-04 Standalone L2 First-Space Warmup
- PendingRelation
- composition_commit.rs
- config_path
- l2_transition_phase_shadow_readout
- apply_short_left_word_rule
- ManifestWriteLock
- dict_tests.rs
- time.rs
- 13. Standalone Full-Neighbor V13, 2026-07-30
- reload-lay-model-services.sh
- decode_ascii_tail
- .remember_visible_text_for_correction
- handle_pending_auto_undo
- memory.rs
- make_ibus_text
- input_gate_space_contract.rs
- learning_deltas
- 13. 2026-08-03 Inverse Length Birth and Tied-Cohort Authority
- syntax_guard.rs
- types.rs
- first_internal_transposition_typo
- 13. 2026-08-05 Atomic Space And Nonblocking L3 Refresh
- time.rs
- ManifestWriteLock
- apply_l2_phase_shadow
- 14. 2026-08-06 Short-Function Boundary Shift And Space Timing
- record_nanda_trace_if_enabled
- BoundedFrequencySketch
- ManifestWriteLock
- CorrectionMode
- live_l3_context_score

## God Nodes (most connected - your core abstractions)
1. `default_typing_assist_pipeline()` - 134 edges
2. `WordCandidate` - 116 edges
3. `KeyEvent` - 103 edges
4. `UnifiedCorrectionCandidate` - 102 edges
5. `TypingErrorClass` - 84 edges
6. `resolve_text_correction()` - 82 edges
7. `request()` - 77 edges
8. `run_l1()` - 76 edges
9. `TextReplacement` - 74 edges
10. `run_l2()` - 73 edges

## Surprising Connections (you probably didn't know these)
- `ime_readout_keeps_replacement_as_a_non_mutating_typed_proposal()` --calls--> `select_ime_candidate_proposals()`  [INFERRED]
  tests/live_candidate_gate_metrics.rs → src/typing_cpu/candidate.rs
- `simulate_space_triggered_typing_assist()` --calls--> `default_typing_assist_pipeline()`  [INFERRED]
  tests/common/mod.rs → src/config/defaults.rs
- `correction_gate_metrics_are_status_only()` --calls--> `default_typing_assist_pipeline()`  [INFERRED]
  tests/correction_gate_metrics.rs → src/config/defaults.rs
- `typing_assist_decoder_preserves_space_and_avoids_known_false_splits()` --calls--> `default_typing_assist_pipeline()`  [INFERRED]
  tests/decoder_transition_regression.rs → src/config/defaults.rs
- `daemon_space_and_enter_decoders_share_input_gate_replacement_contract()` --calls--> `default_typing_assist_pipeline()`  [INFERRED]
  tests/input_gate_space_contract.rs → src/config/defaults.rs

## Import Cycles
- 1-file cycle: `src/time.rs -> src/time.rs`
- 2-file cycle: `src/text_edit.rs -> src/text_edit/action.rs -> src/text_edit.rs`
- 2-file cycle: `src/candidate_explanation.rs -> src/correction_core.rs -> src/candidate_explanation.rs`
- 2-file cycle: `src/correction_core.rs -> src/typing_transition/proposal_admission.rs -> src/correction_core.rs`
- 3-file cycle: `src/text_edit.rs -> src/text_edit/executor.rs -> src/text_edit/action.rs -> src/text_edit.rs`
- 3-file cycle: `src/text_edit.rs -> src/text_edit/executor.rs -> src/typing_transition/executor_contract.rs -> src/text_edit.rs`
- 3-file cycle: `src/text_edit.rs -> src/text_edit/action.rs -> src/text_edit/gate.rs -> src/text_edit.rs`
- 3-file cycle: `src/text_edit.rs -> src/text_edit/transition.rs -> src/text_edit/action.rs -> src/text_edit.rs`
- 3-file cycle: `src/text_edit/action.rs -> src/text_edit/gate.rs -> src/text_edit/transition.rs -> src/text_edit/action.rs`
- 3-file cycle: `src/keyboard.rs -> src/keyboard/event_words.rs -> src/keyboard/event_words/mapping.rs -> src/keyboard.rs`
- 3-file cycle: `src/keyboard.rs -> src/keyboard/event_words.rs -> src/keyboard/event_words/word_split.rs -> src/keyboard.rs`
- 3-file cycle: `src/candidate_explanation.rs -> src/language_action.rs -> src/correction_core.rs -> src/candidate_explanation.rs`
- 4-file cycle: `src/text_edit.rs -> src/text_edit/executor.rs -> src/text_edit/action.rs -> src/text_edit/gate.rs -> src/text_edit.rs`
- 4-file cycle: `src/text_edit.rs -> src/text_edit/executor.rs -> src/typing_transition/executor_contract.rs -> src/text_edit/action.rs -> src/text_edit.rs`
- 4-file cycle: `src/text_edit.rs -> src/text_edit/transition.rs -> src/text_edit/action.rs -> src/text_edit/gate.rs -> src/text_edit.rs`
- 5-file cycle: `src/text_edit.rs -> src/text_edit/executor.rs -> src/typing_transition/executor_contract.rs -> src/text_edit/action.rs -> src/text_edit/gate.rs -> src/text_edit.rs`

## Communities (506 total, 56 thin omitted)

### Community 0 - "learning_loop.rs"
Cohesion: 0.13
Nodes (37): build_experience(), classify_operation(), clean_cyrillic_learning_text(), contains_unsafe_learning_text(), CorrectionExperience, CorrectionSignal, default_correction_log_path(), experiences_from_log() (+29 more)

### Community 1 - "default_typing_assist_pipeline"
Cohesion: 0.08
Nodes (74): default_typing_assist_pipeline(), resolve_text_correction(), adjacent_transposition_cannot_rewrite_l2_known_word_without_state_proof(), adjacent_transposition_keeps_already_known_word(), ambiguous_long_l2_surface_drift_from_live_log_is_suggestion_only(), ambiguous_short_boundary_shift_is_suggestion_only(), boundary_shift_cannot_apply_on_clean_two_word_surfaces(), boundary_shift_is_selected_by_the_unified_correction_core() (+66 more)

### Community 2 - "decode_typing_assist_tail"
Cohesion: 0.16
Nodes (38): add_live_file(), add_live_learning(), append_live_phase_entries(), arg_path(), arg_paths(), arg_string(), arg_u32(), arg_usize() (+30 more)

### Community 3 - "lay_nanda_wave_eval.rs"
Cohesion: 0.10
Nodes (44): append_left_exact(), append_morphology_role(), append_neighbor_role_and_tail(), append_neighbor_shape(), append_neighbor_tail(), build_and_prove_sentence_context_path(), candidate_that_changes_surrounding_context_is_excluded(), evaluate_heldout() (+36 more)

### Community 4 - "static_models.rs"
Cohesion: 0.06
Nodes (72): Sha256, Args, build_cache(), build_corpus(), check_cache(), check_corpus(), check_model(), Command (+64 more)

### Community 5 - "UnifiedCorrectionCandidate"
Cohesion: 0.12
Nodes (21): apply_focus_state(), listen_pointer(), Arc, AtomicU64, InputEvent, Option, PathBuf, Result (+13 more)

### Community 6 - "Lay L1-L4 Runtime Architecture"
Cohesion: 0.05
Nodes (41): 10. Proof and Release Gates, 11. Fast Verification Cadence, 12. Maintenance Rule, 13. Definition of Done, 1. Product Objective, 2. Non-Negotiable Laws, 3. Current Runtime Tree, 4.1 L1: surface observation (+33 more)

### Community 7 - "tests.rs"
Cohesion: 0.03
Nodes (5): measured_precognition_stages(), percentile(), precognition_candidate_generation_stays_under_budget(), LayIbusEngine, Vec

### Community 8 - "tray_support.js"
Cohesion: 0.05
Nodes (53): activateLayoutId(), currentLayoutKind(), focusedWindow(), focusedWindowInfo(), IBUS_SYNC_RETRY_MS, imeEngineForLayoutKind(), LayDaemonService, normalizeLayoutKind() (+45 more)

### Community 9 - "canonical_l2_recent.rs"
Cohesion: 0.05
Nodes (109): canonical_l1_l2_report_input_keeps_short_service_words(), canonical_l2_shadow_words_include_local_lexicon(), collect_generated_positive_candidates(), collect_shadow_lexicon_words(), collect_synthetic_expected_words(), collect_words(), decode_fixture_spaces(), default_probes() (+101 more)

### Community 10 - "text_edit_tests.rs"
Cohesion: 0.08
Nodes (47): committed_separator_is_preserved(), last_token_start_byte(), plan_committed_tail_full_token_replacement(), plan_committed_tail_last_token_replacement(), plan_committed_tail_replacement(), plan_committed_whitespace_insertions(), Option, Vec (+39 more)

### Community 11 - "gate_candidate_with_source"
Cohesion: 0.17
Nodes (38): call_activate_layout(), call_current_layout(), call_dbus_activate_layout(), call_dbus_current_layout(), call_dbus_focused_window_info(), call_dbus_list_layouts(), call_dbus_ping(), call_dbus_replace_text() (+30 more)

### Community 12 - "eval_two_word_models.py"
Cohesion: 0.06
Nodes (63): append_source(), load_manifest(), main(), parse_args(), Namespace, Path, ambiguous_layout_candidates(), ascii_technical_score() (+55 more)

### Community 13 - "lexicon.rs"
Cohesion: 0.13
Nodes (48): ascii_words_from_iter(), common_en_guard_prefixes(), common_en_technical_words(), common_ru_words(), common_ru_words_iter(), common_ru_words_ordered(), extend_common_ru_words(), extend_ru_technical_loanwords() (+40 more)

### Community 14 - "PhaseReadout"
Cohesion: 0.34
Nodes (14): PhaseReadout, input(), learned_phase_competition_redistributes_l2_energy_without_growing_it(), lexical_anti_margin_destructively_orders_the_complete_lattice(), no_phase_competition_preserves_the_released_energy_budget(), normalized_margin_position(), normalized_phase_lattice_strength(), phase_lattice_strength_includes_the_negative_side_of_a_ready_lexical_field() (+6 more)

### Community 15 - "LlmWaveMemory"
Cohesion: 0.08
Nodes (30): DeferredTypingAssistContext, Arc, Device, Instant, Mutex, Option, VirtualDevice, WordBuffer (+22 more)

### Community 16 - "WordCandidate"
Cohesion: 0.09
Nodes (38): Eq, merge_pair_bank(), magnitude_overlap(), max_magnitude_overlap(), max_random_center_coherence(), phase_margin_micro_for_centers(), PhaseProfileBuilder, randomize_active_phases() (+30 more)

### Community 17 - "word_reader.rs"
Cohesion: 0.08
Nodes (63): is_common_ru_word(), is_ru_one_letter_function_word(), is_ru_short_function_word(), is_ru_short_preposition(), is_ru_short_pronoun(), glued_part_fixes(), glued_phrase_part_candidates(), push_glued_phrase_part_candidate() (+55 more)

### Community 18 - "KeyEvent"
Cohesion: 0.07
Nodes (38): map_opposite_events(), original_event_char(), Option, mixed_visual_latin_word_target_layout(), Option, same_key_latin_cyrillic_homoglyph(), mark_single_current_word_layout_if_stale(), mark_word_layout() (+30 more)

### Community 19 - "fixture_rows"
Cohesion: 0.07
Nodes (46): decode_fixture_field(), first_fixture_row(), fixture_data_lines(), fixture_lines(), fixture_lines_from_str(), fixture_path(), fixture_row_by_id(), fixture_rows() (+38 more)

### Community 20 - "LexicalPhaseMemory"
Cohesion: 0.08
Nodes (31): text_replacement_from_fixture(), typed_buffer_from_semicolon_fixture(), edit_navigation_boundaries_reset_word_buffer_before_next_autocorrect(), failed_manual_replay_keeps_already_pending_typing_assist(), leading_cli_option_token_is_ignored_until_space(), marks_current_word_after_replay_for_next_toggle(), successful_manual_replay_clears_already_pending_typing_assist(), test_text_context() (+23 more)

### Community 21 - "TypingErrorClass"
Cohesion: 0.07
Nodes (80): CandidateOrigin, CandidateEvidence, Into, l2_surface_candidate_cannot_apply_left_context_rewrite(), l2_surface_missing_letter_repair_from_dirty_surface_can_apply(), l2_surface_single_letter_repair_from_dirty_surface_can_apply(), TypingErrorClass, inserted_char_position_for_missing_letter() (+72 more)

### Community 22 - "candidate_sources.rs"
Cohesion: 0.07
Nodes (64): boundary_shift_source_keeps_tail_pair_eligible(), boundary_shift_transition_candidate(), canonical_l2_field_keeps_nonleader_neighbor_regressions_unselected(), canonical_l2_field_preserves_surface_parity_when_local_readout_abstains(), canonical_l2_field_route_uses_owned_surface_source_ids(), canonical_l2_field_self_prepares_l11_candidate_without_peak_context(), compatible_composite_typo_shape(), composite_russian_typo_candidate() (+56 more)

### Community 23 - "l3.rs"
Cohesion: 0.04
Nodes (114): adjusted_confidence(), applies_boundary_candidate(), applies_confident_layout_candidate(), applies_split_memory_candidate(), applies_verified_single_word_layout_candidate(), ascii_layout_target_has_authority(), best_context_candidate(), boundary_operator_coherence() (+106 more)

### Community 24 - "direct.rs"
Cohesion: 0.15
Nodes (19): add_rejected_fix_sources(), add_rejected_word_state(), add_state_authority_repel(), add_transition_counts(), add_usage_event_count(), context_ngram_keys(), context_word_key(), correction_feedback_events_from_jsonl() (+11 more)

### Community 25 - "config_runtime.rs"
Cohesion: 0.08
Nodes (40): active_auto_replace(), active_auto_switch_layout(), active_correction_engine(), active_correction_safety(), active_enter_autocorrect(), active_enter_autocorrect_from_env(), active_learning_log(), active_nanda_autocorrect() (+32 more)

### Community 26 - "fixture_rows"
Cohesion: 0.06
Nodes (78): E, Args, bind_socket(), Command, handle_client(), handle_request(), health_report(), HostedMemory (+70 more)

### Community 27 - "surface_motif_memory"
Cohesion: 0.23
Nodes (23): Map, RawFd, deadline_remaining(), focused_window_haystack(), focused_window_identity_from_json(), focused_window_json_is_ignored(), focused_window_state(), FocusedWindowState (+15 more)

### Community 28 - "fuzzy_known_word_candidates"
Cohesion: 0.24
Nodes (21): AmbiguityAuthorityDiagnostic, canonical_heldout(), CleanMissDiagnostic, CompositeClassMetrics, CompositeEvaluation, CompositeMissDiagnostic, evaluate_clean_parallel(), Evaluation (+13 more)

### Community 29 - "typing_assist_deferred.rs"
Cohesion: 0.13
Nodes (34): AbstainReason, ambiguity_shell_extends_the_tied_basin_without_granting_authority(), ambiguity_veto(), anti_evidence_cannot_collapse_an_objective_tied_basin(), anti_veto_preserves_the_full_certified_basin(), authority_evidence_is_consistent(), bounded_evidence(), calibration_can_abstain_without_inventing_a_winner() (+26 more)

### Community 30 - "tests.rs"
Cohesion: 0.12
Nodes (38): actions_path(), CandidateBeforeApplyRecord, compact_action_log(), DirtyTaskRecord, EditPlanRecord, gate_action_name(), home_relative_path(), input_gate_outcome_name() (+30 more)

### Community 31 - "read_entries"
Cohesion: 0.15
Nodes (21): buffered_suffix_fingerprint(), DaemonBufferedSuffix, DaemonInputObservation, DaemonMutationLease, DaemonMutationPolicy, DaemonMutationPreflight, DaemonMutationPreflight<'obs, 'buf>, DaemonTextContext (+13 more)

### Community 32 - "ContextPhasePackage"
Cohesion: 0.18
Nodes (32): canonical_runtime_package(), checksum64(), compact_center_roundtrip_preserves_runtime_coherence(), Cursor, decode_package(), empty_package_roundtrips_and_detects_corruption(), encode_package(), put_i32() (+24 more)

### Community 33 - "hot_field.rs"
Cohesion: 0.06
Nodes (34): daemon_auto_backend_uses_field_snapshot_only(), daemon_uinput_backend_can_use_full_reference_authority(), decode_authority(), decode_route(), encode_authority(), encode_route(), exact_surface_and_reconstructed_form_are_distinct_readouts(), hot_word_readout_does_not_need_full_dictionary_for_common_words() (+26 more)

### Community 34 - "TextReplacement"
Cohesion: 0.06
Nodes (30): 0. Canonical Live Owner Closure, 2026-08-01, 10. Forbidden Behaviors, 11. Canonical Summary, 12. 2026-07-27 Local Readout Safety Tightening, 13. 2026-07-27 Canonical Noun Package Full Safety Proof, 13. 2026-07-27 Full Russian Package And Live Lattice Boundary, 13. 2026-07-28 IBus L2 Cache Budget, 13. 2026-08-04 Internal Layout-Key Projection Contract (+22 more)

### Community 35 - "verifier.rs"
Cohesion: 0.14
Nodes (40): build_and_prove_context_phase_path(), build_and_prove_context_phase_path_with_surface_field(), classify_false_winner(), ContextPhaseCounterexample, ContextPhaseDifferentialProofReport, ContextPhaseProofReport, correct_is_unique_top(), DifferentialTotals (+32 more)

### Community 36 - "ManualTriggerEventContext"
Cohesion: 0.10
Nodes (37): log_manual_trigger_cross_check(), WordBuffer, ManualTriggerEventContext, ManualTriggerEventContext<'a>, ManualTriggerFireContext, PendingMultiTapTimeoutContext, Arc, Device (+29 more)

### Community 37 - "usage_prior.rs"
Cohesion: 0.13
Nodes (13): AntiPostingBlock, AntiPostingCursor, AntiPostingIndex, AtomBlockRange, compact_cursor_preserves_terminal_order_scores_and_skip_counts(), mask_before(), mask_from(), Option (+5 more)

### Community 38 - "candidate_gate.rs"
Cohesion: 0.14
Nodes (31): clear_last_live_completion_timing(), common_completion_outranks_rare_long_surface(), common_english_completion_survives_shared_gate(), is_live_lexical_surface(), l2_known_state_completion_keeps_a_valid_instrumental_form_first(), l2_lattice_births_decoded_morphology_from_prefix(), last_live_completion_timing(), live_completion_candidates() (+23 more)

### Community 39 - "l2_wave_peak.rs"
Cohesion: 0.14
Nodes (28): center_resonance(), context_words_before_last(), drift_anti_wave(), foundation_resonance(), known_surface_mass(), L2CorrectionPeakContext, L2WavePeakScore, layout_projection_does_not_receive_a_fixed_source_bonus() (+20 more)

### Community 40 - "LayIbusEngine"
Cohesion: 0.11
Nodes (21): PreeditFastState, auto_backend_enables_live_composition_for_running_ibus_engine(), client_capabilities_control_surrounding_text_authority(), engine(), handled_press_owns_its_matching_release(), ime_backend_enables_live_composition_independently_from_gray_precognition(), ime_backend_enables_live_composition_without_precognition(), LayIbusEngine (+13 more)

### Community 41 - "data_lines"
Cohesion: 0.06
Nodes (43): apply_manual_correction_output(), ManualCorrectionOutputContext, OutputFlow, Instant, Option, VirtualDevice, WordBuffer, Option (+35 more)

### Community 42 - "safe_extra_letter_candidates"
Cohesion: 0.16
Nodes (22): candidate(), correction_l2_word_candidates_impl(), ime_l2_completion_candidates_impl(), ime_l2_word_candidates_impl(), is_supported_lexical_surface(), l2_center_near_surfaces(), l2_ime_word_candidate_operator_priority(), l2_ime_word_candidate_score() (+14 more)

### Community 43 - "RecentActionGateTrace"
Cohesion: 0.18
Nodes (17): correct_contextual_known_word_missing_letter(), Option, String, correct_repeated_letter(), correct_short_repeated_function_word(), Option, String, generate_extra_letter_candidates() (+9 more)

### Community 44 - "correction_core.rs"
Cohesion: 0.12
Nodes (35): boundary_merge_split_is_verified(), boundary_shift_cannot_change_letters_or_import_context(), boundary_shift_is_verified(), changed_replacement_tokens_have_lexical_mass(), core_words(), EditTransitionInput, EditTransitionProof, exact_layout_projection() (+27 more)

### Community 45 - "gnome_dbus.rs"
Cohesion: 0.05
Nodes (54): enter_autocorrect_candidate_keeps_normal_english_word(), enter_autocorrect_candidate_rejects_multiword_scope(), enter_autocorrect_candidate_rejects_plain_layout_word_guess(), apply_typing_assist_to_text_tail_with(), ascii_hyphen_token_keycodes(), key_event(), key_event_with_shift(), key_events() (+46 more)

### Community 46 - "lay_debug_actions.rs"
Cohesion: 0.10
Nodes (37): append_selected_candidate_reasons(), blocked_unsafe_candidate(), candidate_report_counts_sources_and_bayes_records(), CandidateReport, ibus_trace_path(), is_final_gate_outcome(), is_slow_output(), legacy_applied_outcome() (+29 more)

### Community 47 - "decide_scoped_tail_correction"
Cohesion: 0.09
Nodes (37): assert_smart_pair(), layout_from_fixture(), typed_tail(), current_tail_case(), String, Vec, scoped_tail_keeps_completed_ascii_title_word_and_flips_current_latin_keys(), scoped_tail_keeps_good_english_previous_word_and_flips_current_layout_word() (+29 more)

### Community 48 - ".commit_active_composition_with_suffix"
Cohesion: 0.05
Nodes (99): CorrectionSourceRole, CandidateExplanation, explanation_for_candidate(), Self, String, Vec, TypingErrorEvent, UnifiedCorrectionCandidate (+91 more)

### Community 49 - "correct_wrong_layout_ascii_word"
Cohesion: 0.16
Nodes (19): converted_trailing(), core_is_layout_token(), correct_word_preserving_trailing_punctuation(), Option, String, has_ascii_shift_letter_signal(), is_ascii_layout_letter_symbol(), is_ascii_layout_token_symbol() (+11 more)

### Community 50 - "TailContext"
Cohesion: 0.08
Nodes (43): classifies_mixed_tail(), classify_token(), ContextToken, keeps_last_context_tokens(), Option, Self, String, Vec (+35 more)

### Community 51 - "prefs.js"
Cohesion: 0.11
Nodes (21): applyInputChannel(), BACKEND_OPTIONS, cellVisualLabel(), DEFAULTS, FORCE_KEY_OPTIONS, LAYOUT_BACKEND_OPTIONS, LayPreferences, LayPrefsView (+13 more)

### Community 52 - "candidate_quality.rs"
Cohesion: 0.12
Nodes (40): baseline_rank_tuple(), candidate_is_viable(), candidate_quality_reads_typed_outcome_before_candidate_admission(), candidate_quality_report_flags_trace_and_boundary_risks(), candidate_rank_tuple(), CandidateQualityReport, decision_outcomes_json(), inspect_arbitration() (+32 more)

### Community 53 - "lexical_surface_atoms.rs"
Cohesion: 0.17
Nodes (18): BoundarySlot, encode_boundary_atom(), encode_short_token_identity_atom(), EncodedSurfaceField, encoder_owns_symbols_and_atoms_together(), lower_token_chars(), normalize_surface_token(), FnMut (+10 more)

### Community 54 - "trace.rs"
Cohesion: 0.18
Nodes (28): AsRef, debug_enabled_cached(), enabled(), json_string(), manual_toggle_route(), record(), record_capabilities(), record_committed_tail_replace() (+20 more)

### Community 55 - "surface.rs"
Cohesion: 0.09
Nodes (65): is_ru_live_protected_word(), is_user_protected_word(), corrected_prefix_frontier_contains_attested_comparative(), l2_center_contains_surface(), add_phase_birth_delta(), annotate_typed_damage_support(), apply_learned_transition_pressure(), candidate() (+57 more)

### Community 56 - "compiler.rs"
Cohesion: 0.13
Nodes (38): J, ArtifactParts, assign_best_terminals(), bounded_postings_put_stronger_corpus_support_first(), compile_centers(), compile_decoder(), compile_words(), compile_words_with_training() (+30 more)

### Community 57 - "lay"
Cohesion: 0.05
Nodes (37): Bazzite / Fedora Atomic, CLI, English, lay, License, Быстрый старт, Возможности, Документация (+29 more)

### Community 58 - "tail_memory.rs"
Cohesion: 0.06
Nodes (49): events_for_ascii(), manual_decoder_does_not_apply_visual_b_auto_replace_to_replay(), manual_decoder_keeps_replay_as_explicit_user_command(), manual_decoder_uses_smart_tail_for_mixed_two_words(), Vec, typing_assist_context_decoder_keeps_edit_to_last_tail(), typing_assist_decoder_reemits_committed_space_boundary(), hunspell_ru_parser_keeps_common_lowercase_words_without_flags() (+41 more)

### Community 59 - "run_scenario"
Cohesion: 0.10
Nodes (60): adjacent_transposition(), build_lay_self_teacher_l3_report(), candidate_ladder(), candidate_matches_target(), candidate_order_is_stable(), candidate_snapshot(), clean_phrases(), clean_vocabulary() (+52 more)

### Community 60 - "completed_word.rs"
Cohesion: 0.07
Nodes (77): ascii_layout_prefix_can_be_letter(), boundary_shift_proposal(), BoundaryShiftProposal, correct_moved_prefix_letter_pair(), propose_moved_prefix_letter_pair(), Option, String, Option (+69 more)

### Community 61 - "decode_completed_tail"
Cohesion: 0.09
Nodes (48): accepted_fix_creates_negative_trace_for_corrected_away_word_only(), add_legacy_usage_counts(), add_usage_event_counts(), compact_usage_counts_for_persist(), context_ngram_prior_from_counts(), context_ngram_prior_from_keys(), context_ngram_prior_scores_partial_context_match(), full_rebuild_deduplicates_same_second_same_payload_events() (+40 more)

### Community 62 - "status.rs"
Cohesion: 0.12
Nodes (37): print_cases(), ablation_json(), build_status_json(), candidate_stats_json(), CandidateSourceStats, evaluate_deterministic(), l2_surface_memory_json(), l3_context_sample_cases() (+29 more)

### Community 63 - "llm_tests.rs"
Cohesion: 0.06
Nodes (38): convert_hybrid(), Box, Error, Option, Result, String, is_ascii_context_word(), next_word_segment() (+30 more)

### Community 64 - "TypingCandidateFamily"
Cohesion: 0.05
Nodes (43): 10. Compact Phase Memory, 11. Candidate-Relative Memory, 12. Whole-Token RU/EN Projection, 13. General Single-Grapheme Projection (`b/и`), 14. Joint Interference, 15. Learning Lifecycle And Package, 16. Causal Proof Matrix, 17. Scoreboard And Promotion Gates (+35 more)

### Community 65 - "text_to_key_events"
Cohesion: 0.09
Nodes (37): AnchorSequence, apply_geometry_certificate_interference(), apply_settlement_mode(), base_settled_energy(), bounded_tail_does_not_evict_operator_evidence_already_inside_limit(), bounded_tail_keeps_the_strongest_operator_evidence(), damerau_distance(), damerau_distance_heap() (+29 more)

### Community 66 - "PendingTypingAssist"
Cohesion: 0.11
Nodes (43): active_composition_autocorrect_can_use_nanda_fallback(), active_composition_autocorrect_uses_unified_input_gate(), active_composition_context_replacement_keeps_previous_words_out_of_commit(), active_composition_decision_returns_only_live_text_replacement(), active_composition_gate_text(), active_composition_gate_text_preserves_committed_prefix_for_decision_only(), active_english_layout_does_not_protect_internal_layout_letter_symbol(), active_english_layout_preserves_known_ascii_token_from_layout_projection() (+35 more)

### Community 67 - "manual_toggle.rs"
Cohesion: 0.13
Nodes (20): BufferFilterContext, log_ignored_key(), KeyCode, Option, should_skip_buffer_input(), is_keyboard_context_shortcut(), is_pointer_context_key(), KeyCode (+12 more)

### Community 68 - "settings.js"
Cohesion: 0.14
Nodes (18): app, applyInputChannel(), cellVisualLabel(), choice(), DEFAULTS, loadConfig(), loadNandaWaveStatus(), NANDA_WAVE_STATUS_FALLBACK (+10 more)

### Community 69 - "String"
Cohesion: 0.05
Nodes (55): ClusterUpdate, CompetitionCalibrationCase, CompetitionCalibrationReservoir, delta_projection_base_is_frozen_and_not_reemitted(), execute_l2_probe(), full_negative_bank_cannot_starve_positive_subcenters(), generalized_pair_requires_three_independent_exact_surfaces(), generic_negative_is_calibration_only_and_hard_evidence_is_pairwise() (+47 more)

### Community 70 - "desktop_probe.rs"
Cohesion: 0.06
Nodes (22): compile_context_phase(), compiler_learns_context_centers_and_destructive_competitors(), ContextPhaseCompileInput, package_roundtrip_keeps_compact_centers_without_words(), read_package(), directional_evidence_margin(), balanced_exact_profiles_leave_a_one_token_scene_unresolved(), compiled_hot_context_readout_stays_inside_microsecond_budget() (+14 more)

### Community 71 - "DaemonLoopState"
Cohesion: 0.25
Nodes (16): atom_center_keys(), atom_key(), normalize_surface(), phase_coherence_milli(), Vec, streaming_surface_summary_matches_materialized_atoms(), surface_phase(), surface_phase_and_atom_center_keys() (+8 more)

### Community 72 - "TypingAssistRuleConfig"
Cohesion: 0.04
Nodes (76): LayConfig, Vec, config_path(), LayConfig, Self, apply_auto_replace_policy(), normalize_typing_assist_pipeline(), Vec (+68 more)

### Community 73 - "TypingRuleContext"
Cohesion: 0.19
Nodes (14): ascii_context_can_host_layout_tail(), contextual_tail_allowed_by_previous_token(), LayoutSignal, previous_token_allows_layout_tail(), should_enable_ascii_to_ru_layout(), ContextToken, ContextToken<'a>, has_recent_russian_context_before_last() (+6 more)

### Community 74 - "mixed_script_repair.rs"
Cohesion: 0.26
Nodes (17): is_cyrillic_letter(), contains_mixed_alpha_token(), latin_chars_to_ru(), push_repaired_token(), repair_duplicate_latin_layout_prefix(), repair_mixed_ascii_token(), repair_mixed_russian_token(), repair_mixed_script() (+9 more)

### Community 75 - "WaveOptions"
Cohesion: 0.10
Nodes (33): ambiguity_center_reuses_the_fixed_record_without_changing_its_size(), AmbiguityPhaseCenter64, atom_wave_code_is_exactly_sixteen_bytes(), AtomWaveCode, BasisComponent16, BasisComponent8, ComplexBasisWave, put_i16() (+25 more)

### Community 76 - "replay_layout_decision"
Cohesion: 0.14
Nodes (28): canonical_replay_resolution(), decision_label(), arg_value(), candidate_summary_json(), full_user_target(), input_path(), normalized_text(), print_json() (+20 more)

### Community 77 - "input_gate.rs"
Cohesion: 0.21
Nodes (14): ablation_rows(), AblationRow, print_ablation_rows(), print_eval_summary(), print_failures(), print_gate_status(), print_layer_impact(), print_real_suite() (+6 more)

### Community 78 - "format.rs"
Cohesion: 0.16
Nodes (25): BinaryHeap, Ord, PartialOrd, ArcRecord, ArtifactHeader, CenterRecord, NodeRecord, put_u16() (+17 more)

### Community 79 - "mod.rs"
Cohesion: 0.19
Nodes (34): HeldoutReservoir, corpus_words(), corpus_words_from_lines(), crystallize_l1_lexical_grokking(), crystallize_l1_lexical_grokking_with_rss_budget(), crystallize_l1_lexical_grokking_with_surface_policy(), export_l1_fixed_latency_surfaces(), measure_hot_readout() (+26 more)

### Community 80 - "layout_controller.rs"
Cohesion: 0.13
Nodes (24): decode_completed_tail(), DecodedCompletedTail, build_input_gate_decoded_tail(), decode_input_gate_tail(), Option, word_boundary_correction_mode(), DecoderEditPlan, Option (+16 more)

### Community 81 - "is_cyrillic_word"
Cohesion: 0.18
Nodes (23): corpus_loader_keeps_only_positive_non_layout_rows(), EvalAccumulator, EvalRow, evaluate_rows(), latency_json(), load_rows(), measure_hot_latency(), merge_accumulator() (+15 more)

### Community 82 - "DaemonTextContext"
Cohesion: 0.12
Nodes (34): accumulate_binding_support(), calibrate_from_evidence(), compile_competition_edges(), compile_context_modes(), compile_l2_package(), compile_lemma_centers(), compile_neighbor_couplings(), compile_slot_centers() (+26 more)

### Community 83 - "runtime.rs"
Cohesion: 0.09
Nodes (30): Formatter, ArtifactBytes, decoder_reconstructs_training_form_absent_from_terminal_graph(), decoder_recovers_adjacent_transposition_as_typed_operator(), default_artifact_candidates(), default_artifact_path(), default_memory(), graph_reconstructs_surface_without_raw_word_table() (+22 more)

### Community 84 - "typing_replacements.rs"
Cohesion: 0.13
Nodes (20): ambiguity_geometry_link(), AmbiguityObservation, expanded_pair_residual_wave(), is_anchor_channel(), max_subcenter_coherence(), observed_sequence(), compile_basis(), complex_coherence_milli() (+12 more)

### Community 85 - "signal.rs"
Cohesion: 0.15
Nodes (27): Metadata, append_reads_only_new_complete_lines(), atomic_tail_compaction_reads_only_lines_after_overlap(), complete_prefix_len(), empty_snapshot(), initialize_cursor(), JournalBatch, JournalReadMode (+19 more)

### Community 86 - "eval.rs"
Cohesion: 0.15
Nodes (17): BufWriter, coupling_order(), MaterializedPostings, PostingRecord, PostingSpool, Drop, File, Option (+9 more)

### Community 87 - "typing_context_tests.rs"
Cohesion: 0.11
Nodes (25): Deref, anchor_sequence_hash(), CouplingView, ForwardActivation, geometry_reserve_keeps_the_nearest_basin_and_ambiguity_shell(), is_keyboard_channel(), LexicalGrokkingMemory, ObservedAtom (+17 more)

### Community 88 - "journal_record.rs"
Cohesion: 0.22
Nodes (19): TransitionAdmission, admission_truth_table_uses_verifier_latent_invariants_and_signed_l4_memory(), admit(), admit_with_l4_signal(), candidate(), event(), exact_l4_state_proof_allows_known_form_drift(), hidden_state_allows_unknown_to_known_typo_repair() (+11 more)

### Community 89 - "String"
Cohesion: 0.23
Nodes (22): CandidateContextEvidence, constructive_interference(), context_evidence(), context_preserving_next_token(), evaluate_candidate_with_memory(), evaluate_candidates_with_memory(), evaluate_candidates_with_phase(), evaluate_context_candidates_default() (+14 more)

### Community 90 - "state.rs"
Cohesion: 0.08
Nodes (23): 1. Role Split, 2. What L1.1 Does Not Own, 3. L2 Inputs, 4.10 Honest Tied And Abstain, 4.11 Deterministic Readout, 4.12 Attribution And Evidence, 4.13 Bounded Runtime, 4.14 Learnability (+15 more)

### Community 91 - "compiler.rs"
Cohesion: 0.09
Nodes (43): CompletionEditTrace, adjacent_usage_event_is_duplicate(), append_usage_event(), causal_feedback_summary(), compact_usage_events_if_needed(), correction_feedback_receipts_from_jsonl(), CorrectionFeedbackReceipt, enqueue_usage_persist() (+35 more)

### Community 92 - "ShiftState"
Cohesion: 0.15
Nodes (38): absolute_path(), acquire_manifest_write_lock(), admit_delta(), admit_delta_with_full_proof(), append_center_bank(), append_composite_pair_profile(), compact_manifest(), compose_base_with_deltas() (+30 more)

### Community 93 - "typing_candidate_tests.rs"
Cohesion: 0.07
Nodes (59): bank_support(), candidate_l2_signature(), candidate_l2_signature_for_schema(), candidate_semantic_relation_weight(), candidate_token_hash(), canonical_relation_scene_wave(), canonical_scene_wave(), canonical_scene_wave_scaled() (+51 more)

### Community 94 - "ForceLayoutHotkeyContext"
Cohesion: 0.26
Nodes (8): classify_scored(), MorphCalibration, MorphologyField, MorphReadout, MorphSlotScore, BTreeMap, Vec, ScoredSurface

### Community 95 - "typing_assist_mixed_corpus.rs"
Cohesion: 0.08
Nodes (46): decode_fixture_field(), first_fixture_row(), fixture_cases(), fixture_cols(), fixture_lines(), fixture_row_by_id(), fixture_rows(), fixture_tagged_cases() (+38 more)

### Community 96 - "agree_adjective_like_tail"
Cohesion: 0.10
Nodes (58): ndarray, add_group(), add_hashed(), add_hashed_residual(), add_keyboard_fourier_features(), add_layout_pairs(), add_mechanistic_features(), add_named() (+50 more)

### Community 97 - "transition.rs"
Cohesion: 0.08
Nodes (24): correction_source_name(), candidate_source_label(), correction_gate_runtime_stats(), correction_gate_stats(), correction_gate_stats_json(), CorrectionDecision, CorrectionDecisionSource, CorrectionGateAtomicStats (+16 more)

### Community 98 - "log"
Cohesion: 0.10
Nodes (36): empty_pair(), empty_profile(), EncodedL4Scene, L4CrossSceneContextSignal, L4CrossSceneDisposition, L4CrossSceneInput, L4CrossSceneL2Signal, L4CrossSceneObservation (+28 more)

### Community 99 - "String"
Cohesion: 0.15
Nodes (11): committed_tail_space_autocorrect_keeps_decision_core_authority(), ActiveCompositionAuthority, ActiveCompositionCommit, completion_with_space_does_not_trigger_autocorrect(), LayIbusEngine, Box, Option, Result (+3 more)

### Community 100 - "lay_nanda_dataset.rs"
Cohesion: 0.15
Nodes (48): add_keep_lines(), add_nanda_seed_cases(), add_phase_transition_surfaces(), add_short_alternating_cases(), add_technical_layout_cases(), alternating_expected(), build_cases(), build_rows() (+40 more)

### Community 101 - "correction_bayes.rs"
Cohesion: 0.25
Nodes (25): decode_package(), decode_package_owned(), decoder_rejects_pair_bank_over_budget(), encode_package(), generalized_pair_key_roundtrips_without_candidate_text(), profile_encoded_bytes(), read_centers(), read_i32() (+17 more)

### Community 102 - "lay-kde-tray.py"
Cohesion: 0.09
Nodes (24): RecentActionRecord, record_recent_action(), Instant, Option, Args, KeyCode, try_handle_enter_autocorrect(), DeviceGrabGuard (+16 more)

### Community 103 - "run_command_capture"
Cohesion: 0.14
Nodes (33): apply_feedback_overlay(), build_feedback_corpus(), compile_context_phase_delta_reader_with_projection_base(), compile_context_phase_reader(), compile_context_phase_reader_with_projection_base(), compile_context_phase_reader_with_surface_field(), compile_context_phase_reader_with_surface_field_and_schema(), ContextPhaseCompileReport (+25 more)

### Community 104 - "runtime_flags.rs"
Cohesion: 0.17
Nodes (20): NGramKey, build_compact_node(), channel_id(), graph_assigns_dense_deterministic_atom_ids(), key_symbols(), NGramArc, NGramNode, BTreeSet (+12 more)

### Community 105 - "apply_prepared_typing_assist_after_space"
Cohesion: 0.07
Nodes (60): OP, align8(), ArtifactBytes, base_bytes(), build_lazy_v8_package(), build_lazy_v8_package_with_shard_size(), build_shard(), build_shards_parallel() (+52 more)

### Community 106 - "CandidateOrigin"
Cohesion: 0.14
Nodes (21): AtomicBool, BudgetState, checkpoint(), current_rss_bytes(), high_budget_checkpoint_is_non_destructive_and_reusable(), Arc, AtomicU64, Drop (+13 more)

### Community 107 - "structural_relation.rs"
Cohesion: 0.17
Nodes (20): candidate(), evaluate_structural_relation(), is_guard_prefix(), looks_like_latin_single_letter_label(), relation_coherence(), relation_for(), relation_roles(), relation_verdict() (+12 more)

### Community 108 - "What You Must Do When Invoked"
Cohesion: 0.08
Nodes (24): For /graphify add and --watch, For /graphify query, For the commit hook and native CLAUDE.md integration, For --update and --cluster-only, /graphify, Honesty Rules, Interpreter guard for subcommands, Part A - Structural extraction for code files (+16 more)

### Community 109 - "assert_hyphen_case_keeps_undo"
Cohesion: 0.12
Nodes (20): benchmark_diverse_restoration(), benchmark_host_once(), benchmark_package(), candidate_fingerprint(), candidate_json(), compile_surface_indices(), inspect_package_header(), L1OverlayMemory (+12 more)

### Community 110 - "LayIbusEngine"
Cohesion: 0.11
Nodes (3): LayIbusEngine, String, Vec

### Community 111 - "pattern_wave.rs"
Cohesion: 0.17
Nodes (19): candidate(), evaluate_pattern_wave(), has_guarded_layout_shape(), local_match_energy(), pattern_blocks(), pattern_class(), pattern_resonance(), pattern_verdict() (+11 more)

### Community 112 - "layout_niri.rs"
Cohesion: 0.10
Nodes (39): data_lines(), Item, Iterator, are_ru_keyboard_neighbors(), Option, ru_keyboard_position(), center_contains(), adjective_form_suffixes() (+31 more)

### Community 113 - "hunspell.rs"
Cohesion: 0.19
Nodes (13): Arc, AtomicU64, Box, Mutex, Option, Receiver, Self, SyncSender (+5 more)

### Community 114 - "hot.rs"
Cohesion: 0.09
Nodes (40): K, RejectedStateEvidence, add_hot_context_counts(), add_hot_state_authority_repel(), add_hot_transition_counts(), context_ngram_ids(), context_ngram_prior_from_hot(), parse_context_word_key() (+32 more)

### Community 115 - "ime_hit_rate.rs"
Cohesion: 0.15
Nodes (21): ibus_debug_path(), ImeHitRateReport, inspect_timing(), is_complete_ru_eval_token(), is_eligible_ru_prefix(), is_weak_single_suffix(), percent(), percentile_block() (+13 more)

### Community 116 - "technical.rs"
Cohesion: 0.18
Nodes (31): competing_operator_consensus_classes_abstain(), context_relation_can_resolve_without_a_word_rule(), hypothesis(), HypothesisClass, independent_operator_consensus_resolves_one_semantic_class(), L4ActiveHypothesis, L4ActiveResolutionStatus, L4ResolutionCertificate (+23 more)

### Community 117 - "journal.rs"
Cohesion: 0.16
Nodes (26): append_trace(), append_trace_nonblocking(), CellScore, CellScoreboard, compact_trace_if_needed(), home_path(), load_recent_traces(), load_scoreboard() (+18 more)

### Community 118 - "typing_memory.rs"
Cohesion: 0.07
Nodes (44): accepted_events(), accepted_fix_events_mark_positive_result_and_source(), accepted_layout_projection_uses_the_runtime_operator_identity(), automatic_apply_is_censored_and_has_no_causal_episode(), changed_target_indexes(), common_prefix_chars(), CompletionEditTrace, confirmed_ime_prediction_is_positive_but_not_an_explicit_completion() (+36 more)

### Community 119 - "canonical_l1_l2.rs"
Cohesion: 0.16
Nodes (19): automatic_apply_is_not_positive_feedback(), compile_usage_feedback_snapshot(), compiled_feedback_snapshot_accepts_auto_undo_correction_receipt(), compiled_feedback_snapshot_restores_exact_negative_transition(), context_rejection_does_not_leak_into_empty_context_transition(), corrected_multiword_state_repels_candidate_with_a_different_context_split(), corrected_state_repels_unseen_future_candidate_but_accepts_user_target(), edited_ime_attracts_final_word_without_globally_rejecting_valid_suggestion() (+11 more)

### Community 120 - "focus_guard.rs"
Cohesion: 0.18
Nodes (19): ascii_letter_count(), ascii_letters(), has_ascii_digit(), has_ascii_hyphen_or_underscore_segments(), has_ascii_letter(), has_domain_like_dot(), is_ascii_technical_char(), is_ascii_technical_or_brand_token() (+11 more)

### Community 121 - "ManualCorrectionOutputContext"
Cohesion: 0.07
Nodes (66): is_common_en_technical_word(), candidate_support(), hot_layout_candidate(), hot_layout_candidate_with_noisy_projection(), hot_short_layout_candidates(), ime_l2_boundary_candidates(), ime_l2_completion_candidates(), l1_energy() (+58 more)

### Community 122 - "cyrillic.rs"
Cohesion: 0.19
Nodes (12): ForceLayoutHotkeyContext, ForceLayoutHotkeys, Arc, Duration, Instant, KeyCode, LayConfig, Mutex (+4 more)

### Community 123 - "NANDA Lexical Attractor Model"
Cohesion: 0.08
Nodes (23): Birth Gates, Bootstrap, Cold Surface Production Memory, Consolidation, Evaluation, Hot Lexical Attractor Memory, IME / Precognition Contract, L1: Sensors (+15 more)

### Community 124 - "CellTraceRecord"
Cohesion: 0.21
Nodes (12): current_word_is_known(), l1_surface_signal_detects_mixed_script_before_l2(), l1_surface_signal_preserves_current_event_shape(), L1ScriptProfile, L1SurfaceSignal, l2_backed_inflection_cannot_drift_to_unknown_surface(), latent_state_detects_context_tainted_candidate(), latent_state_separates_context_from_current_word() (+4 more)

### Community 125 - "explain_typing_assist_with_pipeline"
Cohesion: 0.06
Nodes (65): admit_l3_context_delta(), admit_l3_context_delta_with_full_proof(), balanced_l2_surface_words(), build_and_prove_l3_context_phase_memory(), build_and_prove_l3_context_phase_memory_with_surface_evidence(), build_and_prove_l3_sentence_context_memory(), build_l2_lexical_feedback_corpus(), build_l3_context_feedback_corpus() (+57 more)

### Community 126 - "layout_adapter.rs"
Cohesion: 0.23
Nodes (20): Counter, build_distractors(), corpus_support_repetitions(), corruption_variants(), extract_tatoeba(), interleave_files(), internal_index(), load_frequencies() (+12 more)

### Community 127 - "l4_goal_state.rs"
Cohesion: 0.15
Nodes (19): derive_l4_goal_state_trace(), derive_l4_scene_state(), goal_state_trace_with_memory(), is_command_like(), l4_goal_state_reads_corpus_memory_without_applying_text(), l4_scene_suggests_russian_typing_context(), l4_scene_waits_on_short_technical_context(), L4AllowedAction (+11 more)

### Community 128 - "Wave Transition CPU Plan"
Cohesion: 0.09
Nodes (21): 0. Freeze Baseline, 10. IME and Daemon, 11. Proof Suite, 12. Production Metrics, 1. Typed Transition Vocabulary, 2. Relation Atom Encoder, 3. L2 Phase-Center Transition Memory, 4. L4 Adaptive Surface Frontier (+13 more)

### Community 129 - "listen_keyboard"
Cohesion: 0.21
Nodes (18): ArchitectureGraph, build_receipt(), canonical_json(), check(), file_fingerprint(), freshness_comparable_receipt(), graph_freshness_violations(), graph_source_freshness_violation() (+10 more)

### Community 130 - "Vec"
Cohesion: 0.11
Nodes (23): assert_hyphen_case_keeps_undo(), mixed_prefix_case(), String, Vec, scoped_tail_collapses_cyrillic_prefix_before_ascii_hyphen_tail(), scoped_tail_repairs_mixed_cyrillic_prefix_ascii_hyphen_dative_word(), scoped_tail_repairs_mixed_cyrillic_prefix_ascii_hyphen_word_and_keeps_undo(), is_layout_decision_key() (+15 more)

### Community 131 - "previous_token_allows_layout_tail"
Cohesion: 0.20
Nodes (6): LayIbusEngine, Result, SignalEmitter, String, Vec, is_command_like_long_tail()

### Community 132 - "L1-L4 Intelligence Route"
Cohesion: 0.08
Nodes (24): Bayes / Usage Route, Candidate Sources, Commit Route, Completion Is Not Autocorrect, Core Law, Current Cutover Law, Current Progress Matrix, Debt Queue (+16 more)

### Community 133 - "PhysicalInputGrab"
Cohesion: 0.06
Nodes (22): LayConfig, apply_l3_feedback(), derive_l3_feedback(), feedback_summary(), FeedbackAdjustment, L3Feedback, looks_like_technical_tail(), String (+14 more)

### Community 134 - "LayImeBridge"
Cohesion: 0.16
Nodes (6): LayImeBridge, should_suppress_next_autocorrect(), Connection, Result, Shared, String

### Community 135 - "lay_ngram_corpus.rs"
Cohesion: 0.28
Nodes (14): candidate(), class_is_rejected(), estimate_hidden_typing_state(), extensionally_identical_transitions_share_one_semantic_class(), L4HiddenCandidateInput, L4HiddenDisposition, L4HiddenStateReadout, learned_phase_anti_center_makes_competing_states_authoritatively_ambiguous() (+6 more)

### Community 136 - "IME / Daemon Route Map"
Cohesion: 0.10
Nodes (19): Current Overlap, Daemon Decision Owners, Decision Owners Today, Desired Architecture, First Regression Set, IME / Daemon Route Map, IME Decision Owners, Main Finding (+11 more)

### Community 137 - "x11_layout.rs"
Cohesion: 0.24
Nodes (16): RustConnection, Backend, current_group(), current_layout_id(), group_for_layout(), layout_for_group(), lock_group(), lock_layout_id() (+8 more)

### Community 138 - "correction_memory_runtime.rs"
Cohesion: 0.16
Nodes (11): AssistedCorrectionMemory, ManualTextCorrectionMemory, PendingUndoCorrection, remember_assisted_text_correction(), remember_manual_text_correction(), remember_pending_auto_undo(), Option, T (+3 more)

### Community 139 - "main.rs"
Cohesion: 0.14
Nodes (31): InputGateCandidateScoreTrace, InputGateScoreboard, decide_input_gate(), decide_space_autocorrect(), disabled_sources_keep_original_on_boundary(), double_shift_is_visible_as_manual_toggle_operator(), InputGateAction, InputGateDecision (+23 more)

### Community 140 - "llmwave.rs"
Cohesion: 0.31
Nodes (7): normalize_lexical_surface(), String, apply_sequence_certificate_interference(), ForwardScratch, ReadoutMode, Vec, surface_operator_reconstruction_modes()

### Community 141 - "EvalCase"
Cohesion: 0.18
Nodes (25): AntiRelation, anti_relation_order(), coupling_order(), pair_center_order(), Ordering, coupling_order(), Ordering, WaveCoupling (+17 more)

### Community 142 - "l2_candidate_phase.rs"
Cohesion: 0.10
Nodes (45): abstain_demotes_all_owned_surface_candidates_not_only_reported_cohort(), abstain_demotes_lexical_authority_but_preserves_independent_layout(), apply_authority_to_candidate_lattice(), apply_standalone_l2_field(), boundary_text_candidates(), candidate_last_word_lower(), canonical_l2_field_fails_closed_without_l11_seeds(), canonical_owned_text_candidates() (+37 more)

### Community 143 - "text_mutation_monopoly_contract.rs"
Cohesion: 0.16
Nodes (24): assert_before(), candidate_before_apply_logs_use_typed_mutation_routes(), changed_check_runs_shadow_replay_release_gate(), collect_rs_files(), dead_ime_pending_space_autocorrect_route_stays_deleted(), decoder_edit_plan_carries_transition_audit_to_outputs(), dispatched_text_edit_cannot_fall_through_to_a_second_backend(), hidden_typing_state_is_apply_admission_authority() (+16 more)

### Community 144 - "Как работает lay"
Cohesion: 0.11
Nodes (19): Daemon runtime, Data files, DBus model, Desktop backend-и, Double Shift flow, GNOME extension, Known limitations, Learning (+11 more)

### Community 145 - "debug_log.rs"
Cohesion: 0.23
Nodes (18): append_private_line(), compact_to_max_bytes(), DebugLogLine, flush_interval(), flush_pending(), push_pending(), BTreeMap, Duration (+10 more)

### Community 146 - "TextEditBackend"
Cohesion: 0.12
Nodes (16): try_ime_replace_tail(), authorize_backend_edit(), AuthorizedEdit, BackendDispatchReceipt, BackendEditAuthorization, ExecutionBackend, only_an_undispatched_receipt_allows_backend_reselection(), From (+8 more)

### Community 147 - "apply_text_replacement_pipeline"
Cohesion: 0.18
Nodes (18): candidate_prefix_and_next(), combined_phrase_score(), l3_candidate_scoring_uses_long_phrase_context(), llmwave_apply_feedback_is_main_l3_signal(), longer_phrase_context_suppresses_short_wrong_association(), next_token_score_backs_off_to_recent_phrase_modes(), phrase_forecast_candidates(), phrase_forecast_completes_partial_next_word() (+10 more)

### Community 148 - "lay_memory_report.rs"
Cohesion: 0.21
Nodes (21): Args, count(), find_lay_processes(), json_opt(), kb(), main(), print_human(), print_json() (+13 more)

### Community 149 - "CharNgramModel"
Cohesion: 0.08
Nodes (22): EnterAutocorrectContext, Arc, Device, Instant, Mutex, Option, VirtualDevice, WordBuffer (+14 more)

### Community 150 - "TypingAssistExplanation"
Cohesion: 0.10
Nodes (30): CanonicalL2FieldReadout, classify_local(), coherence_milli(), compact_runtime_indexes_preserve_terminal_form_and_lemma_bindings(), contextual_multi_lemma_birth_can_select_a_weaker_seeded_lemma(), cross_lemma_authority_safety_verdict(), inverse_single_edit_lane_finds_package_forms_without_scanning_the_field(), L2FieldAuthority (+22 more)

### Community 151 - "composition_edit.rs"
Cohesion: 0.27
Nodes (11): char_to_byte_idx(), composition_cursor_at_start_does_not_swallow_backspace(), composition_cursor_backspace_edits_before_cursor(), composition_cursor_edits_inside_buffer_without_committing(), empty_composition_backspace_updates_memory_but_stays_unhandled(), engine(), LayIbusEngine, passive_cursor_move_forgets_committed_tail() (+3 more)

### Community 152 - "EnterAutocorrectContext"
Cohesion: 0.21
Nodes (16): assert_smart_insert_memory_case(), assert_undo_tail(), manual_text_correction_keeps_pending_full_undo(), Option, Self, String, Vec, WordBuffer (+8 more)

### Community 153 - "syntax_guard.rs"
Cohesion: 0.12
Nodes (14): candidate_index_for_target(), elapsed_us(), is_hard_precognition_boundary(), is_ime_candidate_russian_word(), is_ime_complete_russian_word(), PreeditFastState, push_unique_ru_known_suffix(), Instant (+6 more)

### Community 154 - "layout_sync.rs"
Cohesion: 0.16
Nodes (18): add_phase_replay(), collect_phase_examples(), phase_evidence_role(), phase_replay_gate(), PhaseEvidenceRole, PhaseReplayBucket, PhaseReplayExamples, PhaseReplayOutcome (+10 more)

### Community 155 - ".run"
Cohesion: 0.27
Nodes (13): decode_enter_autocorrect_tail(), decode_typing_assist_current_tail(), DecoderEditPlan, Option, CorrectionSource, CorrectionTrigger, changed_committed_tail_plan(), changed_committed_tail_plan_from_gate() (+5 more)

### Community 156 - "fixture_row_by_id"
Cohesion: 0.13
Nodes (20): facade_exposes_candidate_scoring(), candidate_tie_breaks_current(), choose_typing_candidate(), rank_typing_candidates(), I, Option, TypingCandidate, TypingCandidateDecision (+12 more)

### Community 157 - "VisibleTailSnapshot"
Cohesion: 0.15
Nodes (14): Into, Option, Self, String, snapshot_identity_keeps_runtime_coordinates_and_tail_hash(), SnapshotIdentity, stable_tail_hash(), tail_suffix() (+6 more)

### Community 158 - "LayImeBridge"
Cohesion: 0.15
Nodes (22): empty_identity(), recognize_token(), word_kind(), word_language(), WordIdentity, WordKind, WordLanguage, WordScript (+14 more)

### Community 159 - "apply_layout_replay"
Cohesion: 0.20
Nodes (20): calibrate_pair_threshold(), calibrate_profile_threshold(), causal_only_compiler_excludes_orphans_and_censored_centers(), compile_observations(), compiled_package_is_already_runtime_canonical(), ConsolidatedScene, contradictory_receipts_form_ambiguity_instead_of_authority(), CrossSceneCompileConfig (+12 more)

### Community 160 - "ManualOutputCommon"
Cohesion: 0.13
Nodes (35): CandidateReadoutRoute, Self, active_typing_safety(), Args, candidate_summary_json(), CandidateRouteArg, compare_candidate_routes(), compare_candidate_routes_for_input() (+27 more)

### Community 161 - "map_original_events"
Cohesion: 0.24
Nodes (12): aggregate_restoration(), ClassMetrics, EditGeometryMetrics, FailureDecomposition, merge_edit_geometry(), merge_evaluation(), merge_failure_decomposition(), merge_restoration() (+4 more)

### Community 162 - "main.rs"
Cohesion: 0.18
Nodes (21): CompletedProcess, acquire_single_instance_lock(), action_kind_label(), config_status_text(), daemon_active(), first_existing_command(), lay_version(), load_config() (+13 more)

### Community 163 - ".from_bytes"
Cohesion: 0.19
Nodes (16): authority_proposal(), long_surface_completion_needs_grounded_memory(), geometry_alone_cannot_authorize_an_unbound_single_letter_suffix(), live_admission_reason(), live_candidate_field_has_authority(), live_field_admission_is_owned_by_decision_core(), live_field_score_combines_l2_l3_l4_inside_decision_core(), live_l4_signed_bias() (+8 more)

### Community 164 - "apply_layout_replay"
Cohesion: 0.07
Nodes (58): append_user_correction_learning_log(), append_learning_entry_to_path(), append_learning_log_to_path(), append_user_correction_learning_log_to_path(), compact_learning_log_if_needed(), keep_jsonl_tail_bytes(), keep_last_jsonl_lines(), LearningEntry (+50 more)

### Community 165 - "state.rs"
Cohesion: 0.22
Nodes (17): derive_llmwave_feedback(), encode_memory(), encode_vocabulary(), hash32(), LlmWaveCandidateScore, LlmWavePhrasePrediction, LlmWavePreviousTokenCandidate, LlmWaveReport (+9 more)

### Community 166 - "candidate.rs"
Cohesion: 0.07
Nodes (26): active_path_preserve_handoff_is_shared_between_engine_objects(), backspace_turns_tab_completion_into_an_edit_trajectory(), close_committed_tail_field_clears_shared_tail_and_preserve_window(), committed_space_keeps_next_word_separated_in_tail_memory(), committed_tail_range_keeps_separator_outside_token(), early_stale_postcondition_waits_for_committed_surrounding_text(), empty_focus_reset_does_not_overwrite_preserved_shared_tail(), focus_engine_can_refresh_empty_tail_from_shared_handoff() (+18 more)

### Community 167 - "time.rs"
Cohesion: 0.14
Nodes (24): adjacent_swap_position(), bucket_position(), CorrectionReceipt, derives_extra_substitution_and_sparse_omission_geometries(), derives_transposition_without_storing_words(), excludes_layout_and_multiword_receipts(), is_single_word(), keeps_only_repeated_generic_missing_geometry() (+16 more)

### Community 168 - "LayIbusEngine"
Cohesion: 0.16
Nodes (20): agree_adjective_like_tail(), clean_ru_token(), grammar_agreement_candidates(), has_russian_verb_tail(), looks_like_plural_anchor(), looks_like_singular_anchor(), preposition_case_completion(), Option (+12 more)

### Community 169 - "Linux input correction research"
Cohesion: 0.14
Nodes (13): Architecture, Best practices for `lay`, Concrete roadmap from this research, Correction behavior, Linux input correction research, Phase 1: harden what exists, Phase 2: cleaner correction core, Phase 3: backend capability layer (+5 more)

### Community 170 - "replacement_tests.rs"
Cohesion: 0.25
Nodes (13): l3_phase_shadow_is_watch_only_trace(), llmwave_shadow_adds_trace_without_changing_default_decision(), run_wave_trace(), trace_has_all_layers(), trace_keeps_live_log_l2_boundary_and_attractor_drifts(), trace_keeps_single_all_caps_russian_term(), trace_layout_flip_uses_exact_english_reference_centers(), trace_layout_flip_works_in_both_directions() (+5 more)

### Community 171 - "lay_daemon.rs"
Cohesion: 0.05
Nodes (41): 10. Architecture Conclusion: What The Experiments Study, 11. References, 1. Problem Statement, 2. Evidence Levels, 3.1 Sparse associative memory, 3.2 Dense and modern Hopfield memory, 3.3 Sparse distributed memory and attention, 3.4 Exact dynamic pruning (+33 more)

### Community 172 - "l4_hidden_state.rs"
Cohesion: 0.26
Nodes (19): Cursor<'a>, decode_package(), decoder_surface(), read_calibration(), read_competition_edge(), read_context_mode(), read_form_ref(), read_lemma_center() (+11 more)

### Community 173 - "UsageEvent"
Cohesion: 0.21
Nodes (22): build_trace_record(), CellTraceCandidate, CellTraceCell, CellTraceMode, CellTracePattern, decision_kind(), l3_role(), merge_top_modes() (+14 more)

### Community 174 - "l3_context_metrics.rs"
Cohesion: 0.08
Nodes (31): advertised_surrounding_text_proves_the_delete_backend(), chromium_style_engine_without_delete_capability_rejects_bridge_preflight(), committed_tail_external_observation(), committed_tail_replace_state_sync_clears_stale_preedit_suffix(), CommittedTailOutputProfile, CommittedTailReplaceRequest, daemon_bridge_request_is_not_confused_with_ime_sources(), duplicate_replace_gate_allows_same_edit_for_new_original_tail() (+23 more)

### Community 175 - "TransitionAudit"
Cohesion: 0.15
Nodes (27): plan_recorded_undo_edit(), String, stale_surrounding(), stale_tail(), verify_visible_text_transition(), candidate(), decide_text_transition(), ime_auto_undo_produces_an_executable_recorded_undo() (+19 more)

### Community 176 - "check-architecture.sh"
Cohesion: 0.40
Nodes (13): assert_file_contains(), assert_live_correction_entrypoint_owned_by_input_gate(), assert_no_import(), assert_no_runtime_example(), assert_no_runtime_rule_id_literals(), assert_no_rust_fixture_phrase(), assert_no_rust_phrase_literal_in_file(), assert_single_owner() (+5 more)

### Community 177 - ".from_bytes"
Cohesion: 0.13
Nodes (22): apply_prepared_typing_assist_after_space(), apply_typing_assist_correction(), defer_complex_edit(), DecoderEditPlan, should_defer_immediate_typing_edit(), action_text_with_right_context(), apply_minimal_typing_replacement(), Option (+14 more)

### Community 178 - "preedit.rs"
Cohesion: 0.14
Nodes (32): apply_layout_replay(), Option, VirtualDevice, emit_backspaces(), emit_backspaces_fast(), emit_backspaces_for_text_replace(), emit_backspaces_for_text_replace_fast(), emit_key_taps() (+24 more)

### Community 179 - "experiment.py"
Cohesion: 0.13
Nodes (20): replacement_last_word_memory_ignores_middle_insert_plan(), DecoderEditPlan, Option, Self, String, ensure_committed_tail_spacing(), String, offset_replacement_plan_for_cursor() (+12 more)

### Community 180 - "UserLearningCorrection"
Cohesion: 0.16
Nodes (23): activate_ibus_engine(), activate_layout(), activate_layout_kde(), active_ibus_engine(), command_exists(), command_summary(), find_qdbus_command(), kde_layout_index() (+15 more)

### Community 181 - "Row"
Cohesion: 0.13
Nodes (28): observation(), candidate_relation_id(), context_signal_from_text(), encode_scene(), encoder_is_candidate_relative_but_application_independent(), keep_relation_id(), last_token(), punctuation_code() (+20 more)

### Community 182 - "precognition.rs"
Cohesion: 0.23
Nodes (20): KeyboardLayouts, Response, choose_layout_index(), chooses_ru_and_us_short_layout_names(), chooses_russian_and_english_layouts_by_name(), current_layout_name(), english_target_falls_back_to_first_non_russian_layout(), fetch_layouts() (+12 more)

### Community 183 - "http.rs"
Cohesion: 0.20
Nodes (20): feature_name(), CaseMetrics, embedded_small_corpus_proves_cross_lemma_declension(), evaluate(), morphology_coverage(), MorphologyCoverage, MorphologyProofReport, percent() (+12 more)

### Community 184 - "update"
Cohesion: 0.19
Nodes (26): AblationScore, build_word_cases(), build_word_observations(), candidate_permutation_parity(), ClassScore, context_at(), evaluate_ablations(), grapheme_data() (+18 more)

### Community 185 - "L4PhaseWitnessBank"
Cohesion: 0.14
Nodes (13): 0. Зафиксировать Baseline, 1. Закрыть Ownership L2, 2. Довести Онлайн-Обучение L3, 3. Расширить L3 до контекста предложения, 4. Типизированное ядро L4, 5. Обучаемый Cross-Scene L4, 6. Производительность без потери кандидатов, 7. Финальный Product Gate (+5 more)

### Community 186 - "l4_active_disambiguation.rs"
Cohesion: 0.13
Nodes (12): classify_typing_confidence(), Option, TypingCandidateDecision, TypingDecisionConfidence, Option, Self, String, TypingCandidate (+4 more)

### Community 187 - ".forward_queued_typing"
Cohesion: 0.11
Nodes (17): common_prefix_char_len(), confident_boundary_split_pair(), confident_boundary_split_sequence(), current_token_boundary_split(), current_token_boundary_split_or_repair(), current_token_repaired_boundary_split(), damerau_levenshtein_bounded(), damerau_levenshtein_impl() (+9 more)

### Community 188 - "WaveTrace"
Cohesion: 0.27
Nodes (14): Case, activate_layout(), activate_layout_kde(), choose_dialog_command(), dialog_args(), dict_env(), ensure_binary(), indent() (+6 more)

### Community 189 - "apply_word_rule"
Cohesion: 0.15
Nodes (13): autocorrect_replacement_has_one_trailing_space(), committed_tail_autocorrect_decision_is_authorized(), committed_tail_toggle_plan_uses_visible_ime_tail_not_old_daemon_buffer(), engine(), ime_auto_undo_contexts(), LayIbusEngine, manual_toggle_suppresses_next_boundary_autocorrect_once(), Option (+5 more)

### Community 190 - "balanced_l2_words_by"
Cohesion: 0.33
Nodes (9): balanced_l2_surface_words(), balanced_l2_words_by(), normalize_surface_bank_word(), RankedSurfaceWord, I, Option, String, Vec (+1 more)

### Community 191 - ".process_pressed_key"
Cohesion: 0.23
Nodes (5): is_completion_learning_boundary(), LayIbusEngine, Result, SignalEmitter, has_command_modifier()

### Community 192 - "Double Shift вместо ручного удаления: локальный помощник раскладки для Linux"
Cohesion: 0.17
Nodes (11): Double Shift вместо ручного удаления: локальный помощник раскладки для Linux, IME/preedit: почему это отдельный режим, Typing assist: помощь после пробела, Базовая идея: replay физических клавиш, Где начинается сложность, Почему это не просто "сделать Punto Switcher", Почему я не сделал LLM главным мозгом, Приватность (+3 more)

### Community 193 - "Double Shift вместо ручного удаления: локальный помощник раскладки для Linux"
Cohesion: 0.17
Nodes (11): Double Shift вместо ручного удаления: локальный помощник раскладки для Linux, IME/preedit: почему это отдельный режим, Typing assist: помощь после пробела, Базовая идея: replay физических клавиш, Где начинается сложность, Почему это не просто "сделать Punto Switcher", Почему я не сделал LLM главным мозгом, Приватность (+3 more)

### Community 195 - "layout_niri.rs"
Cohesion: 0.33
Nodes (10): flush_fragment(), FragmentStreamStats, is_fragment_boundary(), max_fragments_counts_only_accepted_fragments(), F, R, Result, Vec (+2 more)

### Community 196 - "server.rs"
Cohesion: 0.27
Nodes (8): Args, main(), Box, Error, Result, managed_input_enabled(), Result, run()

### Community 197 - "ime_candidate_readout.rs"
Cohesion: 0.04
Nodes (80): ascii_to_russian_layout_candidate(), AsciiToRussianLayoutCandidate, exact_known_projection_is_not_moved_to_a_morphological_neighbour(), l2_phase_covers_raw_projection(), Option, String, ascii_alpha_count(), ascii_segment_is_known_english_word() (+72 more)

### Community 198 - "config.rs"
Cohesion: 0.21
Nodes (20): augmentation_pair_hash(), DamageExample, extend_training_damages(), hybrid_policy_preserves_layout_lane_before_refilling_easy_classes(), push(), push_training_unique(), remove_positions(), BTreeMap (+12 more)

### Community 199 - "desktop.rs"
Cohesion: 0.19
Nodes (15): decision_output(), depth_bucket(), DepthMetrics, Metrics, output(), report_json(), report_json_with_jobs(), report_worker() (+7 more)

### Community 200 - "DirtyLogPair"
Cohesion: 0.21
Nodes (22): layout_after_replacement_plan(), automatic_lease(), automatic_mutation_lease_rejects_non_isolated_edit(), buffered_text(), completed_mixed_tail_continues_in_previous_context_layout(), delayed_mutation_preflight_rejects_changed_original_behind_cursor(), delayed_mutation_preflight_verifies_original_behind_observed_cursor_tail(), lease_context() (+14 more)

### Community 201 - "committed_tail.rs"
Cohesion: 0.20
Nodes (12): ManualToggleAuthority, PendingImeCompletionLearning, PendingSystemOutcomeFeedback, PendingVisiblePostcondition, RecentCommittedTailReplace, Instant, Option, Self (+4 more)

### Community 202 - "run_runtime_smoke.py"
Cohesion: 0.21
Nodes (19): active_composition_plan_does_not_append_separator(), committed_tail_plan_does_not_invent_missing_initial_letter(), committed_tail_plan_is_exact_layout_projection(), committed_tail_plan_keeps_internal_layout_symbol_in_the_token(), committed_tail_plan_preserves_separator(), daemon_word_buffer_plan_marks_daemon_source(), double_shift_replacement(), exact_projection_does_not_delete_separator_before_current_token() (+11 more)

### Community 203 - ".process_key_event"
Cohesion: 0.29
Nodes (5): Result, SignalEmitter, is_accept_completion_with_space_key(), is_key_press(), is_shift_key()

### Community 205 - "NANDA Wave Architecture"
Cohesion: 0.29
Nodes (6): Current Owners, Cut Order, Dependency Law, Goal, Lay Library Architecture, Refactor Budget

### Community 206 - "Форма Хабра"
Cohesion: 0.18
Nodes (10): GitHub, Заголовок, Короткое описание, Лид, Обложка, Примечание автора, Теги, Файл статьи (+2 more)

### Community 207 - "install.sh"
Cohesion: 0.31
Nodes (7): cleanup_legacy_ollama(), install_kde_autostart(), install_packages(), is_kde_available(), is_kde_session(), package_installed(), install.sh script

### Community 208 - "lay-runtime-control.sh"
Cohesion: 0.29
Nodes (10): activate_gnome_layout(), apply_channel(), current_gnome_layout(), select_lay_ime(), select_xkb(), lay-runtime-control.sh script, start_ime(), stop_ime() (+2 more)

### Community 209 - "architecture_contract.rs"
Cohesion: 0.29
Nodes (6): all_contract_lines_pass(), ArchitectureReceipt, receipt(), ReceiptCheck, String, Vec

### Community 210 - "autocorrect_edit_safety"
Cohesion: 0.07
Nodes (87): PhaseMass, accumulate_forward_scores(), AmbiguityCalibrationSamples, anchor_sequence(), AntiSearchStats, atom_couplings(), average_position(), backward_coherence_milli() (+79 more)

### Community 211 - "format.rs"
Cohesion: 0.16
Nodes (28): build_virtual_keyboard(), double_alt(), double_shift(), double_shift_enter(), double_shift_fast(), extra_fast_lshift_taps(), hold_tap(), hold_two_tap() (+20 more)

### Community 212 - "apply_layout_replay"
Cohesion: 0.29
Nodes (15): ExampleRow, FormRow, MorphCorpus, MorphExample, normalize(), normalize_context(), parse_corpus(), parser_preserves_multiple_surfaces_for_one_slot() (+7 more)

### Community 213 - "transition_relation.rs"
Cohesion: 0.08
Nodes (32): transition_changed_token_count(), aligned_changed_tokens_are_layout_projections(), changed_region(), common_suffix_chars(), composite_typo_verifier_rejects_distant_prefix_collapse(), encode(), exact_transition_id(), has_repeated_letter() (+24 more)

### Community 214 - ".run"
Cohesion: 0.11
Nodes (19): Atoms per input, Bounded runtime, Certificate-preserving bounded tail, Composite RU lexical v2 closure, Current full depth-0 package, Current L1.1 architecture and parameter snapshot, 2026-07-26, Depth-0 10k PASS, Depth-0 V7 compaction experiments, 2026-07-26 (+11 more)

### Community 215 - "UserLearningCorrection"
Cohesion: 0.04
Nodes (44): 2026-08-04 Confirmed Prediction Operator Sanitation, Append-only self-teacher promotion gate, 2026-07-30, Causal episode identity and impact-first L3 admission, 2026-08-01, Causal live-feedback reducer and mandatory full gate, 2026-07-31, Compact standalone L2 runtime indexes, 2026-08-03, Confirmed Feedback Corpus, Delta-free live L3 memory, 2026-08-03, Determinism And Snapshots (+36 more)

### Community 216 - "README.md"
Cohesion: 0.20
Nodes (5): Границы, Ник автора, Пакет публикации lay, Порядок публикации, Файлы

### Community 217 - "install-remote.sh"
Cohesion: 0.33
Nodes (7): checkout_repo(), install_rust(), install_system_packages(), kde_available(), main(), rpm_ostree_dependencies_available(), install-remote.sh script

### Community 218 - "Sync"
Cohesion: 0.08
Nodes (73): PhaseOperator, PhaseProofReports, anti_center_repels_same_operator_near_miss(), append_lexical_authority_atoms(), append_projection_atoms(), candidate_matches_operator(), causal_phase_atom(), collect_lexical_pair_reports() (+65 more)

### Community 219 - "correct_wrong_layout_ascii_phrase"
Cohesion: 0.06
Nodes (10): LiveCompletionTiming, record_observed_system_apply_usage(), ObservedSystemTransition, PhraseForecastCandidate, Option, Result, String, TypingCpuOptions (+2 more)

### Community 220 - "key_decode.rs"
Cohesion: 0.24
Nodes (9): changed_focus_receipt_quarantines_committed_tail(), cursor_driven_client_defers_preedit_until_cursor_ack(), engine(), identified_focus_publishes_preedit_without_cursor_ack(), narrow_cursor_uses_terminal_passthrough_profile(), LayIbusEngine, surrounding_text_client_publishes_preedit_immediately(), surrounding_text_client_uses_managed_commit_even_with_cursor_width() (+1 more)

### Community 221 - "key_decode.rs"
Cohesion: 0.18
Nodes (28): accepted_fix_prefers_the_causal_proposal_over_the_typed_source(), accepted_fix_requires_context_and_one_tail_change(), accepted_ime_never_binds_a_prior_rejection(), automatic_apply_is_not_online_learning_authority(), causal_episode_id(), completion_edit_geometry_is_valid(), CompletionEditTrace, direct_relation_observation() (+20 more)

### Community 222 - "EditAction"
Cohesion: 0.06
Nodes (41): classify_planned_replacement(), DecisionTransitionEditInput, downgrade_low_confidence_boundary_edit(), downgrade_low_confidence_wide_edit(), EditAction, EditActionKind, ime_accept_candidate_cannot_rewrite_left_context(), ime_accept_candidate_is_a_planned_edit_action() (+33 more)

### Community 223 - "UserLearningCorrection"
Cohesion: 0.10
Nodes (71): CompactDepth0Views, as_u32(), atom_bytes(), checksum(), Counts, decode(), decode_center_surface(), decode_compact_base() (+63 more)

### Community 224 - "profile_tests.rs"
Cohesion: 0.19
Nodes (16): default_phase_memory_path(), load_default_runtime(), PhaseEvaluator, read_array(), read_center(), read_i16(), read_u16(), read_u32() (+8 more)

### Community 225 - "recognize_token"
Cohesion: 0.16
Nodes (14): gate_action_label(), CorrectionCandidateScoreTrace, producer_allows_authority_evaluation(), EvaluatedTransitionInput, L1TransitionSignal, L2TransitionSignal, L3TransitionSignal, L4SignedTransitionSignal (+6 more)

### Community 226 - "graphify reference: extra exports and benchmark"
Cohesion: 0.22
Nodes (8): graphify reference: extra exports and benchmark, Step 6b - Wiki (only if --wiki flag), Step 7 - Neo4j export (only if --neo4j or --neo4j-push flag), Step 7a - FalkorDB export (only if --falkordb or --falkordb-push flag), Step 7b - SVG export (only if --svg flag), Step 7c - GraphML export (only if --graphml flag), Step 7d - MCP server (only if --mcp flag), Step 8 - Token reduction benchmark (only if total_words > 5000)

### Community 227 - "Optional multi-tap Shift scope"
Cohesion: 0.22
Nodes (9): Config, Goal, Non-goals, Optional multi-tap Shift scope, Recommendation, Runtime contract, Scope mapping, Test matrix (+1 more)

### Community 228 - "Distribution platforms"
Cohesion: 0.22
Nodes (9): AUR, Codeberg / Forgejo, Distribution platforms, Flathub, GitHub, GNOME Extensions, Nix / Nixpkgs / flake, openSUSE OBS / Fedora COPR (+1 more)

### Community 229 - "Lay Neural Arbiter Research Plan"
Cohesion: 0.22
Nodes (8): Completion Criteria, Contract, Dataset Sources, Goal, Lay Neural Arbiter Research Plan, Metrics, Models, Nanda/Grokking Audit

### Community 230 - "Rust Architecture Rules"
Cohesion: 0.22
Nodes (8): 1. Один вход в решение, 2. Вывод не смешивать с решением, 3. Состояние не переносить между backend, 4. Preedit только предлагает, 5. NANDA не вызывать россыпью, 6. Новый runtime-путь требует smoke case, 7. Рефакторинг идёт слоями, Rust Architecture Rules

### Community 231 - "factory.rs"
Cohesion: 0.31
Nodes (7): ObjectPath, LayIbusFactory, Connection, Result, Shared, String, safe_engine_name()

### Community 232 - "tests.rs"
Cohesion: 0.05
Nodes (90): four_cells_emit_per_symbol(), index_in_token(), Vec, run_l1(), run_l1_with_options(), correction_l2_word_candidates(), ime_l2_word_candidates(), run_l2() (+82 more)

### Community 233 - "Command"
Cohesion: 0.20
Nodes (17): l4_signed_memory_signal(), l4_signed_memory_signal_from_parts(), l4_signed_memory_signal_from_readout(), L4SignedMemoryInput, L4SignedMemoryReason, L4SignedMemorySignal, L4SurfaceStatus, Option (+9 more)

### Community 234 - "feedback.rs"
Cohesion: 0.16
Nodes (16): text_field_context_keeps_unfinished_words_separate_inside_same_window(), text_field_context_prunes_old_saved_slots(), text_field_context_separates_fields_without_window_identity(), window_input_state_keeps_separate_word_buffers(), push_text_as_layout(), replacement_memory_can_update_completed_words_without_dropping_current_word(), replacement_memory_keeps_space_boundary_after_i_autofix(), replacement_memory_preserves_current_after_deferred_completed_tail() (+8 more)

### Community 235 - "committed_tail.rs"
Cohesion: 0.20
Nodes (10): emit_forwarded_key_tap(), ForwardedTyping, PhysicalInputGrab<'a>, Device, KeyCode, Option, Result, Self (+2 more)

### Community 236 - ".what_to_replay"
Cohesion: 0.22
Nodes (26): accepted_usage_events(), arg_value(), candidate_short(), changed_word_indexes(), collect(), collect_replay_examples(), DirtyLogPair, latest_transition_states() (+18 more)

### Community 237 - "add_group"
Cohesion: 0.27
Nodes (6): LayImeBridge, Option, Result, String, VisibleTailSource, tail_source_for_authority()

### Community 238 - "core_tests.rs"
Cohesion: 0.25
Nodes (29): absolute_path(), acquire_manifest_write_lock(), admission_report(), admit_delta(), admit_tombstone(), CompositeSpec, DeltaEntry, initialize_manifest() (+21 more)

### Community 239 - "Архитектурный чек-лист lay"
Cohesion: 0.25
Nodes (8): Архитектурный чек-лист lay, Проверочный контур, Текущее дерево ответственности, Чек-лист лучших практик, Чек-лист рефакторинга, Что осталось резать дальше, Что проверять в коде, Что уже исправлено этой ревизией

### Community 240 - "Русские площадки для первой волны"
Cohesion: 0.25
Nodes (7): 1. Хабр, 2. Русские Linux-чаты, 3. OpenNET, 4. Linux.org.ru, 5. Tproger или похожее русское техмедиа, Не используем в первой волне, Русские площадки для первой волны

### Community 241 - "runtime_warmup_plan"
Cohesion: 0.22
Nodes (4): Default, Self, SurfaceWave4096, SurfaceWaveLane

### Community 242 - "choose_best_with_gap"
Cohesion: 0.21
Nodes (9): bank_score(), center_from_scene(), coherence(), merge_center(), MorphSlotProfile, observe_bank(), MorphPhaseCenter64, Default (+1 more)

### Community 243 - ".forward_queued_typing"
Cohesion: 0.08
Nodes (17): LanguageActionProof, From, Option, Self, TransitionAudit, TransitionOperator, TransitionProof, automatic_decision_pair_is_valid() (+9 more)

### Community 244 - "profile_tests.rs"
Cohesion: 0.13
Nodes (14): 10. Персональное Обучение, 11. Восстановление Кода, 12. Порядок Перехода, 13. Обязательные Гейты, 14. Конечное Состояние, 1. Единое Представление, 2. L1: Многоканальное Поле Поверхности, 3. L1 Grokking (+6 more)

### Community 245 - "tray_ui_contract.rs"
Cohesion: 0.22
Nodes (18): debug_log_label_is_action_journal_everywhere(), debug_log_switch_is_not_a_hidden_precognition_toggle(), diagnostics_owns_service_logging_and_recent_actions(), duplicate_auto_input_mode_is_legacy_only(), duplicate_force_layout_hotkeys_are_normalized_before_save(), nanda_button_opens_live_status_instead_of_static_help(), nearby(), opening_tray_refreshes_config_and_visible_selection_state() (+10 more)

### Community 246 - "Чек-лист публикации"
Cohesion: 0.29
Nodes (6): Не сейчас, Перед публикацией, Русские короткие анонсы, Русские техмедиа, Хабр, Чек-лист публикации

### Community 247 - "choose_token_consensus_with_chooser"
Cohesion: 0.43
Nodes (6): LayoutReplayMemory, manual_toggle_gate_trace(), remember_layout_replay_success(), Option, WordBuffer, append_learning_log()

### Community 248 - "ManualCorrectionOutputContext"
Cohesion: 0.06
Nodes (28): composite_damage_training_has_multiple_disjoint_surfaces(), split_damages(), zero_depth_selects_no_damaged_training_surfaces(), append_only_manifest_adds_centers_and_applies_tombstones_without_rewriting_base(), backward_reconstruction_contains_only_clean_reference_atoms(), complete_posting_ablation_reports_baseline_saturation_without_dropping_links(), compressed_forward_format_preserves_exact_readout(), damaged_evidence_cannot_change_the_primary_crystal() (+20 more)

### Community 251 - "text_metrics.rs"
Cohesion: 0.25
Nodes (21): CellTraceRecord, accepted_mode_reinforces_resonance_memory(), apply_observation(), ema(), failed_mode_suppresses_resonance_memory(), load_resonance_memory(), memory_path(), new_entry() (+13 more)

### Community 252 - "update.sh"
Cohesion: 0.36
Nodes (5): current_version(), installed_version(), preserve_local_changes(), refresh_runtime(), update.sh script

### Community 253 - "Development Notes"
Cohesion: 0.33
Nodes (5): Development Notes, Important Constraints, Project, Runtime Architecture, Useful Commands

### Community 254 - "graphify reference: query, path, explain"
Cohesion: 0.33
Nodes (5): For /graphify explain, For /graphify path, graphify reference: query, path, explain, Step 0 — Constrained query expansion (REQUIRED before traversal), Step 1 — Traversal

### Community 255 - "Short posts"
Cohesion: 0.33
Nodes (5): Habr intro teaser, OpenNET / Linux.org.ru style, Published links, Russian Telegram / Linux chat, Short posts

### Community 256 - "Text Mutation Monopoly Plan"
Cohesion: 0.33
Nodes (5): Debt Queue, Laws, Scoreboard, Text Mutation Monopoly Plan, Tree

### Community 257 - "render-publicity-demo.py"
Cohesion: 0.40
Nodes (5): FreeTypeFont, Image, draw_frame(), font(), main()

### Community 258 - "bump-lay-version.sh"
Cohesion: 0.60
Nodes (3): cargo(), bump-lay-version.sh script, usage()

### Community 259 - "check-gnome-extension-runtime.sh"
Cohesion: 0.47
Nodes (3): reload_extension(), check-gnome-extension-runtime.sh script, sync_files()

### Community 260 - "lay-host-vm-guard.sh"
Cohesion: 0.60
Nodes (5): daemon_active(), pause_daemon(), resume_daemon(), lay-host-vm-guard.sh script, viewer_running()

### Community 261 - "read"
Cohesion: 0.17
Nodes (22): candidate_admission_only_marks_eligibility_and_core_selects_transition(), candidate_lattice_merges_typed_evidence_without_owner_priority(), cold_learning_uses_exact_surface_attestation_not_form_settlement(), daemon_uses_typing_cpu_as_its_nanda_runtime_front_door(), decision_thresholds_and_tie_break_have_one_owner(), double_shift_exact_auto_undo_is_a_protected_first_priority_contract(), ime_preedit_uses_shared_candidate_readout_for_ranking(), ime_target_continuity_and_bridge_replay_share_state_transition_contracts() (+14 more)

### Community 262 - "active_typing_assist_pipeline_for_auto_replace"
Cohesion: 0.29
Nodes (9): warmup_plan_can_warm_full_typing_heap_for_uinput_daemon(), warmup_plan_does_not_wait_for_nanda_when_nanda_is_disabled(), warmup_plan_keeps_detect_only_ready_without_background_thread(), warmup_plan_prepares_boundary_memory_for_auto_ime_daemon(), LayConfig, Option, runtime_warmup_plan(), RuntimeWarmupPlan (+1 more)

### Community 263 - "make_ibus_text"
Cohesion: 0.36
Nodes (20): append_anchor_atoms(), append_bag_grams(), append_boundary_wave_atoms(), append_byte_wave_atoms(), append_character_wave_atoms(), append_keyboard_wave_atoms(), append_skip_grams(), append_wave_unit_grams() (+12 more)

### Community 265 - "attrs.rs"
Cohesion: 0.53
Nodes (5): empty(), ibus_attribute(), preedit(), Value, Vec

### Community 266 - "L3Feedback"
Cohesion: 0.08
Nodes (50): alpha_quality_is_low(), build_phrase_experience(), CleanCorpusIngestReport, contract(), decode_memory(), decode_vocabulary(), default_memory_path(), default_phrase_experience_path() (+42 more)

### Community 268 - "replacement_tests.rs"
Cohesion: 0.14
Nodes (17): correct_wrong_layout_cyrillic_word(), correct_wrong_layout_cyrillic_word_experimental(), correct_wrong_layout_cyrillic_word_with_policy(), english_layout_autoswitch_candidates(), EnglishLayoutPolicy, is_known_english_word_for_experimental_layout(), is_known_non_russian_to_english_layout_candidate(), is_plain_cyrillic_layout_token() (+9 more)

### Community 269 - "candidate.rs"
Cohesion: 0.28
Nodes (14): autocorrect_edit_safety(), changed_non_last_word(), checked_delete_range(), core_contains_space(), insertion_point_is_inside_word(), plan_preserves_unchanged_right_context(), replacement_plan_has_valid_cursor(), right_context_starts_at_token_boundary() (+6 more)

### Community 271 - "LLMWave Production Contour"
Cohesion: 0.40
Nodes (4): DEBT QUEUE, LLMWave Production Contour, SCOREBOARD, TREE

### Community 273 - "Text Correction Gate Architecture"
Cohesion: 0.11
Nodes (17): 2026-07-27 delayed surrounding-text observation, 2026-07-27 first live check: FAIL, 2026-07-27 generic exact autocorrect undo: PASS, 2026-07-27 L2 known inflection preservation: PASS, 2026-07-27 second live check: engine-path handoff loss, 2026-07-27 trigger-ownership proof, 2026-08-02 committed-tail capability preflight: PASS physical Chromium, 2026-08-02 Phase 7 deterministic input admission: PASS_CODE (+9 more)

### Community 274 - "Two-word model evaluation"
Cohesion: 0.40
Nodes (4): candidate_coverage, candidate_oracle, layout_rules, Two-word model evaluation

### Community 275 - "check-lay-changed.sh"
Cohesion: 0.47
Nodes (3): cargo(), has_file_matching(), check-lay-changed.sh script

### Community 280 - "LayoutBackend"
Cohesion: 0.20
Nodes (17): event_operation(), event_source(), event_state_word(), event_transition_context(), event_transition_source(), event_transition_target(), event_transition_weight(), event_word_is_changed_target() (+9 more)

### Community 281 - "graphify reference: add a URL and watch a folder"
Cohesion: 0.50
Nodes (3): For /graphify add, For --watch, graphify reference: add a URL and watch a folder

### Community 282 - "graphify reference: commit hook and native CLAUDE.md integration"
Cohesion: 0.50
Nodes (3): For git commit hook, For native CLAUDE.md integration, graphify reference: commit hook and native CLAUDE.md integration

### Community 283 - "graphify reference: incremental update and cluster-only"
Cohesion: 0.50
Nodes (3): For --cluster-only, For --update (incremental re-extraction), graphify reference: incremental update and cluster-only

### Community 285 - "feedback.rs"
Cohesion: 0.24
Nodes (12): handle_force_layout_hotkey(), Option, VirtualDevice, WordBuffer, prepare_uinput_output(), VirtualDevice, settle_after_physical_trigger_release(), release_possible_modifiers() (+4 more)

### Community 287 - "xml.rs"
Cohesion: 0.67
Nodes (3): component_exec_path(), component_xml(), String

### Community 288 - "WaveTrace"
Cohesion: 0.14
Nodes (15): applied_source_for_trace(), candidate_output_for_original(), case_changes_only_last_token(), decision_label(), expected_candidate_blocker(), flow_example(), l2_candidate_flow_worker(), L2CandidateFlowExample (+7 more)

### Community 299 - "NANDA Wave / клеточный мозг"
Cohesion: 0.67
Nodes (3): Ensemble Mode, NANDA Wave / клеточный мозг, Кто учит клетки

### Community 300 - "decode_ascii_tail"
Cohesion: 0.33
Nodes (11): get(), live_completion_cache(), LiveCompletionCacheEntry, LiveCompletionCacheKey, Mutex, Option, String, Vec (+3 more)

### Community 306 - "AGENTS.md"
Cohesion: 0.50
Nodes (3): Architecture evidence discipline, Cargo disk budget, graphify

### Community 320 - "handle_enter_autocorrect"
Cohesion: 0.25
Nodes (7): Candidate Decision, IME, L2/L3, Physical Lines, Refactor Baseline 0.2.264, Runtime Memory, Safety

### Community 326 - "apply_short_left_word_rule"
Cohesion: 0.08
Nodes (18): handle_pending_auto_undo(), remember_auto_undo(), Instant, Option, VirtualDevice, WordBuffer, Option, WordBuffer (+10 more)

### Community 335 - "Phase Word Recovery Baseline 0.2.241"
Cohesion: 0.22
Nodes (8): Known Architectural Gaps, L2 Candidate Flow, Live Candidate Latency, Phase Word Recovery Baseline 0.2.241, Provenance, Real Suite, Runtime Memory, Surface L2 Causal Baseline

### Community 336 - "Lay L2 Transition Phase Route"
Cohesion: 0.50
Nodes (3): candidate_triads, Lay L2 Transition Phase Route, triads

### Community 338 - "SurfaceWave4096"
Cohesion: 0.24
Nodes (16): apply_manual_correction_result(), complete_manual_trigger(), is_single_trigger_id(), ManualTriggerCompletion, Arc, Device, Instant, KeyCode (+8 more)

### Community 339 - "ComplexBasisWave"
Cohesion: 0.22
Nodes (16): has_known_cyrillic_hyphen_fragment(), is_cyrillic_hyphenated_word_for_layout(), is_known_cyrillic_hyphenated_word(), is_plain_cyrillic_technical_source(), is_plausible_cyrillic_hyphenated_word(), is_russian_hyphen_particle(), is_vowel_bearing_cyrillic_hyphen_token(), correct_ascii_prefix_with_ru_layout_tail() (+8 more)

### Community 341 - "correction_core.rs"
Cohesion: 0.42
Nodes (15): add_grouped_positive(), add_keep_lines(), add_ru_to_en_synthetic(), add_short_alternating(), add_three_col(), add_two_col(), decode(), load() (+7 more)

### Community 342 - "decode_typing_assist_tail"
Cohesion: 0.11
Nodes (24): has_safe_adjacent_transposition_candidate(), looks_like_present_tail(), looks_like_unsafe_chsh_deletion(), looks_like_unsafe_final_letter_deletion(), looks_like_unsafe_first_letter_deletion(), looks_like_unsafe_internal_y_deletion(), looks_like_unsafe_leading_pair_deletion(), looks_like_unsafe_vowel_join_deletion() (+16 more)

### Community 343 - "packet.rs"
Cohesion: 0.51
Nodes (10): choose_token_consensus(), choose_token_consensus_with_chooser(), choose_token_hybrid(), choose_token_hybrid_with_chooser(), Box, Error, F, Option (+2 more)

### Community 344 - "learning_deltas"
Cohesion: 0.17
Nodes (13): handle_space_press(), pending_crossed_following_word(), InputEvent, KeyCode, Option, WordBuffer, SpacePressContext, SpaceReleaseContext (+5 more)

### Community 345 - "Project families found"
Cohesion: 0.33
Nodes (6): 1. Old layout switchers, 2. Low-level keyboard daemons, 3. Input methods: IBus, Fcitx, Wayland text-input, 4. Text expanders and personal autocorrect, 5. Spelling correction mathematics, Project families found

### Community 346 - "manual_toggle.rs"
Cohesion: 0.21
Nodes (10): activate_gnome_layout_for_ime(), current_active_ime_layout_is_ru(), ime_engine_for_layout(), LayIbusEngine, manual_toggle_syncs_internal_layout_both_directions(), read_active_ime_engine(), Option, Result (+2 more)

### Community 347 - "Reconciler"
Cohesion: 0.83
Nodes (3): run_bin(), l3-self-teacher-promotion-gate.sh script, usage()

### Community 348 - "phase_proof_json"
Cohesion: 0.08
Nodes (58): bounded_tail_lines(), broad_user_rewrite_is_review_not_positive_training(), classify_operation(), collect_corrections_text(), collect_recent_text(), Collector, collects_layout_replay_as_manual_transition(), collects_typing_assist_with_selected_candidate_lanes() (+50 more)

### Community 349 - "apply_word_rule"
Cohesion: 0.21
Nodes (15): contribution_report_for_scenario(), contribution_scenarios(), ContributionReport, ContributionScenario, l1_sensor_cells(), l3_support_cells(), print_contribution_report(), print_ensemble_contribution_report() (+7 more)

### Community 350 - "handle_enter_autocorrect"
Cohesion: 0.27
Nodes (17): decode_learned_packet(), decode_slot(), encode_learned_packet(), encode_slot(), hash_text(), learned_packet_is_exactly_cell32_sized(), LearnedPacketEntry, operation_code() (+9 more)

### Community 351 - "uninstall.sh"
Cohesion: 0.60
Nodes (5): remove_gnome_input_sources(), remove_system_file(), run_runtime_cleanup(), uninstall.sh script, usage()

### Community 353 - "lexical_phase_compile.rs"
Cohesion: 0.22
Nodes (16): compile_lexical_phase_artifact(), english_lexical_surfaces(), lexical_phase_training_surface_inputs(), LexicalPhaseCompileReport, LexicalPhaseManifest, LexicalPhaseSourceDigest, Item, Iterator (+8 more)

### Community 355 - ".run"
Cohesion: 0.20
Nodes (12): ascii_letter_count(), clean_ascii_to_ru_layout_candidate(), contextual_ascii_to_ru_layout_candidate(), converted_layout_word_lower(), has_layout_punctuation_signal(), has_leading_layout_punctuation_signal(), has_trailing_layout_punctuation_signal(), Option (+4 more)

### Community 356 - "WaveTrace"
Cohesion: 0.67
Nodes (5): alphabet_for(), alphabet_predecessor(), alphabet_successor(), rotate_in_alphabet(), Option

### Community 358 - "russian_lexicon_tests.rs"
Cohesion: 0.32
Nodes (9): engine(), is_committable_char(), LayIbusEngine, passthrough_visible_char_keeps_ascii_prefix_for_completion_memory(), passthrough_visible_char_prefers_client_keyval_over_stale_layout(), physical_char_uses_selected_ime_engine_layout_before_client_keyval(), Option, x11_cyrillic_keysym_char() (+1 more)

### Community 360 - "LayImeBridge"
Cohesion: 0.09
Nodes (62): arg_value(), arg_values(), build_live_llmwave_memory(), candidate_short(), collect_json_string_fields(), collect_json_string_fields_from_str(), collect_json_value_string_fields(), corpus_percent() (+54 more)

### Community 361 - "verify_with_retry"
Cohesion: 0.08
Nodes (41): extra_letter_rule_defers_to_missing_letter_candidates(), has_generated_russian_typo_candidate(), has_plausible_russian_typo_candidate(), looks_like_prefix_plus_known_russian_word(), correct_missing_letter(), has_common_missing_letter_candidate(), missing_letter_candidate_exists(), Item (+33 more)

### Community 363 - "correct_vowel_confusion_impl"
Cohesion: 0.19
Nodes (9): Condvar, reconcile_postcondition(), Reconciler, Request, Arc, Mutex, Option, Self (+1 more)

### Community 365 - "CharNgramModel"
Cohesion: 0.20
Nodes (10): 5.1 Main Memory Objects, 5.2 FormCenterRef, 5.3 LemmaCenter, 5.4 MorphBinding, 5.5 LocalContextMode, 5.6 SlotPhaseCenter, 5.7 NeighborCoupling, 5.8 CompetitionEdge (+2 more)

### Community 366 - "score_typing_candidate"
Cohesion: 0.22
Nodes (13): CandidateTextPair, CandidateTextPair<'a>, classify_typing_rule(), edit_penalty(), family_weight(), internal_space_count(), intervention_penalty(), language_delta() (+5 more)

### Community 367 - "correction_core.rs"
Cohesion: 0.33
Nodes (6): 14. 2026-08-05 Bounded Typo Plus Boundary Repair, General contract, Measured facts, Verdict scope, What was not tested, What was tested

### Community 368 - "cargo-guard.sh"
Cohesion: 0.32
Nodes (4): CARGO_INCREMENTAL, check_budget(), monitor_budget(), cargo-guard.sh script

### Community 369 - "packet.rs"
Cohesion: 0.06
Nodes (71): LlamaSession, SessionParams, build_choice_prompt(), Choice, choose_candidate(), choose_candidate_direct(), choose_candidate_direct_impl(), direct_llm() (+63 more)

### Community 370 - "time.rs"
Cohesion: 0.16
Nodes (15): batch_context_readout_matches_individual_scores(), decode_record(), encode_record(), LlmWaveMemory, LlmWaveNextTokenScore, LlmWaveRecord, LlmWaveRuntimeIndex, phase_for() (+7 more)

### Community 371 - "precognition.rs"
Cohesion: 0.25
Nodes (7): Accepted Architectural Changes, Commands, Current Numbers, L1 Lexical Grokking Shadow Runtime, Remaining Gate, Runtime Boundary, Signal Tree

### Community 372 - "DecisionOutput"
Cohesion: 0.36
Nodes (11): collect_paradigms(), dictionary_words(), main(), noun_lexeme_forms(), parse_args(), Namespace, Path, slot_sort_key() (+3 more)

### Community 373 - "LayImeBridge"
Cohesion: 0.21
Nodes (14): default_manifest_path(), default_memory_lock(), default_memory_path(), load_default_composite(), maybe_reload_default_memory(), reload_default_memory(), Arc, FnOnce (+6 more)

### Community 376 - "eval.rs"
Cohesion: 0.25
Nodes (14): LayStats, load(), record_learning_log_entry(), record_learning_promotion(), record_llm_call(), FnMut, Option, Path (+6 more)

### Community 377 - ".from_bytes"
Cohesion: 0.19
Nodes (9): BirthAtom, birth_atoms_per_channel(), birth_posting_budget(), CachePlanOrder, candidate_birth_keeps_a_rare_budgeted_channel_frontier(), candidate_birth_stays_within_the_global_posting_budget(), first_touch_profile_word_count(), FirstTouchWarmProfile (+1 more)

### Community 378 - ".partition_frontier"
Cohesion: 0.19
Nodes (7): Debug, LexicalPhaseMemory, next_damerau_row_into(), ArtifactBytes, Option, Ordering, String

### Community 379 - "hunspell.rs"
Cohesion: 0.22
Nodes (19): hunspell_condition_matches(), hunspell_dic_entries(), hunspell_entries_skip_count_header_and_extract_optional_flags(), HunspellConditionToken, HunspellSuffixRule, load_hunspell_generated_forms_min_len(), load_hunspell_words_min_len(), load_simple_hunspell_suffix_rules() (+11 more)

### Community 381 - "LayoutBackend"
Cohesion: 0.30
Nodes (11): CachedLexicalCandidates, LexicalReadoutCache, cached_lexical_candidates(), l2_decoder_contains_surface(), LexicalReadoutMode, optional_decoder_contains_surface(), projected_lexical_candidates(), projects_settled_lattice_only_when_the_next_prefix_stays_dense() (+3 more)

### Community 383 - "composition_commit.rs"
Cohesion: 0.28
Nodes (4): ibus_text_value_to_string(), parses_ibus_text_surrounding_text(), Option, Value

### Community 384 - "cases.py"
Cohesion: 0.20
Nodes (11): authorized_l2_single_letter_suffix_reaches_ime_readout(), completion(), decision_core_is_the_live_completion_sort_owner(), ime_projection_preserves_single_core_order_and_deduplicates(), live_completion_has_authority(), live_suffix_has_display_authority(), String, Vec (+3 more)

### Community 387 - "tests.rs"
Cohesion: 0.30
Nodes (5): LatentScriptProfile, LatentTypingState, Self, String, Vec

### Community 388 - "read"
Cohesion: 0.60
Nodes (4): l3_context_metric_exposes_connection_authority_and_utility_separately(), l3_context_metric_has_an_isolated_causal_ablation(), read(), String

### Community 390 - "compare-l2-lexical-evals.py"
Cohesion: 0.43
Nodes (6): delta(), load(), main(), parse_args(), Namespace, Path

### Community 391 - "l3_online.rs"
Cohesion: 0.21
Nodes (19): arg_u64(), ensure_manifest(), enforce_relation_bound(), initialize_source_cursor(), load_state(), Paths, process_once(), replay_existing_feedback() (+11 more)

### Community 393 - "smart_insert_memory.rs"
Cohesion: 0.25
Nodes (13): build_record(), decision_kind(), informative_tick_keeps_word_boundaries_and_candidates(), is_informative_tick(), precognition_path(), PrecognitionCandidate, PrecognitionRecord, record_precognition_tick() (+5 more)

### Community 394 - "decode_ascii_tail"
Cohesion: 0.15
Nodes (13): cell_emits_top_k_modes(), class_bits(), coherence(), NandaCell32, Option, Self, sparse_mode_index(), SymbolStimulus (+5 more)

### Community 395 - "collect_recent_text"
Cohesion: 0.22
Nodes (7): ProbeJob, ProbeReceipt, L2ProbePool, Drop, JoinHandle, Receiver, SyncSender

### Community 396 - ".what_to_replay"
Cohesion: 0.22
Nodes (9): Args, main(), Box, Error, Option, PathBuf, Result, String (+1 more)

### Community 397 - "LayConfig"
Cohesion: 0.07
Nodes (41): handle_double_shift(), ManualCorrectionRequest, Option, VirtualDevice, WordBuffer, ScopedManualCorrectionRequest, Option, run_manual_correction_with_scope() (+33 more)

### Community 398 - "config.rs"
Cohesion: 0.36
Nodes (6): block_end_terminal(), exact_terminal_score(), Option, skip_terminal_block(), wand_collect_top(), WandCursor

### Community 399 - "apply_phrase_case"
Cohesion: 0.11
Nodes (28): empty_word_set(), is_known_cyrillic_hyphen_part(), is_known_short_accusative_a_form(), full_generated_forms_enabled(), full_russian_dictionary(), full_russian_generated_form_dictionary(), full_russian_short_dictionary(), full_russian_tiny_dictionary() (+20 more)

### Community 400 - "auto_undo.rs"
Cohesion: 0.16
Nodes (28): active_layout_backend(), active_text_backend(), call_focused_window_info(), call_ime_ping(), call_ping(), call_replace_text(), detect_auto_layout_backend_hint(), focused_ime_engine_handles_typing() (+20 more)

### Community 401 - "Args"
Cohesion: 0.17
Nodes (11): 10. Ambiguity Contract, 11. Rejected Variants, 1. Purpose, 2. Source Tree, 3. Typed Atom Memory, 4. Exact WordCenter64 Layout, 5. Shared Complex Basis, 6. Two-Wave Runtime (+3 more)

### Community 402 - "measured_precognition_stages"
Cohesion: 0.15
Nodes (13): 9.10 Causal fit-surface depth ablation, 9.11 Class-conditioned allocation, 9.1 Normative L1.0 baseline: V55, 9.2 L1.1 multimodal crystallization: E2 PASS, 9.4 L1.1 crystallization closure: bounded ambiguity shell, 9.5 Historical baselines, 9.6 Final bilingual corpus OOM VETO and bounded compiler, 9.7 Packed anti readout on the full candidate field (+5 more)

### Community 403 - "is_backed_russian_form"
Cohesion: 0.33
Nodes (6): 6.1 L2 Field Birth, 6.2 Same-Lemma Expansion, 6.3 Near-Neighbor Expansion, 6.4 Local Readout, 6.5 IME And Daemon Readout, 6. Canonical L2 Runtime Path

### Community 405 - "mod.rs"
Cohesion: 0.33
Nodes (6): 9.1 Stage A: Package Build, 9.2 Stage B: Reference Compare Readout, 9.3 Stage C: A/B Receipts, 9.4 Stage D: Runtime Flip, 9.5 Stage E: Remove Ownership Drift, 9. Cutover Plan

### Community 406 - ".train"
Cohesion: 0.22
Nodes (13): add_feature(), calibrate(), calibration_observation(), CalibrationObservation, candidate_order_does_not_change_morphology_readout(), hash_text(), Ordering, Result (+5 more)

### Community 407 - "layout_switch_policy.rs"
Cohesion: 0.15
Nodes (22): detect_auto_layout_backend_hint(), Option, log(), keyboard_device_paths(), log_startup_backends(), log_startup_mode(), make_virtual_keyboard_for_runtime(), probe_backends() (+14 more)

### Community 409 - "Lay Self Teacher L3 Plan"
Cohesion: 0.29
Nodes (6): Current Checkpoint, Lay Self Teacher L3 Plan, Promotion Gate, Reminder, Route, Work Items

### Community 410 - "ngram_tests.rs"
Cohesion: 0.18
Nodes (10): Baseline 300k Proof, Canonical Package Identity, Compact Records, Expanded 462k Proof, Honest Boundary, L2 Russian Morphology Phase, Learning, Ownership (+2 more)

### Community 411 - "DecisionTransitionReceipt"
Cohesion: 0.29
Nodes (7): Args, main(), Box, Error, Option, PathBuf, Result

### Community 412 - "read_entries"
Cohesion: 0.29
Nodes (10): arg_value(), canonical_dataset_contains_positive_and_negative_phase_rows(), canonical_dataset_proves_heldout_phase_transfer(), print_json(), read_entries(), Option, Path, Result (+2 more)

### Community 413 - ".what_to_replay"
Cohesion: 0.18
Nodes (7): finite_agreement_family_ignores_tense_and_mood(), MorphBinding16, parse_features(), pronoun_context_slot_projects_to_case_without_identity_features(), Result, String, valid_feature_shape()

### Community 414 - ".run"
Cohesion: 0.64
Nodes (8): append_private_text(), ensure_parent(), restrict_file_permissions(), Path, Result, write_private_bytes(), write_private_text(), writes_and_appends_private_text()

### Community 415 - "LayImeBridge"
Cohesion: 0.08
Nodes (31): facade_exposes_decoder_contract(), facade_exposes_minimal_text_replacement(), facade_exposes_physical_keyboard_mapping(), facade_exposes_replay_layout_decision(), facade_exposes_text_to_uinput_runs(), facade_exposes_word_buffer(), facade_exposes_word_event_splitting_and_text_tail(), key_events() (+23 more)

### Community 416 - "proof_chain.rs"
Cohesion: 0.23
Nodes (14): attempt_relation(), full_proof_passed(), inactive_compact_base(), persist_bound_full_proof(), proof_passed(), prove_full_differential(), Path, PathBuf (+6 more)

### Community 417 - "WordBuffer"
Cohesion: 0.32
Nodes (20): checksum64(), Cursor, encode_package(), fixture(), put_calibration(), put_competition_edge(), put_context_mode(), put_form_ref() (+12 more)

### Community 418 - "packet.rs"
Cohesion: 0.53
Nodes (5): current(), publish_runtime_config(), LayConfig, runtime_debug_action_log(), runtime_usage_learning_enabled()

### Community 419 - "autocorrect_edit_safety"
Cohesion: 0.43
Nodes (6): atomic_write_lines(), main(), normalize_surface(), parse_args(), Namespace, Path

### Community 420 - ".manual_toggle_active_text_target"
Cohesion: 0.23
Nodes (18): canonical_l2_status(), compile_canonical_l2_package(), default_l2_model_dir(), discover_installed_l2_package(), export_unseeded_l11_seed_corpus(), installed_l2_field(), prove_canonical_l2_package(), query_canonical_l2_package() (+10 more)

### Community 422 - "decode_ascii_tail"
Cohesion: 0.26
Nodes (9): bounded_context_key(), bounded_context_tokens(), context_mode(), punctuation_class(), Option, Vec, scene_wave(), stable_key() (+1 more)

### Community 423 - "runtime_flags.rs"
Cohesion: 0.33
Nodes (6): 9.12 Normative single-pass streaming crystallizer contract, Canonical final L1.1 configuration, Final hybrid-5 package VETO, Mandatory invariants, Non-regression boundary for the streaming rewrite, Required whole streaming route

### Community 424 - "text_edit_gate_contract.rs"
Cohesion: 0.24
Nodes (7): LayoutCapabilityPreflight, IntoIterator, Item, Option, Result, Self, String

### Community 425 - "add_mechanistic_features"
Cohesion: 0.29
Nodes (11): expected_correction_action(), manual_decision_case(), String, Vec, single_word_wrong_layout_replay_target_is_opposite_layout(), smart_decision_converts_mixed_layout_neighbor_only(), smart_decision_repairs_brand_plus_letter_inside_larger_tail(), smart_decision_replays_protected_ascii_span_as_manual_toggle() (+3 more)

### Community 426 - "precognition.rs"
Cohesion: 0.43
Nodes (6): atomic_write_lines(), main(), normalize_surface(), parse_args(), Namespace, Path

### Community 427 - "correct_wrong_layout_ascii_phrase"
Cohesion: 0.83
Nodes (3): cold_compiler_writes_a_loadable_artifact_and_matching_manifest(), PathBuf, unique_temp_dir()

### Community 428 - "live_candidate_gate_stats"
Cohesion: 0.15
Nodes (12): live_candidate_gate_stats(), live_candidate_gate_stats_json(), live_stats(), LiveCandidateGateAtomicStats, LiveCandidateGateStats, record_live_gate_stats(), AtomicU64, Instant (+4 more)

### Community 429 - "candidate_explanation.rs"
Cohesion: 0.09
Nodes (31): boundary_signature(), edit_shape(), explain_candidate(), explanation_prefers_boundary_preservation_over_shortcut_loss(), is_soft_punctuation(), lcs_len(), operator_fit_milli(), ratio_milli() (+23 more)

### Community 430 - ".default"
Cohesion: 0.50
Nodes (3): LayConfig, Default, Self

### Community 431 - "keyboard_io.rs"
Cohesion: 0.21
Nodes (13): evaluate_morphology_scenes(), ProofShard, prove_package(), BTreeMap, Path, Result, Self, String (+5 more)

### Community 432 - "flip_word_events"
Cohesion: 0.19
Nodes (9): bloom_seen_and_insert(), HashMap, Item, Iterator, Vec, surface_hashes(), SurfaceSpan, mix64_avalanche() (+1 more)

### Community 433 - "dict_tests.rs"
Cohesion: 0.40
Nodes (5): 3.1 L1.1 Ownership, 3.2 L2 Ownership, 3.3 L3 Ownership, 3.4 Verifier Ownership, 3. Ownership

### Community 434 - "L4PhaseWitnessBank"
Cohesion: 0.31
Nodes (6): collect_rust_files(), only_text_edit_gate_can_issue_or_attach_execution_receipts(), Path, PathBuf, Vec, runtime_text_edits_use_narrow_typed_plans_not_generic_proof_construction()

### Community 435 - "dict.rs"
Cohesion: 0.35
Nodes (10): build_ru_to_us(), build_us_to_ru(), convert(), detect_direction(), Direction, is_latin(), HashMap, String (+2 more)

### Community 436 - "correct_vowel_confusion_impl"
Cohesion: 0.67
Nodes (3): 9.3 Preserved nearest lattice, Deletion-aware multimodal reconstruction, Physical-key geometry

### Community 437 - "managed_ime_session"
Cohesion: 0.61
Nodes (7): ensure_engine(), read_current_layout_is_ru(), read_engine(), Result, String, switch_engine(), is_ru_layout_id()

### Community 440 - ".ru_l2_word_attractor_candidates"
Cohesion: 0.36
Nodes (9): decode_fixture(), grouped_reason(), push_grouped_case(), read_cases(), read_grouped_training_cases(), Path, Result, String (+1 more)

### Community 443 - "handle_enter_autocorrect"
Cohesion: 0.83
Nodes (3): describe_mismatch(), package_matches_contract(), resolve-l2-package.sh script

### Community 444 - "ManifestWriteLock"
Cohesion: 0.11
Nodes (22): L4PhaseWitnessReadout, accepted_word_usage_count_cached(), apply_usage_event_to_cache(), cached_usage_hot_state(), cached_usage_prior_snapshot(), context_word_usage_prior(), context_word_usage_prior_cached(), ensure_usage_cache_initialized() (+14 more)

### Community 445 - "types.rs"
Cohesion: 0.50
Nodes (4): 8.1 Required Proof Families, 8.2 Required Scoreboard, 8.3 Runtime Promotion Gate, 8. Proof Contract

### Community 447 - "l2_transition_phase_shadow_readout"
Cohesion: 0.33
Nodes (6): PendingImeAutoUndo, Instant, Option, String, SharedState, pending_ime_auto_undo_restores_exact_original_surface()

### Community 448 - "LayIbusEngine"
Cohesion: 0.33
Nodes (14): L2TeacherCorpus, normalize(), parse_features(), resolve_scene_lemmas(), Result, Self, String, Vec (+6 more)

### Community 449 - "L2ProbePool"
Cohesion: 0.44
Nodes (9): B, candidate_beats(), choose_best_with_gap(), rank_best_two(), RankedPair, F, I, Option (+1 more)

### Community 451 - "manual_toggle.rs"
Cohesion: 0.50
Nodes (8): Popen, current_ibus_engine(), managed_ime_session(), Path, restore_ibus_engine(), stop_all_lay_ibus_engines(), stop_managed_ime(), write_managed_ime_config()

### Community 452 - "ClassMetrics"
Cohesion: 0.17
Nodes (16): evaluate_cases(), evaluate_parallel(), extend_sparse_omission_ambiguity(), extend_sparse_omission_ambiguity_indexed(), is_ordered_subsequence(), populate_sampled_ambiguity(), ProofProgress, ProofProgress<'a> (+8 more)

### Community 453 - "split_ws_segments"
Cohesion: 0.32
Nodes (5): deferring_committed_tail_toggle_suppresses_next_boundary_autocorrect(), LayIbusEngine, Option, Result, SignalEmitter

### Community 454 - "gate_candidate"
Cohesion: 0.08
Nodes (34): decide_text_correction(), boundary_gate_admits_bounded_current_token_repair_and_split(), boundary_gate_does_not_split_known_single_word(), boundary_gate_does_not_split_known_word_inside_phrase(), boundary_gate_rejects_known_word_split_from_non_boundary_candidate(), boundary_gate_rejects_short_function_split_with_unknown_tail(), boundary_preserving_candidate_survives_explanation_gate(), composite_gate_blocks_same_tail_consonant_semantic_drift() (+26 more)

### Community 455 - "typing.rs"
Cohesion: 0.38
Nodes (10): double_shift_manual(), double_shift_manual_after(), physical_key_for_char(), Error, KeyCode, Result, VirtualDevice, tap_physical_char() (+2 more)

### Community 456 - "runtime_flags.rs"
Cohesion: 0.17
Nodes (14): default_typing_assist_rules(), Vec, parse_bool_fixture(), single_fixture_row(), apply_typing_assist_uses_explain_path(), default_pipeline_matches_rule_graph_metadata(), default_rule_priorities_are_unique_and_ordered(), experimental_pipeline_keeps_normal_word_boundary_pairs() (+6 more)

### Community 458 - "apply_l2_phase_shadow"
Cohesion: 0.31
Nodes (13): args(), collect(), context_for(), corpus_contexts(), corpus_documents(), feature(), is_single_edit_neighbor(), learned_neighbor_scenes() (+5 more)

### Community 459 - "convert"
Cohesion: 0.39
Nodes (7): assert_convert_fixture(), detect(), parse_direction(), preserves_unknown_chars(), ru_to_us_basic(), us_shift_punctuation_maps_to_physical_ru_letters(), us_to_ru_basic()

### Community 460 - ".what_to_replay"
Cohesion: 0.50
Nodes (4): I, Result, Self, String

### Community 461 - "l2_transition_phase_shadow_readout"
Cohesion: 0.29
Nodes (7): 13.1 Kernel Ownership, 13. 2026-07-29 Standalone RU L2 V6 Evidence Authority, 13.2 Compiled Package, 13.3 Fixed Heldout Proof, 13.4 Rejected Safety Experiments, 13.5 Verdict And Authority, 13.6 Full L1.1 -> L2 -> L3 -> Verifier Replay

### Community 462 - "choose_best_with_gap"
Cohesion: 0.39
Nodes (4): PathBuf, temp_home(), user_protected_ascii_word_is_not_overridden_by_layout_scoring(), user_protected_cyrillic_word_is_not_overridden_by_technical_ascii()

### Community 463 - ".ru_l2_word_attractor_candidates"
Cohesion: 0.52
Nodes (6): parse_u32(), parse_usize(), print_json(), Option, Result, String

### Community 464 - "preedit_readout.rs"
Cohesion: 0.11
Nodes (26): inactive_ime_token_is_complete_russian_word(), LayIbusEngine, PreeditFastState, Vec, four_letter_russian_prefix_can_use_wave_lookup(), preedit_candidates_suppress_noisy_single_letter_suffixes(), ImeCandidateProposal, ImeCandidateReadoutRequest (+18 more)

### Community 467 - "candidate_explanation.rs"
Cohesion: 0.15
Nodes (28): accepted_prior_from_count(), bayes_exposes_high_risk_reflexive_noise_without_deciding(), bayes_gives_clear_common_typo_stronger_evidence(), bayes_score_candidate(), bayes_score_candidate_with_readout(), BayesCandidateScore, boundary_split_keeps_its_likelihood_with_left_context(), boundary_transition_likelihood() (+20 more)

### Community 468 - "composition_commit.rs"
Cohesion: 0.25
Nodes (8): 12. Promotion Gate, Canonical V8 shard-1 shadow runtime, 2026-07-29, Final V8 working-gate proof, 2026-07-29, Fixed proof, Global L1.1 + L2 seed compaction, 2026-07-30, Grouped cache runtime, Phase 6 canonical first-touch runtime, 2026-08-02, Shadow service

### Community 469 - "text_edit_gate_contract.rs"
Cohesion: 0.33
Nodes (11): correct_contextual_ascii_conjunction_i(), has_hard_ascii_separator(), immediate_right_has_russian_phrase_support(), is_ascii_b_conjunction_candidate(), is_hard_context_barrier(), is_russian_phrase_support(), I, Option (+3 more)

### Community 470 - "desktop.rs"
Cohesion: 0.16
Nodes (27): command_exists(), Result, String, run_command_capture(), detect_auto_backend_hint(), find_qdbus_command(), layout_ids(), layout_index() (+19 more)

### Community 471 - "precognition.rs"
Cohesion: 0.11
Nodes (11): force_target_layout_for_replacement(), script_signal(), ScriptSignal, target_layout_for_replacement(), continuation_layout_after_completed_tail(), preferred_layout_for_text(), Option, String (+3 more)

### Community 472 - "ime.py"
Cohesion: 0.38
Nodes (5): l2_surface_memory_status(), L2SurfaceMemoryStatus, warm_up_ime_word_candidate_memory(), warm_up_prefixes(), warm_up_surface_motif_memory()

### Community 473 - "13. 2026-08-04 Standalone L2 First-Space Warmup"
Cohesion: 0.40
Nodes (5): 13. 2026-08-04 Standalone L2 First-Space Warmup, Measured facts, Verdict scope, What was not tested, What was tested

### Community 474 - "PendingRelation"
Cohesion: 0.20
Nodes (11): OnlineFeedbackStats, OnlineState, PendingRelation, BTreeMap, Default, relation(), Option, String (+3 more)

### Community 475 - "composition_commit.rs"
Cohesion: 0.19
Nodes (15): correct_invalid_adjective_tail(), is_pattern_term_stem(), looks_like_plausible_russian_past_tense(), looks_like_present_or_reflexive_verb(), rewrites_protected_pattern_term_stem(), Option, String, unknown_cyrillic_lower() (+7 more)

### Community 476 - "config_path"
Cohesion: 0.33
Nodes (5): Option, String, Vec, WaveDecision, WaveTrace

### Community 477 - "l2_transition_phase_shadow_readout"
Cohesion: 0.39
Nodes (7): compact_depth0(), compact_depth0_package(), depth0_compaction_removes_redundant_banks_and_rebuilds_complete_relations(), Path, Result, String, Value

### Community 478 - "apply_short_left_word_rule"
Cohesion: 0.25
Nodes (6): OsString, EnvGuard, Drop, Option, Path, Self

### Community 479 - "ManifestWriteLock"
Cohesion: 0.25
Nodes (13): find_all_keyboards(), find_all_pointers(), input_event_paths(), lock_virtual_keyboard(), Arc, Mutex, MutexGuard, Option (+5 more)

### Community 480 - "dict_tests.rs"
Cohesion: 0.33
Nodes (4): center_memory_recovers_typical_dirty_surfaces(), fuzzy_known_word_candidates(), String, Vec

### Community 482 - "13. Standalone Full-Neighbor V13, 2026-07-30"
Cohesion: 0.67
Nodes (3): 13.1 Release cutover 0.2.333, 13.2 Public V13 package distribution, 2026-08-01, 13. Standalone Full-Neighbor V13, 2026-07-30

### Community 484 - "decode_ascii_tail"
Cohesion: 0.14
Nodes (17): is_lang_vowel(), is_plausible_word(), score(), bad_english_text(), bad_russian_text(), good_english_text(), good_russian_text(), is_risky_consonant_insert_before_final_verb_tail() (+9 more)

### Community 486 - "handle_pending_auto_undo"
Cohesion: 0.40
Nodes (18): Copy, is_backed_clean_reference_form(), is_backed_ka_declension_form(), is_backed_russian_ch_verb_present_form(), is_backed_russian_consonant_alternating_form(), is_backed_russian_form(), is_backed_russian_imperative_i_form(), is_backed_russian_imperative_y_form() (+10 more)

### Community 487 - "memory.rs"
Cohesion: 0.52
Nodes (6): FnMut, verify_current_layout(), verify_gnome_layout_stack(), verify_gnome_shell_layout(), verify_layout_with_retry_config(), verify_with_retry()

### Community 488 - "make_ibus_text"
Cohesion: 0.57
Nodes (6): ibus_text(), make_ibus_text(), make_preedit_ibus_text(), String, Value, Vec

### Community 489 - "input_gate_space_contract.rs"
Cohesion: 0.52
Nodes (6): daemon_space_and_enter_decoders_share_input_gate_replacement_contract(), decide_space(), decide_space_deterministic(), isolate_live_usage_memory(), space_autocorrect_keeps_existing_public_gate_contract(), weak_known_word_drift_is_suggest_only_without_latent_state_proof()

### Community 490 - "learning_deltas"
Cohesion: 0.40
Nodes (6): learning_deltas(), learning_deltas_show_live_phrase_becoming_l3_route(), LlmWaveLearningDelta, Item, Iterator, strongest_learning_delta()

### Community 491 - "13. 2026-08-03 Inverse Length Birth and Tied-Cohort Authority"
Cohesion: 0.29
Nodes (7): 13. 2026-08-03 Inverse Length Birth and Tied-Cohort Authority, Installed state, Measured facts, Not tested, Rejected experiment, Tested change, Verdict and authority

### Community 492 - "syntax_guard.rs"
Cohesion: 0.33
Nodes (3): ime_manual_toggle_outcome_keeps_legacy_wire_format_at_the_boundary(), ImeManualToggleOutcome, Self

### Community 493 - "types.rs"
Cohesion: 0.15
Nodes (25): ManualOutputCommon, authorize_native_text_edit(), failed_native_output(), NativeReplaceAttempt, NativeReplaceOutput, remember_native_replace(), Option, String (+17 more)

### Community 494 - "first_internal_transposition_typo"
Cohesion: 0.18
Nodes (14): apply_auto_replace(), apply_typing_assist_to_text_tail(), Option, String, seed_test_replacements(), select_typing_assist(), select_typing_assist_exact(), typing_assist_moves_letter_from_next_word_back() (+6 more)

### Community 495 - "13. 2026-08-05 Atomic Space And Nonblocking L3 Refresh"
Cohesion: 0.50
Nodes (4): 13. 2026-08-05 Atomic Space And Nonblocking L3 Refresh, Candidate Birth And Blocking Points, Measured Facts, Scope And Gate

### Community 496 - "time.rs"
Cohesion: 0.47
Nodes (3): OnlineContextPhaseLearner, String, Vec

### Community 498 - "ManifestWriteLock"
Cohesion: 0.39
Nodes (8): ascii_events(), decode_ascii_tail(), manual_decoder_keeps_single_word_toggle_reversible(), manual_decoder_replaces_only_bad_word_in_mixed_pair(), ranked_decoder_handles_three_word_tail_without_retyping_good_prefix(), ranked_decoder_keeps_ascii_context_and_flips_uppercase_current_tail(), Vec, typing_assist_decoder_preserves_space_and_avoids_known_false_splits()

### Community 499 - "apply_l2_phase_shadow"
Cohesion: 0.33
Nodes (3): Self, String, TypingCandidate

### Community 500 - "14. 2026-08-06 Short-Function Boundary Shift And Space Timing"
Cohesion: 0.40
Nodes (5): 14. 2026-08-06 Short-Function Boundary Shift And Space Timing, Canonical Structural Gate, Evidence Scope, Observed Input Shape, Space Hot-Path Measurement

### Community 501 - "record_nanda_trace_if_enabled"
Cohesion: 0.40
Nodes (3): active_nanda_wave_options(), TypingCpuOptions, record_nanda_trace_if_enabled()

### Community 503 - "ManifestWriteLock"
Cohesion: 0.40
Nodes (4): manifest_writer_lock_serializes_independent_transactions(), ManifestWriteLock, Drop, File

### Community 504 - "CorrectionMode"
Cohesion: 0.29
Nodes (4): Self, CorrectionMode, ActiveCompositionGateConfig, Self

### Community 505 - "live_l3_context_score"
Cohesion: 0.29
Nodes (5): live_l3_context_score(), LiveGateRecord, LiveL3ContextReadout, LiveSignedOutcomeStats, Option

## Knowledge Gaps
- **730 isolated node(s):** `dev-reload.sh script`, `IBUS_SYNC_RETRY_MS`, `_uid`, `LayIndicator`, `NANDA_WAVE_STATUS_FALLBACK` (+725 more)
  These have ≤1 connection - possible missing edges or undocumented components.
- **56 thin communities (<3 nodes) omitted from report** — run `graphify query` to explore isolated nodes.

## Suggested Questions
_Questions this graph is uniquely positioned to answer:_

- **Why does `UnifiedCorrectionCandidate` connect `.commit_active_composition_with_suffix` to `ManualOutputCommon`, `transition.rs`, `.what_to_replay`, `replay_layout_decision`, `l2_candidate_phase.rs`, `.forward_queued_typing`, `TypingErrorClass`, `candidate_sources.rs`, `TypingAssistExplanation`, `journal_record.rs`?**
  _High betweenness centrality (0.060) - this node is a cross-community bridge._
- **Why does `WaveOptions` connect `PhysicalInputGrab` to `canonical_l2_recent.rs`, `L3Feedback`, `main.rs`, `candidate_sources.rs`, `l3.rs`, `layout_sync.rs`, `WaveTrace`, `state.rs`, `LayIbusEngine`, `replacement_tests.rs`, `surface.rs`, `status.rs`, `replay_layout_decision`, `input_gate.rs`, `phase_proof_json`, `apply_word_rule`, `transition.rs`, `LayImeBridge`, `tests.rs`, `.what_to_replay`, `ManualCorrectionOutputContext`, `l4_goal_state.rs`?**
  _High betweenness centrality (0.045) - this node is a cross-community bridge._
- **Why does `damerau_levenshtein()` connect `TypingErrorClass` to `l2_wave_peak.rs`, `DaemonLoopState`, `safe_extra_letter_candidates`, `score_typing_candidate`, `.commit_active_composition_with_suffix`, `TailContext`, `candidate_explanation.rs`, `l3.rs`, `autocorrect_edit_safety`, `candidate_sources.rs`, `surface.rs`, `ManualCorrectionOutputContext`, `.forward_queued_typing`?**
  _High betweenness centrality (0.043) - this node is a cross-community bridge._
- **Are the 129 inferred relationships involving `default_typing_assist_pipeline()` (e.g. with `action_log_writes_candidate_score_trace_from_input_gate()` and `typing_pipeline_with_disabled()`) actually correct?**
  _`default_typing_assist_pipeline()` has 129 INFERRED edges - model-reasoned connections that need verification._
- **What connects `dev-reload.sh script`, `IBUS_SYNC_RETRY_MS`, `_uid` to the rest of the system?**
  _730 weakly-connected nodes found - possible documentation gaps or missing edges._
- **Should `learning_loop.rs` be split into smaller, more focused modules?**
  _Cohesion score 0.13229018492176386 - nodes in this community are weakly interconnected._
- **Should `default_typing_assist_pipeline` be split into smaller, more focused modules?**
  _Cohesion score 0.07811921510551648 - nodes in this community are weakly interconnected._