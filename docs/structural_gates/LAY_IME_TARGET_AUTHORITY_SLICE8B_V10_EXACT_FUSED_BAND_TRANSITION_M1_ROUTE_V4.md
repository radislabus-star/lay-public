...
task_id: lay-v10-exact-fused-band-transition-m1-v4
domain: code
query: Check the owner-linked eight-route transition microproof and authority boundary
...
| s3 | U1 candidate | consumes | G1 equality bits and one packed u64 V10 state | M1 Exact Transition Input clause | 1.0 | fused implementation | exact candidate input | fused | m1-fused |
| s4 | U1 candidate | returns | exact next cells metadata minimum terminal distance and survive decision | M1 Exact Transition Output clause | 1.0 | fused implementation | exact candidate output | fused | m1-fused |
...
| s11 | M1 scoped evidence | admits_only | separate full-executor candidate paper contract | M1 Verdict final paragraph | 1.0 | scoped evidence | future paper gate | boundary | m1-boundary |
| s12 | M1 scoped evidence | does_not_admit | V12 full B runtime integration deployment or latency PASS | M1 Decision Input and Verdict boundary | 1.0 | scoped evidence | forbidden authority | boundary | m1-boundary |
...
| c3 | U1 candidate | consumes | G1 equality bits and one packed u64 V10 state | M1 Exact Transition Input clause | 1.0 | fused implementation | exact candidate input | fused | m1-fused |
| c4 | U1 candidate | returns | exact next cells metadata minimum terminal distance and survive decision | M1 Exact Transition Output clause | 1.0 | fused implementation | exact candidate output | fused | m1-fused |
...
| c11 | M1 scoped evidence | admits_only | separate full-executor candidate paper contract | M1 Verdict final paragraph | 1.0 | scoped evidence | future paper gate | boundary | m1-boundary |
| c12 | M1 scoped evidence | does_not_admit | V12 full B runtime integration deployment or latency PASS | M1 Decision Input and Verdict boundary | 1.0 | scoped evidence | forbidden authority | boundary | m1-boundary |

## notes

- V1 VETO, V2 size-only WATCH and V3 weak-link VETO receipts remain immutable.
- V4 links only same-owner U1 input/output and same-owner claim-boundary rows.
