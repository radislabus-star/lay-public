...
static const gchar *
atomic_admission_rejection (
        BusInputContext                    *context,
        const IBusAtomicCapabilityRecordV1 *capability,
        const IBusAtomicPriorReceiptV1      *prior_receipt,
        guint64                              input_event_identity)
{
    if (context->atomic_call_in_flight)
        return "call_in_flight";
    if (context->atomic_lineage_poisoned)
        return "lineage_poisoned";
    if (!atomic_consume_prior_receipt (context, prior_receipt))
        return "prior_receipt";
    if (!ibus_atomic_capability_record_v1_is_admitted (capability))
    ...
    if (context->atomic_focus_lineage_identity != 0 &&
        ...
        return "focus_lineage";
    }
    if (input_event_identity <= context->atomic_last_event_identity)
        return "event_identity";
    if (capability->lease.capability_epoch <
            context->atomic_last_capability_epoch) {
        return "capability_epoch";
    }
    if (capability->lease.native_transaction_epoch <=
            context->atomic_last_native_transaction_epoch) {
        return "native_transaction_epoch";
    }
    if (!atomic_snapshot_matches (context, capability))
        return "surrounding_snapshot";
    return NULL;
}
                                 ...
                                 GDBusMethodInvocation *invocation)
{
    ...
    gsize digest_size;
    GError *error = NULL;
    const gchar *admission_rejection;
    guint keyval;
    guint keycode;
    guint modifiers;
    guint64 input_event_identity;
    ...
    admission_rejection = atomic_admission_rejection (
            context,
            &capability,
            &prior_receipt,
            input_event_identity);
    if (admission_rejection != NULL) {
        g_message ("LAY_ATOMIC_IBUS_ADMISSION reject=%s "
                   "event=%" G_GUINT64_FORMAT
                   " last_event=%" G_GUINT64_FORMAT
                   " capability_epoch=%" G_GUINT64_FORMAT
                   " last_capability_epoch=%" G_GUINT64_FORMAT
                   " native_epoch=%" G_GUINT64_FORMAT
                   " last_native_epoch=%" G_GUINT64_FORMAT
                   " focus=%d engine=%d fake=%d "
                   "lease_snapshot=%d context_snapshot=%d",
                   admission_rejection,
                   input_event_identity,
                   context->atomic_last_event_identity,
                   ...
                   context->engine != NULL,
                   context->fake,
                   capability.lease.surrounding_snapshot_present,
