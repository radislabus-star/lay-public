...
LOCAL_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v8.py"
...
CONTROLLER_IMPLEMENTATION = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_REMOTE_CONTROLLER_V8_V1_2026-08-27/IMPLEMENTATION_RECEIPT.json"

RECEIPTS = ROOT / "docs/structural_gates/receipts"
ADMISSION_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_EXECUTION_ADMISSION_V1_2026-08-27"
BOOTSTRAP_AUDIT_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_BOOTSTRAP_AUDIT_V1_2026-08-27"
BUILD_AUDIT_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_BUILD_AUDIT_V1_2026-08-27"
QUIET_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_QUIET_ADMISSION_V1_2026-08-27"
TERMINAL_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_TERMINAL_AUDIT_V1_2026-08-27"
...
def fixed_local() -> dict[str, Any]:
    ...
    need(implementation.get("verdict") == "M3_V8_REMOTE_CONTROLLERS_VERIFIED_UNRUN", "controller implementation verdict drift")
    for key, path in {
...
out={
 ...
 'cargo':cmd(['/home/e/.cargo/bin/cargo','-V']),'rustc':cmd(['/home/e/.cargo/bin/rustc','-Vv']),
...
def validate_host(value: Mapping[str, Any]) -> None:
    ...
    need(value.get("cargo", {}).get("stdout") == "cargo 1.97.1 (c980f4866 2026-06-30)", "remote Cargo drift")
    rustc = str(value.get("rustc", {}).get("stdout", ""))
...
def live_admission() -> dict[str, Any]:
    ...
    probe_path = f"/home/e/.cache/lay-m3-v8-admission-{TRANSACTION_ID}"
    ...
    receipt = {
        "schema": "lay.m3-v8-execution-admission.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8_EXECUTION_ADMITTED",
...
def bootstrap_audit() -> dict[str, Any]:
    ...
    need(admission.get("verdict") == "M3_V8_EXECUTION_ADMITTED", "execution admission absent")
    ...
    need(receipt.get("verdict") == "M3_V8_BOOTSTRAP_CREATED_UNAUDITED", "remote bootstrap producer verdict drift")
    ...
    receipt_out = {
        "schema": "lay.m3-v8-bootstrap-audit.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED",
...
def build_audit() -> dict[str, Any]:
    ...
    need(bootstrap.get("verdict") == "M3_V8_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED", "bootstrap audit absent")
    snapshot = remote_snapshot()
    validate_host(snapshot)
    ...
    temporary = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v8-build-audit-"))
    try:
        ...
        need(provenance.get("verdict") == "M3_V8_BUILD_CREATED_UNAUDITED", "build producer verdict drift")
        ...
        elf = remote_build / "m3-v8-test-elf"
        ...
        receipt = {
            "schema": "lay.m3-v8-build-audit.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "M3_V8_BUILD_AUDIT_PASS_E2E_ADMITTED",
...
def quiet_admission() -> dict[str, Any]:
    ...
    need(build.get("verdict") == "M3_V8_BUILD_AUDIT_PASS_E2E_ADMITTED", "build audit absent")
    before = remote_snapshot()
    validate_host(before)
    ...
    receipt = {
        "schema": "lay.m3-v8-quiet-admission.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8_QUIET_HOST_E2E_ADMITTED",
...
def terminal_decision(subject: Mapping[str, Any], wrapper: Mapping[str, Any], build: Mapping[str, Any]) -> tuple[str, dict[str, list[str]]]:
    ...
    if wrapper.get("verdict") != "M3_V8_E2E_CREATED_UNAUDITED" or wrapper.get("controller_error") is not None:
    ...
    if build.get("verdict") != "M3_V8_BUILD_AUDIT_PASS_E2E_ADMITTED":
...
def terminal_audit() -> dict[str, Any]:
    ...
    temporary = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v8-terminal-"))
    try:
        ...
        receipt = {
            "schema": "lay.m3-v8-terminal-audit.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": verdict,
...
def self_check() -> dict[str, Any]:
    ...
    wrapper = {
        "verdict": "M3_V8_E2E_CREATED_UNAUDITED",
    ...
    build = {"verdict": "M3_V8_BUILD_AUDIT_PASS_E2E_ADMITTED", "elf_sha256": "a" * 64}
    ...
    return {
        "schema": "lay.m3-v8-independent-auditor-self-check.v1",
        "verdict": "M3_V8_INDEPENDENT_AUDITOR_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "auditor_sha256": sha256_file(AUDITOR),
...
def status() -> dict[str, Any]:
    return {
        "schema": "lay.m3-v8-auditor-status.v1",
        "verdict": "M3_V8_AUDITOR_STATUS",
        "receipts": {
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.m3-v8-auditor-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
