#!/usr/bin/env python3
"""Fault-closed local controller for the M3 dafsa-typed-view repair experiment."""
...
REMOTE_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-remote.py"
BOOTSTRAP_AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-bootstrap-audit.py"
BUILD_AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-build-audit.py"
TERMINAL_AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-terminal-audit.py"
FRAGMENT = ROOT / "scripts/lay_v10_e1_traversal_w1_dafsa_typed_view_m3_test_module.rs.inc"
D1_FRAGMENT = ROOT / "scripts/lay_v10_e1_remaining_cost_d1_test_module.rs.inc"
...
BOOTSTRAP_AUDIT_RECEIPT = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_"
    "BOOTSTRAP_AUDIT_V1_2026-08-27/M3_BOOTSTRAP_AUDIT_RECEIPT.json"
)
BUILD_AUDIT_RECEIPT = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_"
    "BUILD_AUDIT_V1_2026-08-27/M3_BUILD_AUDIT_RECEIPT.json"
)
TERMINAL_AUDIT_RECEIPT = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_"
    "TERMINAL_AUDIT_V1_2026-08-27/M3_TERMINAL_AUDIT_RECEIPT.json"
)
...
PREFLIGHT_SHA256 = "6f54103f12b197213839f3628a9b4eefdafce4e1ab4e8bb1a07e3a674316ad28"
PREFLIGHT_RECEIPT_SHA256 = "fbac92051b64ce78ae07c30ca34b1bf740b310b2c7420d433319fa1e8ff8eb41"
FRAGMENT_SHA256 = "5a2e164c47c88677b74baf44d500c939749c98deff3092a1621086cf6e800875"
ASSEMBLED_SHA256 = "56a0c3243ec8ea9fc3a4f8bf49c4b56f10e5ba38b1158026271dd34218f13aba"
...
ACTION_SPECS = (
    ...
    ("create-markers", "bootstrap-audit-upload", "atomic six-marker creation", None),
...
class M3ControllerError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise M3ControllerError(message)
...
def journal_metadata(admission: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-execution-journal.v1",
        "task_id": TASK_ID,
...
def append_intent(root: pathlib.Path, action_id: str) -> dict[str, Any]:
    ...
    intent = {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-action-intent.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
...
def append_completion(root: pathlib.Path, intent: Mapping[str, Any], response: Mapping[str, Any]) -> dict[str, Any]:
    ...
    completion = {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-action-completion.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
...
def controller_failure_payload(
    ...
    return {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-controller-failure.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def publish_controller_failure(
    ...
    try:
        write_json(stage / "M3_CONTROLLER_FAILURE_RECEIPT.json", receipt)
        write_json(
    ...
    return {
        **receipt,
        "receipt_sha256": sha256_file(CONTROLLER_FAILURE / "M3_CONTROLLER_FAILURE_RECEIPT.json"),
    }
...
def finalize_execution_journal(root: pathlib.Path, terminal: Mapping[str, Any]) -> None:
    write_json(
        root / "FINAL.json",
        {
            "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-execution-journal-final.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def run(command: Sequence[str], *, input_bytes: bytes | None = None, timeout: float = 3_600, check: bool = True) -> subprocess.CompletedProcess[bytes]:
    ...
    if check and result.returncode != 0:
        raise M3ControllerError(f"command failed ({result.returncode}): {shlex.join(command)}\n{result.stderr.decode(errors='replace')[-5000:]}")
    return result
...
def assemble_source() -> bytes:
    ...
    need(len(fragment) == 149_110 and sha256_bytes(fragment) == FRAGMENT_SHA256, "M3 fragment drift")
    need(sha256_bytes(fragment[:113_204]) == D1_FRAGMENT_SHA256, "D1 fragment prefix drift")
    assembled = v10[:-2] + fragment + b"}\n"
    need(len(assembled) == 240_628 and sha256_bytes(assembled) == ASSEMBLED_SHA256, "assembled source drift")
    need(sha256_bytes(assembled[:39_047]) == PRODUCTION_PREFIX_SHA256, "production prefix drift")
    return assembled


def verify_preflight() -> dict[str, Any]:
    preflight_row = require_file(PREFLIGHT, digest=PREFLIGHT_SHA256, size=16_075, mode="0444")
    receipt_row = require_file(PREFLIGHT_RECEIPT, digest=PREFLIGHT_RECEIPT_SHA256, size=8_683, mode="0444")
    preflight = json.loads(PREFLIGHT.read_text())
    receipt = json.loads(PREFLIGHT_RECEIPT.read_text())
    need(receipt.get("verdict") == "READY_TO_IMPLEMENT" and receipt.get("safe_to_implement") is True and not receipt.get("blockers"), "M3 implementation preflight is not ready")
    need(
        receipt.get("task_id")
        == "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_IMPLEMENTATION_V2_2026-08-27",
        "M3 implementation preflight task drift",
    )
    ...
    need(len(baseline) == 17, f"M3 pinned file closure drift: {len(baseline)}")
...
def verify_python_graph() -> dict[str, Any]:
    ...
    for auditor in (BOOTSTRAP_AUDITOR, BUILD_AUDITOR, TERMINAL_AUDITOR):
        ...
        need("IMPLEMENTATION_SELF_CHECK_V1_2026-08-27.json" in auditor_text, f"M3 receipt pin absent: {auditor.name}")
        need("lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v1.py" in auditor_text, f"M3 controller pin absent: {auditor.name}")
    require_file(REMOTE_CONTROLLER, mode="0555")
    ...
    need(
        execute_source.count("finalize_execution_journal(journal, terminal)") == 3,
        "terminal execution-journal finalization coverage drift",
    )
    execute_parents = {
        child: parent
...
def verify_fragment() -> dict[str, Any]:
    fragment = FRAGMENT.read_bytes()
    predecessor = D1_FRAGMENT.read_bytes()
    need(
        len(predecessor) == 113_204
        and sha256_bytes(predecessor) == D1_FRAGMENT_SHA256
        and fragment.startswith(predecessor),
        "M3 exact D1 fragment prefix drift",
    )
    text = fragment.decode()
    delta = fragment[len(predecessor) :]
    required = (
        "struct M3PackedState",
        "struct M3PackedEdge",
        "struct M3TypedView",
        "fn m3_materialize",
        "fn m3_enumerate_lane_prepared_t",
        "fn m3_search_t",
        "fn m3_full_parity",
        "v10_m3_dafsa_typed_view_parity",
        "v10_m3_dafsa_typed_view_physical",
        "B0-BYTE-VIEW",
        "T0-TYPED-VIEW",
        "T1-TYPED-VIEW",
        "B1-BYTE-VIEW",
    ...
    for token in required:
        need(token in text, f"M3 fragment token absent: {token}")
    need(
        text.count("fn m3_enumerate_lane_prepared_t") == 1
        and text.count("fn v10_m3_dafsa_typed_view_parity") == 1
        and text.count("fn v10_m3_dafsa_typed_view_physical") == 1,
        "M3 owner or test definition cardinality drift",
    )
    delta_text = delta.decode()
    for token in ("unsafe", "get_unchecked", "I0-INTERLEAVED", "I1-INTERLEAVED"):
        need(token not in delta_text, f"forbidden M3 Rust token: {token}")
    need(
        "self.states[state_id as usize]" in delta_text
        and "&self.edges[start..start + state.edge_count as usize]" in delta_text,
        "M3 safe direct typed indexing absent",
    )
    compile_proof = json.loads(LOCAL_COMPILE_PROOF.read_text())
    need(
        compile_proof.get("verdict") == "M3_LOCAL_COMPILE_PROOF_PASS"
        ...
        "M3 local compile proof drift",
    )
    assembled = assemble_source()
    parsed = run(["rustfmt", "--edition", "2024", "--emit", "stdout"], input_bytes=assembled, timeout=120)
    need(parsed.returncode == 0, "assembled M3 source failed rustfmt parse")
    return {
        "fragment": file_row(FRAGMENT),
        "predecessor_fragment": file_row(D1_FRAGMENT),
...
def verify_terminal_state_model() -> dict[str, Any]:
    ...
    cases = {
        ...
        "parity-provenance-block": (
            ...
            {"BUILD", "PARITY"},
            "M3_BUILD_AUDITED_PARITY_ADMITTED",
            "BLOCKED_PROVENANCE",
            2,
        ),
        "full-route-pass": (
...
def fault_response(action_id: str) -> dict[str, Any]:
    if action_id == "remote-bootstrap":
        return {
            "verdict": "M3_BOOTSTRAP_CREATED_UNAUDITED",
            "markers_created": 0,
            "markers_consumed": 0,
            "cargo_invocations": 0,
            "perf_stat_invocations": 0,
    ...
    if action_id == "create-markers":
        return {
            "verdict": "M3_ALL_MARKERS_AVAILABLE",
            "markers": {
    ...
    if action_id == "build":
        return {"verdict": "M3_BUILD_CREATED_UNAUDITED", "build": {"cargo_invocations": 1}}
    if action_id == "parity":
    ...
    verdicts = {
        ...
        "bootstrap-audit": "M3_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED",
        "bootstrap-audit-upload": "BOOTSTRAP_AUDIT_UPLOADED",
        "build-audit": "M3_BUILD_AUDITED_PARITY_ADMITTED",
        "build-audit-upload": "BUILD_AUDIT_UPLOADED",
        "terminal-audit": "W1_DAFSA_TYPED_VIEW_REJECTED",
    }
...
def verify_failure_publication_model() -> dict[str, Any]:
    ...
    need(len(action_ids) == 15 and len(set(action_ids)) == 15, "external action registry cardinality drift")
    for failed_index, failed_action in enumerate(action_ids):
        with tempfile.TemporaryDirectory(prefix="lay-m3-v3-fault-") as raw:
            journal = initialize_execution_journal(
                pathlib.Path(raw) / "journal",
                {
                    "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-fault-model.v1",
                    "task_id": TASK_ID,
                    "transaction_id": TRANSACTION_ID,
                    "retry_permitted": False,
                },
    ...
    with tempfile.TemporaryDirectory(prefix="lay-m3-v3-finalization-") as raw:
        journal = initialize_execution_journal(
            pathlib.Path(raw) / "journal",
            {
                "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-finalization-model.v1",
                "task_id": TASK_ID,
                "transaction_id": TRANSACTION_ID,
                "retry_permitted": False,
            },
        ...
        for action_id in action_ids:
            response = run_journaled(journal, action_id, lambda current=action_id: fault_response(current))
            update_execution_knowledge(known, action_id, response)
            terminal = response
        terminal["receipt_sha256"] = "a" * 64
        finalize_execution_journal(journal, terminal)
        final = json.loads((journal / "FINAL.json").read_text())
        need(
            final.get("terminal_verdict") == "W1_DAFSA_TYPED_VIEW_REJECTED"
            and final.get("terminal_receipt_sha256") == "a" * 64
            and mode_string(journal) == "0555"
            and mode_string(journal / "FINAL.json") == "0444"
            and mode_string(journal / "SHA256SUMS") == "0444",
            "clean terminal journal finalization drift",
        )
    return {
        ...
        "clean_terminal": {
            "finalized": True,
            "journal_mode": "0555",
            "terminal_verdict": "W1_DAFSA_TYPED_VIEW_REJECTED",
        },
        "hard_crash": {
...
def verify_atomic_self_check_publication() -> dict[str, Any]:
    payload = canonical(
        {
            "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-publication-fault-model.v1",
            "verdict": "M3_CONTROLLER_VERIFIED_UNRUN",
        }
    )
    with tempfile.TemporaryDirectory(prefix="lay-m3-v3-publication-") as raw:
        ...
        try:
            ...
            raise M3ControllerError("publication fault injection did not interrupt")
...
def self_check() -> dict[str, Any]:
    ...
    need(
        ...
        "M3 execution evidence exists during offline implementation pass",
    )
    source_rows = {path.name: file_row(path) for path in (*python_sources(), FRAGMENT)}
    return {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-controller-self-check.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_CONTROLLER_STATIC_SELF_CHECK_PASS",
        "admission": admission,
        "source": source,
        "sources": source_rows,
        "python_graph": graph,
        ...
        "candidate_registry": ["BYTE_VIEW_BASELINE", "PREVALIDATED_TYPED_VIEW"],
...
def seal_self_check() -> dict[str, Any]:
    ...
    need(runtime_after == runtime_before, "local runtime changed during M3 implementation seal")
    receipt = {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-implementation-self-check.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_CONTROLLER_VERIFIED_UNRUN",
        "controller_sha256": sha256_file(CONTROLLER),
        ...
        "m2r1_terminal_receipt_sha256": "98660957aeb31eb17b332868212cbb3ca295f35b979b511ed093fe807e0ea469",
        "m3_paper_sha256": "55ab0bb2bcda695bde3653fabad74243edb6230f4e4072264a5a33a807ba04be",
        ...
        "next_action_admitted": "independent M3 live execution admission only; remote experiment remains forbidden",
    }
...
def verify_execution_admission() -> dict[str, Any]:
    ...
    need(implementation.get("verdict") == "M3_CONTROLLER_VERIFIED_UNRUN", "implementation receipt verdict drift")
    need(implementation.get("controller_sha256") == sha256_file(CONTROLLER), "controller changed after seal")
    need(EXECUTION_ADMISSION.is_file(), "M3 execution admission absent; remote actions are forbidden")
    admission = json.loads(EXECUTION_ADMISSION.read_text())
    need(admission.get("verdict") == "M3_EXECUTION_ADMITTED" and admission.get("safe_to_execute") is True, "M3 execution not admitted")
...
def prepare_remote_bootstrap(admission: Mapping[str, Any]) -> pathlib.Path:
    local = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-bootstrap-"))
    files = {
        "v10.rs": V10_SOURCE,
    ...
    payload = {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-bootstrap-payload.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def execute_once() -> dict[str, Any]:
    ...
    need(
        ...
        "M3 local execution evidence already exists",
    )
    ...
    try:
        ...
        run_checked_action(
            ...
            "remote-bootstrap",
            remote_bootstrap,
            ("M3_BOOTSTRAP_CREATED_UNAUDITED",),
        )
        bootstrap_audit = run_checked_action(
            ...
            ("M3_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED",),
        )
        remote_bootstrap_audit = REMOTE_CACHE / "M3_BOOTSTRAP_AUDIT_RECEIPT.json"
        run_checked_action(
            journal,
            knowledge,
            "bootstrap-audit-upload",
        ...
        build = run_checked_action(
            ...
            ("M3_BUILD_CREATED_UNAUDITED", "BLOCKED_BUILD"),
        )
        ...
        build_audit = run_checked_action(
            ...
            ("M3_BUILD_AUDITED_PARITY_ADMITTED", "BLOCKED_BUILD", "BLOCKED_PROVENANCE"),
        )
        if build_audit.get("verdict") in {"BLOCKED_BUILD", "BLOCKED_PROVENANCE"}:
            ...
            finalize_execution_journal(journal, terminal)
            return terminal
        remote_build_audit = REMOTE_CACHE / "M3_BUILD_AUDIT_RECEIPT.json"
        run_checked_action(
            journal,
            knowledge,
            "build-audit-upload",
...
def main() -> int:
    ...
    try:
        ...
        print(f"M3 CONTROLLER ERROR: {type(error).__name__}: {error}", file=sys.stderr)
        return 1
