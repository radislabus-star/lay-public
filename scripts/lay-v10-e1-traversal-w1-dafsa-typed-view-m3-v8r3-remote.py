#!/usr/bin/env python3
"""Fail-closed remote producer for the M3 V8R3 direct-exec proof."""
...
EXECUTABLE = BOOTSTRAP / "m3-v8r3-test-elf"
E2E = PARENT / "e2e-v1"
...
class V8R3RemoteError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise V8R3RemoteError(message)
...
def append_state(sequence: int, state: str, **extra: Any) -> None:
    write_json(
        ...
        {
            "schema": "lay.m3-v8r3-remote-state.v1",
            "task_id": TASK_ID,
...
def verify_execution_admission(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(value.get("verdict") == "M3_V8R3_EXECUTION_ADMITTED", "execution admission verdict drift")
...
def uid_capability_probe(stage_parent: pathlib.Path) -> dict[str, Any]:
    ...
    code = (
        ...
        "f=open(a,'xb'); f.write(b'm3-v8r3-uid-proof\\n'); f.flush(); os.fsync(f.fileno()); f.close(); "
        "os.rename(a,b); assert b.read_bytes()==b'm3-v8r3-uid-proof\\n'; b.unlink()"
    )
    result = run(
...
def bootstrap_once(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(not PARENT.exists() and not STATE.exists(), "V8R3 remote namespace already exists")
    payload = load_json(path / "PAYLOAD.json")
    need(payload.get("schema") == "lay.m3-v8r3-bootstrap-payload.v1", "bootstrap payload schema drift")
    ...
    try:
        ...
        executable = copied / "m3-v8r3-test-elf"
        ...
        need(elf_build_id(executable) == EXPECTED_BUILD_ID, "V8R3 copied Build ID drift")
        ...
        write_new(state_stage / "route.lock", b"m3-v8r3-route-lock\n", 0o600)
        receipt = {
            "schema": "lay.m3-v8r3-bootstrap.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "M3_V8R3_BOOTSTRAP_CREATED_UNAUDITED",
...
def verify_bootstrap_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "M3_V8R3_BOOTSTRAP_AUDIT_PASS_MARKER_ADMITTED", "bootstrap audit did not admit marker")
...
def create_marker(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    markers = STATE / "markers"
    need(not markers.exists(), "V8R3 marker tree already exists")
    ...
    return {
        "schema": "lay.m3-v8r3-marker.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R3_E2E_MARKER_AVAILABLE",
        "markers": marker_inventory(),
        "marker_row": row,
        "markers_created": 1,
        "markers_consumed": 0,
...
def verify_quiet_admission(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "M3_V8R3_QUIET_HOST_E2E_ADMITTED", "quiet admission verdict drift")
...
def e2e_once(quiet_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(not E2E.exists() and not (PARENT / "e2e-failure-v1").exists(), "V8R3 E2E evidence already exists")
    require_file(EXECUTABLE, size=EXPECTED_ELF_SIZE, digest=EXPECTED_ELF_SHA256, mode="0555")
    need(elf_build_id(EXECUTABLE) == EXPECTED_BUILD_ID, "V8R3 executable Build ID drift")
    ...
    wrapper = {
        "schema": "lay.m3-v8r3-e2e-wrapper.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R3_E2E_CREATED_UNAUDITED" if producer_complete else "BLOCKED_PROVENANCE",
...
def status() -> dict[str, Any]:
    return {
        "schema": "lay.m3-v8r3-status.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R3_STATUS",
...
def self_check() -> dict[str, Any]:
    ...
    need(len({V8R1_TASK_ID, V8R2_TASK_ID, TASK_ID}) == 3, "namespace registry drift")
    return {
        "schema": "lay.m3-v8r3-remote-self-check.v1",
        "verdict": "M3_V8R3_REMOTE_CONTROLLER_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.m3-v8r3-remote-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
