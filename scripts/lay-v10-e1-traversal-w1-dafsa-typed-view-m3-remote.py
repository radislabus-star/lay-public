#!/usr/bin/env python3
"""Remote one-shot producer for the M3 dafsa-typed-view repair experiment."""
...
EXPECTED = {
    ...
    "fragment": "5a2e164c47c88677b74baf44d500c939749c98deff3092a1621086cf6e800875",
    "assembled": "56a0c3243ec8ea9fc3a4f8bf49c4b56f10e5ba38b1158026271dd34218f13aba",
...
class M3RemoteError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise M3RemoteError(message)
...
def run(
    ...
    *,
    ...
    if check and result.returncode != 0:
        ...
        raise M3RemoteError(f"command failed ({result.returncode}): {command!r}\n{detail.decode(errors='replace')}")
    return result
...
def assemble_source(v10: bytes, fragment: bytes) -> bytes:
    ...
    need(sha256_bytes(fragment) == EXPECTED["fragment"] and len(fragment) == 149_110, "M3 fragment drift")
    need(v10.endswith(b"}\n"), "V10 terminal brace drift")
    assembled = v10[:-2] + fragment + b"}\n"
    need(len(assembled) == 240_628 and sha256_bytes(assembled) == EXPECTED["assembled"], "assembled M3 source drift")
...
def verify_bootstrap(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(payload.get("execution_admission_verdict") == "M3_EXECUTION_ADMITTED", "execution admission missing")
    files = {}
...
def marker_payload(route: str, controller_sha256: str) -> bytes:
    ...
    return canonical_json_bytes({
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-marker.v1",
        "task_id": TASK_ID,
...
def append_state(sequence: int, state: str, **extra: Any) -> None:
    write_new_json(
        ...
        {
            "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-state.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "sequence": sequence,
            "state": state,
...
def uid_capability_probe(parent: pathlib.Path) -> dict[str, Any]:
    ...
    code = (
        ...
        "a=p/'a'; b=p/'b'; f=open(a,'xb'); f.write(b'm3-uid-proof\\n'); f.flush(); os.fsync(f.fileno()); f.close(); "
        "os.rename(a,b); assert b.read_bytes()==b'm3-uid-proof\\n'; b.unlink()"
    )
...
def bootstrap_once(bootstrap: pathlib.Path) -> dict[str, Any]:
    ...
    need(not PARENT.exists() and not STATE.exists(), "M3 namespace already exists")
    ...
    try:
        ...
        write_new_bytes(state_stage / "route.lock", b"m3-route-lock\n", 0o600)
        receipt = {
            "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-bootstrap.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
        ...
        write_new_json(stage / "M3_BOOTSTRAP_RECEIPT.json", receipt)
...
def create_markers(admission: pathlib.Path) -> dict[str, Any]:
    ...
    need(value.get("verdict") == "M3_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED", "bootstrap audit did not admit markers")
    ...
    markers = STATE / "markers"
    need(not markers.exists(), "M3 marker tree already exists")
    ...
    return {"verdict": "M3_ALL_MARKERS_AVAILABLE", "markers": marker_inventory(), "markers_created": len(MARKER_ROUTES), "markers_consumed": 0}
...
def build_once(controller_sha: str) -> dict[str, Any]:
    need(not BUILD.exists() and not (PARENT / "build-failure-v1").exists(), "M3 build artifact already exists")
    paths = input_paths()
    ...
    try:
        ...
        with (stage / "cargo.log").open("wb") as log:
            ...
            log.flush()
            os.fsync(log.fileno())
        need(result.returncode == 0, f"M3 Cargo exited {result.returncode}")
        elf = find_test_elf(workspace / "target")
        candidate = stage / "diagnostic-test-elf"
        shutil.copyfile(elf, candidate)
        candidate.chmod(0o555)
        provenance = {
            "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-build.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "M3_BUILD_CREATED_UNAUDITED",
        ...
        if marker is not None and stage.exists():
            failure = {
                "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-build-failure.v1",
                "task_id": TASK_ID,
                "transaction_id": TRANSACTION_ID,
...
def wait_file(process: subprocess.Popen[bytes], path: pathlib.Path, timeout: float) -> None:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.002)
    raise M3RemoteError(f"timeout waiting for {path}")
    ...
    while time.monotonic() < deadline:
        try:
            ...
            if error.errno not in (6, 11):
                raise
            time.sleep(0.005)
    raise M3RemoteError(f"FIFO open timeout: {path}")
...
def read_fifo_line(descriptor: int, deadline: float) -> str:
    ...
    while time.monotonic() < deadline:
        ...
        time.sleep(0.002)
    raise M3RemoteError("perf acknowledgement timeout")


def require_build_audit(path: pathlib.Path) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "M3_BUILD_AUDITED_PARITY_ADMITTED", "build audit did not admit parity")
...
def parity_once(controller_sha: str, build_audit: pathlib.Path) -> dict[str, Any]:
    ...
    need(not root.exists(), "M3 parity already exists")
    ...
    wrapper = {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-parity-wrapper.v2",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": verdict,
...
def run_physical(route: str, controller_sha: str) -> dict[str, Any]:
    need(route in ROUTE_ORDER, f"unknown M3 route: {route}")
    ...
    need(states and load_json(states[-1]).get("state") == previous, f"M3 route predecessor drift: {route}")
    root = PARENT / f"route-{index + 1:02d}-{marker_name(route)}"
    need(not root.exists(), f"M3 route already exists: {route}")
    ...
    environment["LAY_V10_M3_CONTROL_DIR"] = str(control)
...
def self_check() -> dict[str, Any]:
    need(
        set(ROUTE_ORDER)
        == {"B0-BYTE-VIEW", "T0-TYPED-VIEW", "T1-TYPED-VIEW", "B1-BYTE-VIEW"},
        "route registry drift",
    )
    need(len(MARKER_ROUTES) == 6 and MARKER_ROUTES[:2] == ("BUILD", "PARITY"), "marker registry drift")
    ...
    return {"verdict": "M3_REMOTE_CONTROLLER_VERIFIED_UNRUN", "task_id": TASK_ID, "transaction_id": TRANSACTION_ID, "routes": list(ROUTE_ORDER), "markers": list(MARKER_ROUTES), "cargo_invocations": 0, "perf_stat_invocations": 0, "subject_executions": 0, "runtime_authority_changed": False}
...
def main() -> int:
    ...
    try:
        if arguments.action == "self-check":
            ...
            need(PARENT.is_dir() and STATE.is_dir(), "M3 bootstrap absent")
        ...
        print(f"M3 REMOTE ERROR: {type(error).__name__}: {error}", file=sys.stderr)
        return 1
