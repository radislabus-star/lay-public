#!/usr/bin/env python3
"""Independent bootstrap audit for the M3 dafsa-typed-view repair experiment."""
...
RESULT = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_"
    "BOOTSTRAP_AUDIT_V1_2026-08-27"
)
EXPECTED_FRAGMENT = "5a2e164c47c88677b74baf44d500c939749c98deff3092a1621086cf6e800875"
...
def fixed_inputs() -> dict[str, Any]:
    need(IMPLEMENTATION_RECEIPT.is_file(), "sealed M3 implementation receipt absent")
    ...
    need(sha256_file(FRAGMENT) == EXPECTED_FRAGMENT, "M3 fragment drift")
    for name, path in {
...
def self_check() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-bootstrap-audit-self-check.v3",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_BOOTSTRAP_AUDITOR_VERIFIED_UNRUN",
...
receipt=parent/'M3_BOOTSTRAP_RECEIPT.json'
...
def validate_live(value: dict[str, Any], check: dict[str, Any]) -> dict[str, Any]:
    ...
    need(value.get("parent_entries") == ["M3_BOOTSTRAP_RECEIPT.json", "SHA256SUMS", "bootstrap-v1", "uid-probe"], "bootstrap membership drift")
    need(value.get("state_entries") == ["STATE-00-BOOTSTRAP_CREATED_UNAUDITED.json", "route.lock"], "pre-marker state membership drift")
    need(value.get("markers_exists") is False, "M3 markers exist before bootstrap audit")
    receipt = value.get("bootstrap_receipt", {})
    need(receipt.get("verdict") == "M3_BOOTSTRAP_CREATED_UNAUDITED", "bootstrap producer verdict drift")
...
def audit() -> dict[str, Any]:
    ...
    need(admission.get("verdict") == "M3_EXECUTION_ADMITTED", "execution admission verdict drift")
    ...
    receipt = {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-bootstrap-audit.v3",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED",
        ...
        "next_action_admitted": "one atomic six-marker M3 creation action only",
    }
    stage = pathlib.Path(f"{RESULT}.stage-{os.getpid()}-{time.time_ns()}")
    stage.mkdir(parents=True, mode=0o700)
    try:
        write_json(stage / "M3_BOOTSTRAP_AUDIT_RECEIPT.json", receipt)
        ...
        if stage.exists():
            shutil.rmtree(stage)
        raise
    return {**receipt, "receipt_sha256": sha256_file(RESULT / "M3_BOOTSTRAP_AUDIT_RECEIPT.json")}
...
def main() -> int:
    ...
    try:
        ...
        print(f"M3 BOOTSTRAP AUDIT ERROR: {type(error).__name__}: {error}", file=sys.stderr)
        return 1
