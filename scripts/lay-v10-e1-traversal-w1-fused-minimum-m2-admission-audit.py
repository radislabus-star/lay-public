#!/usr/bin/env python3
"""Independent auditor and sole publisher for M2 execution admission V2."""
...
PRODUCER = ROOT / "scripts/lay-v10-e1-traversal-w1-fused-minimum-m2-live-preflight.py"
EXPECTED_PRODUCER_SHA256 = "ed2b04bc2e2076b5882439abc2ef4ed700ec17fe9f682f6155b33bdc1fda784b"
...
EXECUTION_ADMISSION = ROOT / (
    ...
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_FUSED_MINIMUM_"
    "M2_EXECUTION_ADMISSION_V2_2026-08-26.json"
)
M2_JOURNAL = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_FUSED_MINIMUM_"
...
EXPECTED_SOURCES = {
    "controller_sha256": (M2_CONTROLLER, "4f2f6484d7ada483688b59a2403354548d4aaffdc594074c4447326c7b8c1f7f"),
    "remote_controller_sha256": (M2_REMOTE_CONTROLLER, "14c7c60814927a8f3bcfc607a391d78552fdf8cdd51253eccd429d1f40c50dd6"),
    "bootstrap_auditor_sha256": (M2_BOOTSTRAP_AUDITOR, "30da6d116df0fa5d5372dc8718c86bb3e46c4cb59edcc918429cceb523959869"),
    "build_auditor_sha256": (M2_BUILD_AUDITOR, "091e97c8aa0e1d8a7455fefba0fb038ea04566996b149d4a10feaad0e1d5030e"),
    "terminal_auditor_sha256": (M2_TERMINAL_AUDITOR, "1656e3a2fa8b772a5ef8c01daab7daf03b751c73fc2ac4bbda02a1ed6520b92b"),
    "fragment_sha256": (M2_FRAGMENT, "b0a775420edf9e9d6e7f0b59f9ad840e4822cd0fdd0adc2429ea22a3e9e3a175"),
}
...
task='slice8b-v10-e1-traversal-w1-fused-minimum-m2-v1-20260826'; tx='c760eea52b6416b3529f9d684c315147b5a1140522114642c417d7db4065102c'
parent=pathlib.Path('/home/e/.local/share/lay/provenance')/task; state=pathlib.Path('/home/e/.local/state/lay')/task
cache=pathlib.Path('/home/e/.cache')/('lay-m2-'+tx); probe=pathlib.Path('/home/e/.cache')/('lay-m2-execution-admission-v2-'+tx)
...
def verify_implementation() -> dict[str, Any]:
    ...
    need(value.get("verdict") == "M2_ADMISSION_CONTROLLER_VERIFIED_UNRUN", "implementation verdict drift")
    ...
    for key, (path, digest) in EXPECTED_SOURCES.items():
        need(path.is_file() and sha256_file(path) == digest, f"sealed M2 source drift: {key}")
    return value


def verify_live_result() -> tuple[dict[str, Any], dict[str, str]]:
    rows = verify_manifest(LIVE_RESULT)
    expected_files = {"SNAPSHOT-1.json", "SNAPSHOT-2.json", "UID_PROOF.json", "RUNTIME_BEFORE.json", "RUNTIME_AFTER.json", "M2_LIVE_PREFLIGHT_RECEIPT.json"}
    need(set(rows) == expected_files, "live preflight evidence inventory drift")
    receipt = json.loads((LIVE_RESULT / "M2_LIVE_PREFLIGHT_RECEIPT.json").read_text())
    need(receipt.get("verdict") == "M2_LIVE_PREFLIGHT_PASS_UNADMITTED" and receipt.get("safe_to_execute") is False, "live preflight verdict drift")
    ...
    need(proof.get("payload_sha256") == "6245ab0ebee6664a2af02e92d1a0259292399df4a19ead2a6a2fe6fd1f37c39a", "UID proof payload drift")
...
def self_check() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="m2-admission-auditor-check-") as raw:
    ...
    return {
        "schema": "lay.m2-execution-admission-auditor-self-check.v1",
        "task_id": TASK_ID,
...
def audit() -> dict[str, Any]:
    ...
    need(not M2_JOURNAL.exists(), "M2 execution journal exists before admission")
    ...
    try:
        ...
        receipt = {
            "schema": "lay.m2-execution-admission-independent-audit.v2",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "M2_EXECUTION_ADMISSION_AUDIT_PASS_READY_TO_PUBLISH",
            "producer_sha256": EXPECTED_PRODUCER_SHA256,
            "auditor_sha256": sha256_file(AUDITOR),
            ...
            "live_preflight_receipt_sha256": sha256_file(LIVE_RESULT / "M2_LIVE_PREFLIGHT_RECEIPT.json"),
            ...
            "next_action_admitted": "atomic publication of exact M2 execution admission only",
        }
        write_new(stage / "M2_EXECUTION_ADMISSION_AUDIT_RECEIPT.json", canonical(receipt))
    ...
    audit_receipt = AUDIT_RESULT / "M2_EXECUTION_ADMISSION_AUDIT_RECEIPT.json"
    admission = {
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-execution-admission.v2",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M2_EXECUTION_ADMITTED",
        "safe_to_execute": True,
        **{key: digest for key, (_, digest) in EXPECTED_SOURCES.items()},
        "implementation_receipt_sha256": sha256_file(M2_IMPLEMENTATION_RECEIPT),
        ...
        "producer_sha256": EXPECTED_PRODUCER_SHA256,
        "auditor_sha256": sha256_file(AUDITOR),
        "code_route_v5_contract_sha256": "0fdfa5ece5b1e53a4d4acd661c4403978f0524b086c1fd5d52ef50c1521501d3",
        "code_route_v5_receipt_sha256": "54b66f7a30d6f7b6edc67c2eb07748259e78fa45f4d26c9da241325e2a455acb",
        "live_snapshots": 3,
        "conflicting_processes": 0,
        "open_perf_event_descriptors": 0,
        "m2_remote_namespace_present": False,
        "execution_journal_present": False,
        "markers_created": 0,
        "markers_consumed": 0,
        "cargo_compilations": 0,
        ...
        "subject_executions": 0,
        "runtime_authority_changed": False,
        "next_action_admitted": "one invocation of sealed M2 V3 controller run",
    }
...
def main() -> int:
    ...
    try:
        ...
        print(f"M2 ADMISSION AUDIT ERROR: {type(error).__name__}: {error}", file=sys.stderr)
        return 1
