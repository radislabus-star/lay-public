...
EXPECTED_TRANSPORT_PROGRAM_SHA256 = "e13b5acd904707c67df1881892a46349e38fe932c57fcefd08f353ff1819bc53"
EXPECTED_TRANSPORT_NONCE = "v10r4-stdin-transport-eeac980119d265ff"
EXPECTED_TRANSPORT_ARGUMENTS = ("v10r4-argv-alpha", "v10r4-argv-beta")
EXPECTED_TRANSPORT_SOURCE_ID = "lay-v10r4-stdin-observer-v1"
EXPECTED_INPUTS = {
...
class V10R4RemoteError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise V10R4RemoteError(message)
...
def verify_execution_admission(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(value.get("verdict") == "V10R4_EXECUTION_ADMITTED", "execution admission verdict drift")
    ...
    need(value.get("stdin_transport_admitted") is True, "stdin transport predecessor absent")
    need(value.get("stdin_transport_program_sha256") == EXPECTED_TRANSPORT_PROGRAM_SHA256, "stdin transport program identity drift")
    observer = value.get("stdin_transport_observer")
    need(isinstance(observer, dict), "stdin transport observer absent")
    need(observer.get("schema") == "lay.v10r4-stdin-transport.v1", "stdin transport observer schema drift")
    need(observer.get("nonce") == EXPECTED_TRANSPORT_NONCE, "stdin transport observer nonce drift")
    need(observer.get("argv") == list(EXPECTED_TRANSPORT_ARGUMENTS), "stdin transport observer argv drift")
    need(observer.get("source_id") == EXPECTED_TRANSPORT_SOURCE_ID, "stdin transport observer source identity drift")
    need(observer.get("uid") == 0 and observer.get("hostname") == "e-MEGA-MINI-M1-13th", "stdin transport observer authority drift")
    return value
...
def uid_capability_probe(stage_parent: pathlib.Path) -> dict[str, Any]:
    ...
    code = (
        ...
        "a=p/'a'; b=p/'b'; f=open(a,'xb'); f.write(b'v10r4-uid-proof\\n'); "
        "f.flush(); os.fsync(f.fileno()); f.close(); os.rename(a,b); "
        "assert b.read_bytes()==b'v10r4-uid-proof\\n'; b.unlink()"
    )
    result = run([
...
def append_state(sequence: int, state: str, **extra: Any) -> None:
    write_json(STATE / f"STATE-{sequence:02d}-{state}.json", {
        "schema": "lay.v10r4-remote-state.v1",
        "task_id": TASK_ID,
...
def bootstrap_reuse(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(not PARENT.exists() and not STATE.exists(), "V10R4 namespace already exists")
    payload = load_json(path / "PAYLOAD.json")
    need(payload.get("schema") == "lay.v10r4-bootstrap-reuse-payload.v1", "bootstrap payload schema drift")
    ...
    try:
        ...
        write_new(state_stage / "route.lock", b"v10r4-trace-reuse-route-lock\n", 0o600)
        receipt = {
            "schema": "lay.v10r4-bootstrap-reuse.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "V10R4_BOOTSTRAP_REUSE_CREATED_UNAUDITED",
...
def verify_bootstrap_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10R4_BOOTSTRAP_REUSE_AUDIT_PASS_QUIET_ADMITTED", "bootstrap audit did not admit quiet gate")
...
def verify_quiet_admission(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10R4_QUIET_READY_TRACE_ADMITTED", "quiet audit did not admit TRACE")
...
def create_marker(quiet_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(not markers.exists(), "V10R4 marker tree already exists")
    ...
    return {
        "schema": "lay.v10r4-marker.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R4_TRACE_MARKER_AVAILABLE",
        "marker": row,
        "markers": marker_inventory(),
        "markers_created": 1,
        "markers_consumed": 0,
...
def trace_once(quiet_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(not TRACE.exists() and not (PARENT / "trace-failure-v1").exists(), "V10R4 TRACE evidence already exists")
    ...
    wrapper = {
        ...
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R4_TRACE_REUSE_CREATED_UNAUDITED" if producer_complete else "BLOCKED_PROVENANCE",
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10r4-status.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R4_STATUS",
...
def self_check() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10r4-remote-self-check.v1",
        "verdict": "V10R4_REMOTE_CONTROLLER_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        ...
        "stdin_transport_program_sha256": EXPECTED_TRANSPORT_PROGRAM_SHA256,
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.v10r4-remote-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
