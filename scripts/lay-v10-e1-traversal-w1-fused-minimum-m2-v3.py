#!/usr/bin/env python3
"""Fault-closed V3 local controller for the W1 fused-minimum M2 experiment."""
...
V2_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-fused-minimum-m2.py"
REMOTE_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-fused-minimum-m2-remote.py"
BOOTSTRAP_AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-fused-minimum-m2-bootstrap-audit-v3.py"
BUILD_AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-fused-minimum-m2-build-audit-v3.py"
TERMINAL_AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-fused-minimum-m2-terminal-audit-v3.py"
...
PREFLIGHT = ROOT / (
    ...
    "IMPLEMENTATION_V5_2026-08-26.json"
)
PREFLIGHT_RECEIPT = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_FUSED_MINIMUM_M2_"
    "IMPLEMENTATION_V5_PREFLIGHT_2026-08-26.json"
)
...
EXECUTION_ADMISSION = ROOT / (
    "docs/structural_gates/preflights/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_FUSED_MINIMUM_M2_"
    "EXECUTION_ADMISSION_V2_2026-08-26.json"
)
...
TERMINAL_AUDIT_RECEIPT = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_FUSED_MINIMUM_M2_"
    "TERMINAL_AUDIT_V2_2026-08-26/M2_TERMINAL_AUDIT_RECEIPT.json"
)
EXECUTION_JOURNAL = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_FUSED_MINIMUM_M2_"
    "EXECUTION_JOURNAL_V1_2026-08-26"
)
CONTROLLER_FAILURE = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_FUSED_MINIMUM_M2_"
    "CONTROLLER_FAILURE_V1_2026-08-26"
)
PREFLIGHT_SHA256 = "60a676d2d59d296e5e8445b61311ff46edd9a62d3aa950da78a1ea0c9c3b2598"
PREFLIGHT_RECEIPT_SHA256 = "a33fadd7fdfe135e1f726f91b9a2cae85bc7b27343a3ab21ed55f91a9c2561c3"
...
def journal_metadata(admission: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-execution-journal.v1",
        "task_id": TASK_ID,
...
def append_intent(root: pathlib.Path, action_id: str) -> dict[str, Any]:
    ...
    intent = {
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-action-intent.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
...
def append_completion(root: pathlib.Path, intent: Mapping[str, Any], response: Mapping[str, Any]) -> dict[str, Any]:
    ...
    completion = {
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-action-completion.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
...
def controller_failure_payload(
    ...
    return {
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-controller-failure.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def publish_controller_failure(
    ...
    try:
        write_json(stage / "M2_CONTROLLER_FAILURE_RECEIPT.json", receipt)
        write_json(
    ...
    return {
        **receipt,
        "receipt_sha256": sha256_file(CONTROLLER_FAILURE / "M2_CONTROLLER_FAILURE_RECEIPT.json"),
    }
...
def finalize_execution_journal(root: pathlib.Path, terminal: Mapping[str, Any]) -> None:
    write_json(
        root / "FINAL.json",
        {
            "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-execution-journal-final.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def verify_preflight() -> dict[str, Any]:
    preflight_row = require_file(PREFLIGHT, digest=PREFLIGHT_SHA256, size=13_532, mode="0444")
    receipt_row = require_file(PREFLIGHT_RECEIPT, digest=PREFLIGHT_RECEIPT_SHA256, size=10_700, mode="0444")
    ...
    need(
        receipt.get("task_id")
        == "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_FUSED_MINIMUM_M2_IMPLEMENTATION_V5_2026-08-26",
    ...
    need(len(baseline) == 16, f"M2 pinned file closure drift: {len(baseline)}")
...
def verify_python_graph() -> dict[str, Any]:
    ...
    for auditor in (BOOTSTRAP_AUDITOR, BUILD_AUDITOR, TERMINAL_AUDITOR):
        ...
        need("IMPLEMENTATION_SELF_CHECK_V3_2026-08-26.json" in auditor_text, f"V3 receipt pin absent: {auditor.name}")
        need("lay-v10-e1-traversal-w1-fused-minimum-m2-v3.py" in auditor_text, f"V3 controller pin absent: {auditor.name}")
    require_file(
        V2_CONTROLLER,
        digest="0454bdd5dc4051987cc94a293496dd4a39836cb5b981d3fd219d29bcc217e5e3",
        size=30_867,
        mode="0555",
    )
    require_file(
        REMOTE_CONTROLLER,
        digest="14c7c60814927a8f3bcfc607a391d78552fdf8cdd51253eccd429d1f40c50dd6",
        size=40_111,
        mode="0555",
    )
    ...
    for failed_index, failed_action in enumerate(action_ids):
        with tempfile.TemporaryDirectory(prefix="lay-m2-v3-fault-") as raw:
            journal = initialize_execution_journal(
                ...
                {
                    "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-fault-model.v1",
                    "task_id": TASK_ID,
                    "transaction_id": TRANSACTION_ID,
                    "retry_permitted": False,
                },
...
def verify_atomic_self_check_publication() -> dict[str, Any]:
    payload = canonical(
        {
            "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-publication-fault-model.v1",
...
def self_check() -> dict[str, Any]:
    ...
    need(
        ...
        "V3 execution evidence exists during offline implementation pass",
    )
    source_rows = {path.name: file_row(path) for path in (*python_sources(), FRAGMENT)}
    return {
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-controller-self-check.v3",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def seal_self_check() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2-implementation-self-check.v3",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M2_CONTROLLER_V3_VERIFIED_UNRUN",
        "controller_sha256": sha256_file(CONTROLLER),
        ...
        "v2_controller_sha256": sha256_file(V2_CONTROLLER),
        "v2_implementation_receipt_sha256": "ecc3d5a4f99b060c7bae647674380ca313c6f30c3301e64881a9643f9f696133",
        ...
        "next_action_admitted": "independent M2 V3 execution preflight only; remote experiment remains forbidden",
    }
...
def verify_execution_admission() -> dict[str, Any]:
    ...
    need(implementation.get("verdict") == "M2_CONTROLLER_V3_VERIFIED_UNRUN", "implementation receipt verdict drift")
