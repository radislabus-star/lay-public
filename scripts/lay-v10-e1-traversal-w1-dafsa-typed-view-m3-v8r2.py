#!/usr/bin/env python3
"""Local fail-closed orchestrator for the M3 V8R2 direct-exec proof."""
...
AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v8r2-audit.py"
V8_SOURCE = ROOT / "src/nanda_wave/l2_field/v13_typed_peak.rs"
PREFLIGHT_MANIFEST = ROOT / "docs/structural_gates/preflights/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_DIRECT_EXEC_V8R2_V2_2026-08-27.json"
PREFLIGHT_RECEIPT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_DIRECT_EXEC_V8R2_V2_PREFLIGHT_2026-08-27.json"
...
CONTROLLER_EVIDENCE = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_REMOTE_CONTROLLER_V8R2_V1_2026-08-27"
CONTROLLER_IMPLEMENTATION = CONTROLLER_EVIDENCE / "IMPLEMENTATION_RECEIPT.json"
EXECUTION_JOURNAL = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R2_EXECUTION_JOURNAL_V1_2026-08-27"
ADMISSION_RECEIPT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R2_EXECUTION_ADMISSION_V1_2026-08-27/EXECUTION_ADMISSION.json"
BOOTSTRAP_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R2_BOOTSTRAP_AUDIT_V1_2026-08-27/BOOTSTRAP_AUDIT.json"
QUIET_ADMISSION = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R2_QUIET_ADMISSION_V1_2026-08-27/QUIET_ADMISSION.json"
TERMINAL_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R2_TERMINAL_AUDIT_V1_2026-08-27/TERMINAL_AUDIT.json"
...
class V8R2ControllerError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise V8R2ControllerError(message)
...
def remote_call(action: str, *arguments: str, cache: bool = False, timeout: float = 3_600) -> dict[str, Any]:
    ...
    value = json.loads(lines[-1])
    need(isinstance(value, dict), f"remote {action} returned a non-object")
    return value


def verify_preflight() -> dict[str, Any]:
    need(sha256_file(PREFLIGHT_MANIFEST) == "4018fe6b5b58379949cd8ae907b11c8c358e42fb94c20967960666ffdf71864a", "preflight manifest SHA drift")
    need(sha256_file(PREFLIGHT_RECEIPT) == "4ce7e413299c797295db1682269c1589917b7c623a0ee2da9d0cd0b53f674775", "preflight receipt SHA drift")
    ...
    need(receipt.get("manifest_sha256") == "a119067339d4129e2de8c48fe67a6eb5c7976b95565ea82efe0aa59e74af1303", "NANDA manifest identity drift")
    return receipt
...
def parse_python(path: pathlib.Path) -> ast.Module:
    try:
        ...
        raise V8R2ControllerError(f"invalid controller source: {path}: {error}") from error
...
def static_graph(remote_check: Mapping[str, Any], auditor_check: Mapping[str, Any]) -> dict[str, Any]:
    ...
    need(isinstance(command, list) and str(REMOTE_PARENT / "bootstrap-v1/m3-v8r2-test-elf") in command, "direct executable absent")
...
def fault_model() -> dict[str, Any]:
    rows = []
    with tempfile.TemporaryDirectory(prefix="lay-m3-v8r2-faults-") as raw:
        ...
        for test_id, source in (
            ...
            try:
                ...
                raise V8R2ControllerError(f"partial source was accepted: {test_id}")
        ...
        try:
            ...
            raise V8R2ControllerError("exclusive receipt publication fault failed")
    rows.extend([
        ...
        {"id": "namespace-parity", "verdict": "PASS", "v8r1_distinct": TASK_ID != "slice8b-v10-e1-traversal-w1-dafsa-typed-view-m3-e2e-v8r1-20260827"},
...
def response_retention_fault() -> dict[str, Any]:
    synthetic = subprocess.CompletedProcess(
        args=["synthetic"],
        returncode=1,
        stdout=canonical({"verdict": "BLOCKED_PROVENANCE", "error": "synthetic"}),
        stderr=b"retained stderr\n",
    )
    parsed = parse_json_output(synthetic, "synthetic response")
    need(parsed.get("verdict") == "BLOCKED_PROVENANCE" and synthetic.stderr == b"retained stderr\n", "structured failure retention drift")
    return {"verdict": "PASS", "stdout_retained": True, "stderr_retained": True}
...
def self_check() -> dict[str, Any]:
    ...
    need(remote.get("verdict") == "M3_V8R2_REMOTE_CONTROLLER_STATIC_PASS", "remote self-check failed")
    need(auditor.get("verdict") == "M3_V8R2_INDEPENDENT_AUDITOR_STATIC_PASS", "auditor self-check failed")
    graph = static_graph(remote, auditor)
    faults = fault_model()
    retention = response_retention_fault()
    return {
        "schema": "lay.m3-v8r2-controller-self-check.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R2_CONTROLLER_STATIC_SELF_CHECK_PASS",
...
def seal_self_check() -> dict[str, Any]:
    ...
    check = self_check()
    receipt = {
        "schema": "lay.m3-v8r2-controller-implementation.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R2_CONTROLLERS_VERIFIED_UNRUN",
        ...
        "next_action_admitted": "independent V8R2 live execution admission only",
    }
...
def prepare_bootstrap(admission: Mapping[str, Any]) -> pathlib.Path:
    staging = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v8r2-bootstrap-"))
    try:
        files = {
        ...
        payload = {
            "schema": "lay.m3-v8r2-bootstrap-payload.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def initialize_journal() -> pathlib.Path:
    need(not EXECUTION_JOURNAL.exists(), "V8R2 execution journal already exists; retry is forbidden")
    EXECUTION_JOURNAL.mkdir(parents=True, mode=0o700)
    write_new(EXECUTION_JOURNAL / "JOURNAL.json", canonical({
        "schema": "lay.m3-v8r2-execution-journal.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def append_intent(root: pathlib.Path, sequence: int, action: str) -> None:
    ...
    write_new(root / f"{sequence:02d}-{action}.intent.json", canonical({
        "schema": "lay.m3-v8r2-external-intent.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def append_completion(root: pathlib.Path, sequence: int, action: str, response: Mapping[str, Any]) -> None:
    write_new(root / f"{sequence:02d}-{action}.complete.json", canonical({
        "schema": "lay.m3-v8r2-external-completion.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def create_remote_cache() -> dict[str, Any]:
    ...
    return {"verdict": "M3_V8R2_REMOTE_CACHE_CREATED", "stdout_sha256": sha256_bytes(output)}
...
def upload_bootstrap(staging: pathlib.Path) -> dict[str, Any]:
    ...
    return {"verdict": "M3_V8R2_BOOTSTRAP_UPLOADED", "payload_sha256": sha256_file(staging / "PAYLOAD.json")}
...
def verify_implementation() -> dict[str, Any]:
    ...
    need(value.get("verdict") == "M3_V8R2_CONTROLLERS_VERIFIED_UNRUN", "controller implementation verdict drift")
    for key, path in {
...
def execute() -> dict[str, Any]:
    ...
    try:
        admission = journaled(journal, sequence, "live-admission", lambda: auditor_call("live-admission"), ["M3_V8R2_EXECUTION_ADMITTED"])
        sequence += 1
        staging = prepare_bootstrap(admission)
        journaled(journal, sequence, "remote-cache-create", create_remote_cache, ["M3_V8R2_REMOTE_CACHE_CREATED"])
        sequence += 1
        journaled(journal, sequence, "bootstrap-upload", lambda: upload_bootstrap(staging), ["M3_V8R2_BOOTSTRAP_UPLOADED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            ...
            ["M3_V8R2_BOOTSTRAP_CREATED_UNAUDITED"],
        )
        sequence += 1
        journaled(journal, sequence, "bootstrap-audit", lambda: auditor_call("bootstrap"), ["M3_V8R2_BOOTSTRAP_AUDIT_PASS_MARKER_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "bootstrap-audit-upload",
            lambda: upload_audit(BOOTSTRAP_AUDIT, "BOOTSTRAP_AUDIT.json", "M3_V8R2_BOOTSTRAP_AUDIT_UPLOADED"),
            ["M3_V8R2_BOOTSTRAP_AUDIT_UPLOADED"],
        )
        sequence += 1
        ...
        journaled(
            ...
            ["M3_V8R2_E2E_MARKER_AVAILABLE"],
        )
        sequence += 1
        journaled(journal, sequence, "quiet-audit", lambda: auditor_call("quiet"), ["M3_V8R2_QUIET_HOST_E2E_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "quiet-audit-upload",
            lambda: upload_audit(QUIET_ADMISSION, "QUIET_ADMISSION.json", "M3_V8R2_QUIET_ADMISSION_UPLOADED"),
            ["M3_V8R2_QUIET_ADMISSION_UPLOADED"],
        )
        sequence += 1
        ...
        journaled(
            ...
            ["M3_V8R2_E2E_CREATED_UNAUDITED", "BLOCKED_PROVENANCE"],
        )
        sequence += 1
        terminal = journaled(
            journal,
        ...
        write_new(journal / "TERMINAL.json", canonical({
            "schema": "lay.m3-v8r2-controller-terminal.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
        ...
        failure = {
            "schema": "lay.m3-v8r2-controller-failure.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
        ...
        write_manifest(journal)
        seal_tree(journal)
        raise V8R2ControllerError(json.dumps(failure, sort_keys=True)) from error
    finally:
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.m3-v8r2-controller-status.v1",
        "verdict": "M3_V8R2_CONTROLLER_STATUS",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.m3-v8r2-controller-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "BLOCKED_PROVENANCE",
            "error": f"{type(error).__name__}: {error}",
