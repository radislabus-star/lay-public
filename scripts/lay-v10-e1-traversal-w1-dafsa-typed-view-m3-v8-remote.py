...
def verify_source_closure(root: pathlib.Path) -> dict[str, Any]:
    ...
    need(manifest.get("schema") == "lay.m3-v8-source-closure.v1", "source manifest schema drift")
...
def verify_execution_admission(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(value.get("verdict") == "M3_V8_EXECUTION_ADMITTED", "execution admission verdict drift")
...
def append_state(sequence: int, state: str, **extra: Any) -> None:
    write_json(
        ...
        {
            "schema": "lay.m3-v8-remote-state.v1",
            "task_id": TASK_ID,
...
def bootstrap_once(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(payload.get("schema") == "lay.m3-v8-bootstrap-payload.v1", "bootstrap payload schema drift")
    ...
    try:
        ...
        receipt = {
            "schema": "lay.m3-v8-bootstrap.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "M3_V8_BOOTSTRAP_CREATED_UNAUDITED",
...
def marker_payload(route: str, admission: Mapping[str, Any]) -> bytes:
    ...
    return canonical({
        "schema": "lay.m3-v8-one-shot-marker.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "route": route,
...
def verify_bootstrap_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "M3_V8_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED", "bootstrap audit did not admit markers")
...
def create_markers(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    return {
        "schema": "lay.m3-v8-markers.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8_ALL_MARKERS_AVAILABLE",
        "markers": marker_inventory(),
        "marker_rows": rows,
        "markers_created": 2,
        "markers_consumed": 0,
...
def build_once(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    workspace = pathlib.Path("/home/e/.cache") / f"lay-m3-v8-build-{TRANSACTION_ID}"
    ...
    try:
        ...
        candidate = stage / "m3-v8-test-elf"
        ...
        provenance = {
            "schema": "lay.m3-v8-build.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "M3_V8_BUILD_CREATED_UNAUDITED",
            "source": {
                "source_manifest_sha256": sha256_file(BOOTSTRAP / "source-closure/SOURCE_MANIFEST.json"),
        ...
        if marker is not None and stage.exists():
            failure = {
                "schema": "lay.m3-v8-build-failure.v1",
                "task_id": TASK_ID,
                "transaction_id": TRANSACTION_ID,
...
def verify_build_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "M3_V8_BUILD_AUDIT_PASS_E2E_ADMITTED", "build audit did not admit E2E")
    ...
    need(value.get("elf_sha256") == sha256_file(BUILD / "m3-v8-test-elf"), "build audit ELF SHA drift")
    return value


def verify_quiet_admission(path: pathlib.Path, controller_sha: str, build_audit: Mapping[str, Any]) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "M3_V8_QUIET_HOST_E2E_ADMITTED", "quiet-host audit did not admit E2E")
...
def e2e_once(build_audit_path: pathlib.Path, quiet_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    fsync_dir(stage)
    marker = consume_marker("E2E", verify_bootstrap_audit(quiet_path, controller_sha) if quiet.get("verdict") == "M3_V8_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED" else quiet)
    ...
    wrapper = {
        "schema": "lay.m3-v8-e2e-wrapper.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8_E2E_CREATED_UNAUDITED" if producer_complete else "BLOCKED_PROVENANCE",
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.m3-v8-status.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8_STATUS",
...
def self_check() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.m3-v8-remote-self-check.v1",
        "verdict": "M3_V8_REMOTE_CONTROLLER_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.m3-v8-remote-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
