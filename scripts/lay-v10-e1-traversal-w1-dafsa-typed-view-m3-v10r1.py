...
AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v10r1-audit.py"
...
CONTROLLER_EVIDENCE = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R1_REMOTE_CONTROLLER_V1_2026-08-27"
CONTROLLER_IMPLEMENTATION = CONTROLLER_EVIDENCE / "IMPLEMENTATION_RECEIPT.json"
EXECUTION_JOURNAL = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R1_EXECUTION_JOURNAL_V1_2026-08-27"

ADMISSION_RECEIPT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R1_EXECUTION_ADMISSION_V1_2026-08-27/EXECUTION_ADMISSION.json"
BOOTSTRAP_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R1_BOOTSTRAP_AUDIT_V1_2026-08-27/BOOTSTRAP_AUDIT.json"
BUILD_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R1_BUILD_AUDIT_V1_2026-08-27/BUILD_AUDIT.json"
QUIET_ADMISSION = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R1_QUIET_ADMISSION_V1_2026-08-27/QUIET_ADMISSION.json"
TERMINAL_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R1_TERMINAL_AUDIT_V1_2026-08-27/TERMINAL_AUDIT.json"
...
def verify_controller_preflight() -> dict[str, Any]:
    ...
    need(value.get("manifest_sha256") == "17ed3df22f9411dc0143bf5ed19be5f3493d19e09ee71254c782a481ae8dc980", "controller preflight manifest identity drift")
    return value
...
def self_check() -> dict[str, Any]:
    ...
    need(remote.get("verdict") == "V10R1_REMOTE_CONTROLLER_STATIC_PASS", "remote self-check failed")
    need(auditor.get("verdict") == "V10R1_INDEPENDENT_AUDITOR_STATIC_PASS", "auditor self-check failed")
    faults = fault_model()
    payload_fault = error_payload_fault()
    source_closure = source_closure_snapshot()
    return {
        "schema": "lay.v10r1-controller-self-check.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R1_CONTROLLER_STATIC_SELF_CHECK_PASS",
...
def seal_self_check() -> dict[str, Any]:
    ...
    receipt = {
        "schema": "lay.v10r1-controller-implementation.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R1_REMOTE_CONTROLLERS_VERIFIED_UNRUN",
...
def build_source_closure(destination: pathlib.Path) -> dict[str, Any]:
    ...
    need(len(rows) >= 700, "source closure unexpectedly small")
    manifest = {
        "schema": "lay.v10r1-source-closure.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "files": rows,
        "files_count": len(rows),
...
def prepare_bootstrap(admission: Mapping[str, Any]) -> pathlib.Path:
    staging = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v10r1-bootstrap-"))
    try:
        ...
        payload = {
            "schema": "lay.v10r1-bootstrap-payload.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def initialize_journal() -> pathlib.Path:
    ...
    metadata = {
        "schema": "lay.v10r1-execution-journal.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def append_intent(root: pathlib.Path, sequence: int, action: str) -> pathlib.Path:
    ...
    write_new(path, canonical({
        "schema": "lay.v10r1-external-intent.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def append_completion(root: pathlib.Path, sequence: int, action: str, response: Mapping[str, Any]) -> pathlib.Path:
    ...
    write_new(path, canonical({
        "schema": "lay.v10r1-external-completion.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def create_remote_cache() -> dict[str, Any]:
    ...
    return {"verdict": "V10R1_REMOTE_CACHE_CREATED", "stdout_sha256": sha256_bytes(output)}
...
def upload_bootstrap(staging: pathlib.Path) -> dict[str, Any]:
    ...
    return {"verdict": "V10R1_BOOTSTRAP_UPLOADED", "payload_sha256": sha256_file(staging / "PAYLOAD.json")}
...
def verify_implementation() -> dict[str, Any]:
    ...
    need(value.get("verdict") == "V10R1_REMOTE_CONTROLLERS_VERIFIED_UNRUN", "controller implementation verdict drift")
    for key, path in {
...
def finish_journal(journal: pathlib.Path, verdict: str, *, terminal_audit: pathlib.Path | None = None) -> None:
    terminal = {
        "schema": "lay.v10r1-controller-terminal.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": verdict,
...
def execute() -> dict[str, Any]:
    ...
    try:
        admission = journaled(journal, sequence, "live-admission", lambda: auditor_call("live-admission"), ["V10R1_EXECUTION_ADMITTED"])
        sequence += 1
        staging = prepare_bootstrap(admission)
        journaled(journal, sequence, "remote-cache-create", create_remote_cache, ["V10R1_REMOTE_CACHE_CREATED"])
        sequence += 1
        journaled(journal, sequence, "bootstrap-upload", lambda: upload_bootstrap(staging), ["V10R1_BOOTSTRAP_UPLOADED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            ...
            ["V10R1_BOOTSTRAP_CREATED_UNAUDITED"],
        )
        sequence += 1
        journaled(journal, sequence, "bootstrap-audit", lambda: auditor_call("bootstrap"), ["V10R1_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "bootstrap-audit-upload",
            lambda: upload_audit(BOOTSTRAP_AUDIT, "BOOTSTRAP_AUDIT.json", "V10R1_BOOTSTRAP_AUDIT_UPLOADED"),
            ["V10R1_BOOTSTRAP_AUDIT_UPLOADED"],
        )
        sequence += 1
        ...
        build = journaled(
            ...
            ["V10R1_BUILD_CREATED_UNAUDITED", "BLOCKED_BUILD"],
        )
        sequence += 1
        build_audit = journaled(
            ...
            ["V10R1_BUILD_AUDIT_PASS_TRACE_ADMITTED", "BLOCKED_BUILD"],
        )
        sequence += 1
        if build.get("verdict") == "BLOCKED_BUILD" or build_audit.get("verdict") == "BLOCKED_BUILD":
            ...
            return build_audit
        need(build.get("verdict") == "V10R1_BUILD_CREATED_UNAUDITED", "build producer verdict drift")
        journaled(
            journal,
            sequence,
            "build-audit-upload",
            lambda: upload_audit(BUILD_AUDIT, "BUILD_AUDIT.json", "V10R1_BUILD_AUDIT_UPLOADED"),
            ["V10R1_BUILD_AUDIT_UPLOADED"],
        )
        sequence += 1
        journaled(journal, sequence, "quiet-audit", lambda: auditor_call("quiet"), ["V10R1_QUIET_HOST_TRACE_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "quiet-audit-upload",
            lambda: upload_audit(QUIET_ADMISSION, "QUIET_ADMISSION.json", "V10R1_QUIET_ADMISSION_UPLOADED"),
            ["V10R1_QUIET_ADMISSION_UPLOADED"],
        )
        sequence += 1
        ...
        journaled(
            ...
            ["V10R1_TRACE_CREATED_UNAUDITED", "BLOCKED_PROVENANCE"],
        )
        sequence += 1
        terminal = journaled(
            journal,
        ...
        failure = {
            "schema": "lay.v10r1-controller-failure.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10r1-controller-status.v1",
        "verdict": "V10R1_CONTROLLER_STATUS",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.v10r1-controller-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "BLOCKED_PROVENANCE",
            "error": f"{type(error).__name__}: {error}",
