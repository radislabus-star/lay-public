...
TASK_ID = "slice8b-v10-e1-traversal-w1-dafsa-typed-view-m3-e2e-v8-20260827"
TRANSACTION_ID = "e9c2674282e9feaff1e75c7ca17b74040b3a8dd8b1bbab2c26e999917a6d35ec"
...
REMOTE_CACHE = pathlib.PurePosixPath("/home/e/.cache") / f"lay-m3-v8-{TRANSACTION_ID}"

CONTROLLER = pathlib.Path(__file__).resolve()
REMOTE_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v8-remote.py"
AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v8-audit.py"
V8_SOURCE = ROOT / "src/nanda_wave/l2_field/v13_typed_peak.rs"
CONTROLLER_PREFLIGHT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_REMOTE_CONTROLLER_V8_V1_PREFLIGHT_2026-08-27.json"
...
CONTROLLER_EVIDENCE = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_REMOTE_CONTROLLER_V8_V1_2026-08-27"
CONTROLLER_IMPLEMENTATION = CONTROLLER_EVIDENCE / "IMPLEMENTATION_RECEIPT.json"
EXECUTION_JOURNAL = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_EXECUTION_JOURNAL_V1_2026-08-27"

ADMISSION_RECEIPT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_EXECUTION_ADMISSION_V1_2026-08-27/EXECUTION_ADMISSION.json"
BOOTSTRAP_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_BOOTSTRAP_AUDIT_V1_2026-08-27/BOOTSTRAP_AUDIT.json"
BUILD_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_BUILD_AUDIT_V1_2026-08-27/BUILD_AUDIT.json"
QUIET_ADMISSION = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_QUIET_ADMISSION_V1_2026-08-27/QUIET_ADMISSION.json"
TERMINAL_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8_TERMINAL_AUDIT_V1_2026-08-27/TERMINAL_AUDIT.json"
...
def verify_controller_preflight() -> dict[str, Any]:
    ...
    need(value.get("manifest_sha256") == "6871c00af60972a18d8c17db75fc388b14017a5406a59e1268ef9e69294f0d24", "controller preflight manifest identity drift")
    return value
...
def self_check() -> dict[str, Any]:
    ...
    need(remote.get("verdict") == "M3_V8_REMOTE_CONTROLLER_STATIC_PASS", "remote self-check failed")
    need(auditor.get("verdict") == "M3_V8_INDEPENDENT_AUDITOR_STATIC_PASS", "auditor self-check failed")
    faults = fault_model()
    return {
        "schema": "lay.m3-v8-controller-self-check.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8_CONTROLLER_STATIC_SELF_CHECK_PASS",
...
def seal_self_check() -> dict[str, Any]:
    ...
    check = self_check()
    receipt = {
        "schema": "lay.m3-v8-controller-implementation.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8_REMOTE_CONTROLLERS_VERIFIED_UNRUN",
...
def build_source_closure(destination: pathlib.Path) -> dict[str, Any]:
    ...
    manifest = {
        "schema": "lay.m3-v8-source-closure.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "files": rows,
...
def prepare_bootstrap(admission: Mapping[str, Any]) -> pathlib.Path:
    staging = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v8-bootstrap-"))
    try:
        ...
        payload = {
            "schema": "lay.m3-v8-bootstrap-payload.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def initialize_journal() -> pathlib.Path:
    ...
    metadata = {
        "schema": "lay.m3-v8-execution-journal.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def append_intent(root: pathlib.Path, sequence: int, action: str) -> pathlib.Path:
    ...
    write_new(path, canonical({
        "schema": "lay.m3-v8-external-intent.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def append_completion(root: pathlib.Path, sequence: int, action: str, response: Mapping[str, Any]) -> pathlib.Path:
    ...
    write_new(path, canonical({
        "schema": "lay.m3-v8-external-completion.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def create_remote_cache() -> dict[str, Any]:
    ...
    return {"verdict": "M3_V8_REMOTE_CACHE_CREATED", "stdout_sha256": sha256_bytes(output)}
...
def upload_bootstrap(staging: pathlib.Path) -> dict[str, Any]:
    ...
    return {"verdict": "M3_V8_BOOTSTRAP_UPLOADED", "payload_sha256": sha256_file(staging / "PAYLOAD.json")}
...
def verify_implementation() -> dict[str, Any]:
    ...
    need(value.get("verdict") == "M3_V8_REMOTE_CONTROLLERS_VERIFIED_UNRUN", "controller implementation verdict drift")
    for key, path in {
...
def execute() -> dict[str, Any]:
    ...
    try:
        admission = journaled(journal, sequence, "live-admission", lambda: auditor_call("live-admission"), ["M3_V8_EXECUTION_ADMITTED"])
        sequence += 1
        staging = prepare_bootstrap(admission)
        journaled(journal, sequence, "remote-cache-create", create_remote_cache, ["M3_V8_REMOTE_CACHE_CREATED"])
        sequence += 1
        journaled(journal, sequence, "bootstrap-upload", lambda: upload_bootstrap(staging), ["M3_V8_BOOTSTRAP_UPLOADED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            ...
            ["M3_V8_BOOTSTRAP_CREATED_UNAUDITED"],
        )
        sequence += 1
        journaled(journal, sequence, "bootstrap-audit", lambda: auditor_call("bootstrap"), ["M3_V8_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "bootstrap-audit-upload",
            lambda: upload_audit(BOOTSTRAP_AUDIT, "BOOTSTRAP_AUDIT.json", "M3_V8_BOOTSTRAP_AUDIT_UPLOADED"),
            ["M3_V8_BOOTSTRAP_AUDIT_UPLOADED"],
        )
        sequence += 1
        ...
        build = journaled(
            ...
            ["M3_V8_BUILD_CREATED_UNAUDITED", "BLOCKED_BUILD"],
        )
        sequence += 1
        need(build.get("verdict") == "M3_V8_BUILD_CREATED_UNAUDITED", "one remote build ended terminally")
        journaled(journal, sequence, "build-audit", lambda: auditor_call("build"), ["M3_V8_BUILD_AUDIT_PASS_E2E_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "build-audit-upload",
            lambda: upload_audit(BUILD_AUDIT, "BUILD_AUDIT.json", "M3_V8_BUILD_AUDIT_UPLOADED"),
            ["M3_V8_BUILD_AUDIT_UPLOADED"],
        )
        sequence += 1
        journaled(journal, sequence, "quiet-audit", lambda: auditor_call("quiet"), ["M3_V8_QUIET_HOST_E2E_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "quiet-audit-upload",
            lambda: upload_audit(QUIET_ADMISSION, "QUIET_ADMISSION.json", "M3_V8_QUIET_ADMISSION_UPLOADED"),
            ["M3_V8_QUIET_ADMISSION_UPLOADED"],
        )
        sequence += 1
        ...
        journaled(
            ...
            ["M3_V8_E2E_CREATED_UNAUDITED", "BLOCKED_PROVENANCE"],
        )
        sequence += 1
        terminal = journaled(
            journal,
        ...
        write_new(journal / "TERMINAL.json", canonical({
            "schema": "lay.m3-v8-controller-terminal.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
        ...
        failure = {
            "schema": "lay.m3-v8-controller-failure.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.m3-v8-controller-status.v1",
        "verdict": "M3_V8_CONTROLLER_STATUS",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.m3-v8-controller-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "BLOCKED_PROVENANCE",
            "error": f"{type(error).__name__}: {error}",
