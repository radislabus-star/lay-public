#!/usr/bin/env python3
"""Fail-closed local orchestrator for the V10R3 TRACE-only recovery."""
...
AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v10r3-audit.py"
...
CONTROLLER_PREFLIGHT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R3_TRACE_REUSE_CONTROLLER_V1_PREFLIGHT_2026-08-27.json"
CONTROLLER_EVIDENCE = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R3_TRACE_REUSE_CONTROLLER_V1_2026-08-27"
CONTROLLER_IMPLEMENTATION = CONTROLLER_EVIDENCE / "IMPLEMENTATION_RECEIPT.json"
EXECUTION_JOURNAL = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R3_EXECUTION_JOURNAL_V1_2026-08-27"
...
ADMISSION_RECEIPT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R3_EXECUTION_ADMISSION_V1_2026-08-27/EXECUTION_ADMISSION.json"
BOOTSTRAP_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R3_BOOTSTRAP_REUSE_AUDIT_V1_2026-08-27/BOOTSTRAP_AUDIT.json"
QUIET_ADMISSION = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R3_QUIET_READINESS_V1_2026-08-27/QUIET_ADMISSION.json"
TERMINAL_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10R3_TERMINAL_AUDIT_V1_2026-08-27/TERMINAL_AUDIT.json"
...
class V10R3ControllerError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise V10R3ControllerError(message)
...
def verify_controller_preflight() -> dict[str, Any]:
    ...
    need(value.get("manifest_sha256") == "10e233ca801e4c747901fa768f311c47097b960b4d732bc56c9f77b124a4d49a", "controller preflight manifest identity drift")
    return value
...
def verify_fixed_inputs() -> dict[str, Any]:
    expected = {
        ...
        CONTROLLER_PREFLIGHT: (22_930, "50261f2b14bc45a97ed62e81c8a41149c65815d3d70021ed899c3f32e867c110", "0444"),
...
def python_literal(tree: ast.Module, name: str) -> Any:
    for node in tree.body:
        ...
        if isinstance(node, ast.AnnAssign) and isinstance(node.target, ast.Name) and node.target.id == name:
            return ast.literal_eval(node.value)
    raise V10R3ControllerError(f"Python registry absent: {name}")
...
def static_graph() -> dict[str, Any]:
    ...
    need(tuple(python_literal(auditor_tree, "ACTIONS")) == ("self-check", "live-admission", "bootstrap", "quiet", "terminal", "status"), "auditor action registry drift")
...
def error_payload_fault() -> dict[str, Any]:
    ...
    except V10R3ControllerError as error:
        message = str(error)
    ...
    raise V10R3ControllerError("error payload fault did not fail")
...
def self_check() -> dict[str, Any]:
    ...
    need(remote.get("verdict") == "V10R3_REMOTE_CONTROLLER_STATIC_PASS", "remote self-check failed")
    need(auditor.get("verdict") == "V10R3_INDEPENDENT_AUDITOR_STATIC_PASS", "auditor self-check failed")
    return {
        "schema": "lay.v10r3-controller-self-check.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R3_CONTROLLER_STATIC_SELF_CHECK_PASS",
...
def seal_self_check() -> dict[str, Any]:
    ...
    check = self_check()
    receipt = {
        "schema": "lay.v10r3-controller-implementation.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R3_TRACE_REUSE_CONTROLLERS_VERIFIED_UNRUN",
...
def verify_implementation() -> dict[str, Any]:
    ...
    need(value.get("verdict") == "V10R3_TRACE_REUSE_CONTROLLERS_VERIFIED_UNRUN", "implementation verdict drift")
...
def prepare_bootstrap(admission: Mapping[str, Any]) -> pathlib.Path:
    stage = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v10r3-bootstrap-"))
    try:
        ...
        payload = {
            "schema": "lay.v10r3-bootstrap-reuse-payload.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def initialize_journal() -> pathlib.Path:
    ...
    write_new(EXECUTION_JOURNAL / "JOURNAL_HEADER.json", canonical({
        "schema": "lay.v10r3-controller-journal.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def append_intent(root: pathlib.Path, sequence: int, action: str) -> pathlib.Path:
    ...
    write_new(path, canonical({
        "schema": "lay.v10r3-controller-intent.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def append_completion(root: pathlib.Path, sequence: int, action: str, response: Mapping[str, Any]) -> pathlib.Path:
    ...
    write_new(path, canonical({
        "schema": "lay.v10r3-controller-completion.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def create_remote_cache() -> dict[str, Any]:
    ...
    return {"task_id": TASK_ID, "transaction_id": TRANSACTION_ID, "verdict": "V10R3_REMOTE_CACHE_CREATED"}
...
def upload_bootstrap(staging: pathlib.Path) -> dict[str, Any]:
    ...
    return {
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R3_BOOTSTRAP_UPLOADED",
...
def finish_journal(journal: pathlib.Path, verdict: str, *, terminal_audit: pathlib.Path | None = None) -> None:
    write_new(journal / "TERMINAL.json", canonical({
        "schema": "lay.v10r3-controller-terminal.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": verdict,
...
def execute() -> dict[str, Any]:
    ...
    try:
        admission = journaled(journal, sequence, "live-admission", lambda: auditor_call("live-admission"), ("V10R3_EXECUTION_ADMITTED",))
        sequence += 1
        staging = prepare_bootstrap(admission)
        journaled(journal, sequence, "remote-cache-create", create_remote_cache, ("V10R3_REMOTE_CACHE_CREATED",))
        sequence += 1
        journaled(journal, sequence, "bootstrap-upload", lambda: upload_bootstrap(staging), ("V10R3_BOOTSTRAP_UPLOADED",))
        sequence += 1
        journaled(journal, sequence, "remote-bootstrap", lambda: remote_call("bootstrap-reuse", "--bootstrap", str(REMOTE_CACHE), cache=True), ("V10R3_BOOTSTRAP_REUSE_CREATED_UNAUDITED",))
        sequence += 1
        journaled(journal, sequence, "bootstrap-audit", lambda: auditor_call("bootstrap"), ("V10R3_BOOTSTRAP_REUSE_AUDIT_PASS_QUIET_ADMITTED",))
        sequence += 1
        journaled(journal, sequence, "bootstrap-audit-upload", lambda: upload_audit({"BOOTSTRAP_AUDIT.json": BOOTSTRAP_AUDIT}, "V10R3_BOOTSTRAP_AUDIT_UPLOADED"), ("V10R3_BOOTSTRAP_AUDIT_UPLOADED",))
        sequence += 1
        quiet = journaled(journal, sequence, "quiet-audit", lambda: auditor_call("quiet"), ("V10R3_QUIET_READY_TRACE_ADMITTED", "BLOCKED_QUIET_BEFORE_MARKER"))
        sequence += 1
        if quiet.get("verdict") == "BLOCKED_QUIET_BEFORE_MARKER":
            finish_journal(journal, "BLOCKED_QUIET_BEFORE_MARKER")
            return quiet
        journaled(journal, sequence, "quiet-audit-upload", lambda: upload_audit({"QUIET_ADMISSION.json": QUIET_ADMISSION}, "V10R3_QUIET_ADMISSION_UPLOADED"), ("V10R3_QUIET_ADMISSION_UPLOADED",))
        sequence += 1
        quiet_remote = str(REMOTE_CACHE / "admissions/QUIET_ADMISSION.json")
        journaled(journal, sequence, "create-marker", lambda: remote_call("create-marker", "--quiet", quiet_remote), ("V10R3_TRACE_MARKER_AVAILABLE",))
        sequence += 1
        journaled(journal, sequence, "trace-once", lambda: remote_call("trace-once", "--quiet", quiet_remote, timeout=10_800), ("V10R3_TRACE_REUSE_CREATED_UNAUDITED", "BLOCKED_PROVENANCE"))
        sequence += 1
        ...
        failure = {
            "schema": "lay.v10r3-controller-failure.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
        ...
        write_manifest(journal)
        seal_tree(journal)
        raise V10R3ControllerError(json.dumps(failure, sort_keys=True)) from error
    finally:
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10r3-controller-status.v1",
        "verdict": "V10R3_CONTROLLER_STATUS",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.v10r3-controller-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "BLOCKED_PROVENANCE",
            "error": f"{type(error).__name__}: {error}",
