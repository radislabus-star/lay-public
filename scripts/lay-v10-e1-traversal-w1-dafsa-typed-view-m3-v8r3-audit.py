#!/usr/bin/env python3
"""Independent read-only auditor for the M3 V8R3 direct-exec proof."""
...
REMOTE_EXECUTABLE = REMOTE_PARENT / "bootstrap-v1/m3-v8r3-test-elf"
...
LOCAL_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v8r3.py"
...
V8R3_PREFLIGHT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_TERMINAL_PROJECTION_V1_PREFLIGHT_2026-08-27.json"
...
CONTROLLER_IMPLEMENTATION = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_REMOTE_CONTROLLER_V8R3_V1_2026-08-27/IMPLEMENTATION_RECEIPT.json"

RECEIPTS = ROOT / "docs/structural_gates/receipts"
ADMISSION_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_EXECUTION_ADMISSION_V1_2026-08-27"
BOOTSTRAP_AUDIT_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_BOOTSTRAP_AUDIT_V1_2026-08-27"
QUIET_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_QUIET_ADMISSION_V1_2026-08-27"
TERMINAL_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_TERMINAL_AUDIT_V1_2026-08-27"
...
class V8R3AuditError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise V8R3AuditError(message)
...
def fixed_local() -> dict[str, Any]:
    expected = {
        ...
        V8R3_PREFLIGHT: "ca3745efde0d2c9311b0f2d777c671ae53e4a3146d11c9ce0ef20897b84d4cb3",
    ...
    for path in (V8R3_PAPER, V8R3_ROUTE, V8R3_ROUTE_RECEIPT, V8R3_PREFLIGHT, V8R2_DIAGNOSIS, V8R2_JOURNAL, V8R2_CONTROLLER):
        need(mode_string(path) == "0444", f"sealed predecessor mode drift: {path}")
    validate_local_predecessor_pair()
    preflight = load_json(V8R3_PREFLIGHT)
    need(preflight.get("verdict") == "READY_TO_IMPLEMENT" and preflight.get("safe_to_implement") is True, "effective V8R3 preflight drift")
    ...
    need(implementation.get("verdict") == "M3_V8R3_CONTROLLERS_VERIFIED_UNRUN", "controller implementation verdict drift")
    for key, path in {
...
v8r2_parent=pathlib.Path(sys.argv[9]); v8r2_state=pathlib.Path(sys.argv[10]); v8r2_cache=pathlib.Path(sys.argv[11])
def sha(path):
 h=hashlib.sha256()
...
out={
 ...
 'v8r1':{'parent':old_parent.exists(),'state':old_state.exists(),'elf':row(old_elf) if old_elf.is_file() else None,'wrapper':row(old_parent/'e2e-v1/E2E_WRAPPER.json') if (old_parent/'e2e-v1/E2E_WRAPPER.json').is_file() else None,'latest_state':latest(old_state),'markers':markers(old_state),'inputs':{name:row(old_inputs/name) for name in input_names if (old_inputs/name).is_file()}},
...
def validate_local_predecessor_pair() -> dict[str, Any]:
    ...
    diagnosis = load_json(V8R2_DIAGNOSIS)
    ...
    need(
        ...
        "local V8R1 producer/terminal pair drift",
    )
    need(
        diagnosis.get("verdict") == "V8R2_REMOTE_TERMINAL_PROJECTION_DEFECT_CONFIRMED"
        and diagnosis.get("execution_verdict") == "BLOCKED_PROVENANCE"
        and diagnosis.get("execution_journal_sha256sums_sha256") == sha256_file(V8R2_JOURNAL),
        "V8R2 terminal diagnosis drift",
    )
    return {
        ...
        "v8r2_verdict": diagnosis["execution_verdict"],
    }
...
def validate_v8r1(value: Mapping[str, Any]) -> None:
    ...
    need(
        producer.get("task_id") == V8R1_TASK_ID
        and producer.get("transaction_id") == V8R1_TRANSACTION_ID
        and producer.get("state") == "E2E_CREATED_UNAUDITED"
        and producer.get("e2e_wrapper_sha256") == EXPECTED_V8R1_WRAPPER_SHA256,
        "V8R1 producer state drift",
    )
    wrapper = old.get("wrapper", {})
    need(wrapper.get("mode") == "0444" and wrapper.get("sha256") == EXPECTED_V8R1_WRAPPER_SHA256, "V8R1 remote wrapper drift")
...
try:
 a=p/'a'; b=p/'b'; f=open(a,'xb'); f.write(b'm3-v8r3-admission\n'); f.flush(); os.fsync(f.fileno()); f.close(); os.rename(a,b)
 assert b.read_bytes()==b'm3-v8r3-admission\n'; b.unlink(); p.rmdir()
...
def live_admission() -> dict[str, Any]:
    ...
    need(before["paths"] == {"parent": False, "state": False, "cache": False}, "V8R3 remote namespace is not absent")
    ...
    probe_path = f"/home/e/.cache/lay-m3-v8r3-admission-{TRANSACTION_ID}"
    ...
    receipt = {
        "schema": "lay.m3-v8r3-execution-admission.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R3_EXECUTION_ADMITTED",
        ...
        "v8r2_diagnosis_sha256": sha256_file(V8R2_DIAGNOSIS),
        "v8r2_journal_sha256sums_sha256": sha256_file(V8R2_JOURNAL),
        "host_before": before,
        "host_after": after,
...
def bootstrap_audit() -> dict[str, Any]:
    ...
    need(admission.get("verdict") == "M3_V8R3_EXECUTION_ADMITTED", "execution admission absent")
    ...
    need(copied.get("mode") == "0555", "V8R3 executable copy mode drift")
    ...
    need(bootstrap.get("verdict") == "M3_V8R3_BOOTSTRAP_CREATED_UNAUDITED", "bootstrap producer verdict drift")
    ...
    receipt = {
        "schema": "lay.m3-v8r3-bootstrap-audit.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R3_BOOTSTRAP_AUDIT_PASS_MARKER_ADMITTED",
        "local_controller_sha256": sha256_file(LOCAL_CONTROLLER),
        "remote_controller_sha256": sha256_file(REMOTE_CONTROLLER),
        "auditor_sha256": sha256_file(AUDITOR),
...
def marker_payload(admission: Mapping[str, Any]) -> bytes:
    return canonical({
        "schema": "lay.m3-v8r3-one-shot-marker.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "route": "E2E",
...
def quiet_admission() -> dict[str, Any]:
    ...
    need(bootstrap.get("verdict") == "M3_V8R3_BOOTSTRAP_AUDIT_PASS_MARKER_ADMITTED", "bootstrap audit absent")
    before = remote_snapshot()
    validate_host(before)
    validate_v8r1(before)
    ...
    receipt = {
        "schema": "lay.m3-v8r3-quiet-admission.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R3_QUIET_HOST_E2E_ADMITTED",
...
def terminal_decision(subject: Mapping[str, Any], wrapper: Mapping[str, Any]) -> tuple[str, dict[str, list[str]]]:
    ...
    if wrapper.get("verdict") != "M3_V8R3_E2E_CREATED_UNAUDITED" or wrapper.get("controller_error") is not None:
...
def terminal_audit() -> dict[str, Any]:
    ...
    need(quiet.get("verdict") == "M3_V8R3_QUIET_HOST_E2E_ADMITTED", "quiet admission absent")
    snapshot = remote_snapshot()
    validate_host(snapshot)
    validate_v8r1(snapshot)
    ...
    temporary = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v8r3-terminal-"))
    try:
        ...
        receipt = {
            "schema": "lay.m3-v8r3-terminal-audit.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": verdict,
...
def synthetic_positive() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    wrapper = {
        "verdict": "M3_V8R3_E2E_CREATED_UNAUDITED",
...
def self_check() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.m3-v8r3-independent-auditor-self-check.v1",
        "verdict": "M3_V8R3_INDEPENDENT_AUDITOR_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "auditor_sha256": sha256_file(AUDITOR),
...
def status() -> dict[str, Any]:
    return {
        "schema": "lay.m3-v8r3-auditor-status.v1",
        "verdict": "M3_V8R3_AUDITOR_STATUS",
        "receipts": {
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.m3-v8r3-auditor-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
