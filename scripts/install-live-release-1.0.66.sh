...
exact_v13_file_identity() {
    local path="${1:?file path required}"
    local output_bytes="${2:?byte output name required}"
    local output_sha256="${3:?hash output name required}"
    local -n bytes_ref="$output_bytes"
    local -n sha256_ref="$output_sha256"

    [[ -f "$path" && ! -L "$path" ]] || return 1
    bytes_ref="$(stat -c '%s' -- "$path")" || return 1
    [[ "$bytes_ref" =~ ^[1-9][0-9]*$ ]] || return 1
    sha256_ref="$(sha256sum -- "$path" | awk '{print $1}')" || return 1
    [[ "$sha256_ref" =~ ^[0-9a-f]{64}$ ]]
}

exact_v13_receipt_bytes_are_text() {
    local receipt="${1:?receipt required}"
    local final_byte

    [[ -f "$receipt" && ! -L "$receipt" && -s "$receipt" ]] || return 1
    final_byte="$(tail -c 1 -- "$receipt" | od -An -tu1 | tr -d '[:space:]')" \
        || return 1
    [[ "$final_byte" == 10 ]] || return 1
    LC_ALL=C od -An -v -tu1 -- "$receipt" | awk '
        {
            for (field_index = 1; field_index <= NF; field_index++) {
                if ($field_index != 10 && ($field_index < 32 || $field_index > 126)) {
                    exit 1
                }
            }
        }
...
exact_v13_read_receipt() {
    local receipt="${1:?receipt required}"
    local line key value
    local -A fields=()
    local -a required=(
        schema release_version compile_operation compile_exit_status
        package_name package_bytes package_sha256
        compiler_name compiler_bytes compiler_sha256
        sidecar_name sidecar_bytes sidecar_sha256
    )

    [[ -f "$receipt" && ! -L "$receipt" && -s "$receipt" ]] || return 1
    [[ "$(stat -c '%a' -- "$receipt")" == 644 ]] || return 1
    exact_v13_receipt_bytes_are_text "$receipt" || return 1
    ...
    [[ "${#fields[@]}" == "${#required[@]}" ]] || return 1
    for key in "${required[@]}"; do
        [[ -n "${fields[$key]+present}" ]] || return 1
    done

    [[ "${fields[schema]}" == lay.release.exact-v13-sidecar.v1 ]] || return 1
    [[ "${fields[release_version]}" == "$EXPECTED_VERSION" ]] || return 1
    [[ "${fields[compile_operation]}" == "$EXACT_V13_COMPILE_OPERATION" ]] || return 1
    [[ "${fields[compile_exit_status]}" == 0 ]] || return 1
    [[ "${fields[package_name]}" == "$LAY_CANONICAL_L2_PACKAGE_NAME" ]] || return 1
    [[ "${fields[package_bytes]}" == "$LAY_CANONICAL_L2_PACKAGE_BYTES" ]] || return 1
    [[ "${fields[package_sha256]}" == "$LAY_CANONICAL_L2_PACKAGE_SHA256" ]] || return 1
    [[ "${fields[compiler_name]}" == lay-nanda-wave-train ]] || return 1
    [[ "${fields[compiler_bytes]}" == "$EXACT_V13_COMPILER_BYTES" ]] || return 1
    [[ "${fields[compiler_sha256]}" == "$EXACT_V13_COMPILER_SHA256" ]] || return 1
    [[ "${fields[sidecar_name]}" == "$EXACT_V13_SIDECAR_NAME" ]] || return 1
    [[ "${fields[sidecar_bytes]}" == "$EXACT_V13_SIDECAR_BYTES" ]] || return 1
    [[ "${fields[sidecar_sha256]}" == "$EXACT_V13_SIDECAR_SHA256" ]]
}
...
capture_exact_v13_sidecar_admission() {
    ...
    source "$ROOT/scripts/l2-package-contract.sh" || return 1
    EXACT_V13_PACKAGE_SOURCE="$(
        LAY_L2_OFFLINE=1 "$ROOT/scripts/resolve-l2-package.sh"
    )" || return 1
    [[ "$(basename -- "$EXACT_V13_PACKAGE_SOURCE")" == "$LAY_CANONICAL_L2_PACKAGE_NAME" ]] \
        || return 1
    [[ -x "$EXACT_V13_COMPILER" ]] || return 1
    [[ "$(stat -c '%a' -- "$EXACT_V13_SIDECAR_SOURCE")" == 644 ]] || return 1
    [[ -f "$L2_DIR/$EXACT_V13_SIDECAR_NAME" \
        && ! -L "$L2_DIR/$EXACT_V13_SIDECAR_NAME" ]] || return 1

    exact_v13_file_identity \
        "$EXACT_V13_PACKAGE_SOURCE" EXACT_V13_PACKAGE_BYTES EXACT_V13_PACKAGE_SHA256 \
        || return 1
    [[ "$EXACT_V13_PACKAGE_BYTES" == "$LAY_CANONICAL_L2_PACKAGE_BYTES" \
        && "$EXACT_V13_PACKAGE_SHA256" == "$LAY_CANONICAL_L2_PACKAGE_SHA256" ]] \
        || return 1
    exact_v13_file_identity \
        "$EXACT_V13_COMPILER" EXACT_V13_COMPILER_BYTES EXACT_V13_COMPILER_SHA256 \
        || return 1
    exact_v13_file_identity \
        "$EXACT_V13_SIDECAR_SOURCE" EXACT_V13_SIDECAR_BYTES EXACT_V13_SIDECAR_SHA256 \
        || return 1
    exact_v13_file_identity \
        "$EXACT_V13_SIDECAR_RECEIPT" receipt_bytes EXACT_V13_RECEIPT_SHA256 \
        || return 1
    exact_v13_read_receipt "$EXACT_V13_SIDECAR_RECEIPT"
}

exact_v13_sidecar_sources_unchanged() {
    local package_bytes package_sha256 compiler_bytes compiler_sha256
    local sidecar_bytes sidecar_sha256 receipt_bytes receipt_sha256

    exact_v13_file_identity \
        "$EXACT_V13_PACKAGE_SOURCE" package_bytes package_sha256 || return 1
    [[ "$package_bytes" == "$EXACT_V13_PACKAGE_BYTES" \
        && "$package_sha256" == "$EXACT_V13_PACKAGE_SHA256" ]] || return 1
    exact_v13_file_identity \
        "$EXACT_V13_COMPILER" compiler_bytes compiler_sha256 || return 1
    [[ "$compiler_bytes" == "$EXACT_V13_COMPILER_BYTES" \
        && "$compiler_sha256" == "$EXACT_V13_COMPILER_SHA256" ]] || return 1
    exact_v13_file_identity \
        "$EXACT_V13_SIDECAR_SOURCE" sidecar_bytes sidecar_sha256 || return 1
    [[ "$sidecar_bytes" == "$EXACT_V13_SIDECAR_BYTES" \
        && "$sidecar_sha256" == "$EXACT_V13_SIDECAR_SHA256" ]] || return 1
    exact_v13_file_identity \
        "$EXACT_V13_SIDECAR_RECEIPT" receipt_bytes receipt_sha256 || return 1
    [[ "$receipt_sha256" == "$EXACT_V13_RECEIPT_SHA256" ]] || return 1
    exact_v13_read_receipt "$EXACT_V13_SIDECAR_RECEIPT"
}

verify_installed_exact_v13_sidecar() {
    local destination="$L2_DIR/$EXACT_V13_SIDECAR_NAME"
    local installed_bytes installed_sha256

    verify_file_bytes_mode "$EXACT_V13_SIDECAR_SOURCE" "$destination" 644 \
        || return 1
    exact_v13_file_identity "$destination" installed_bytes installed_sha256 \
        || return 1
    [[ "$installed_bytes" == "$EXACT_V13_SIDECAR_BYTES" \
        && "$installed_sha256" == "$EXACT_V13_SIDECAR_SHA256" ]]
}
...
verify_forward() {
    ...
    local package_file package_hash package_mode sidecar_file sidecar_mode sidecar_hash
    local specs=(
        "lay:lay"
    ...
    [[ "$(file_count "$L2_DIR")" == 7 ]] || return 1
    [[ -s "$sidecar_file" && "$sidecar_mode" == 644 ]] || return 1
    exact_v13_sidecar_sources_unchanged || return 1
    verify_installed_exact_v13_sidecar || return 1
    sidecar_hash="$(sha256sum "$sidecar_file" | awk '{print $1}')" || return 1
...
if [[ "$RELEASE_MODE" == forward ]]; then
    ...
    capture_exact_v13_sidecar_admission || {
        echo "remote exact V13 sidecar or receipt failed preflight admission" >&2
        exit 2
    }
else
...
(
    ...
    LAY_SKIP_EXACT_V13_SIDECAR=1 \
        LAY_L2_OFFLINE=1 \
        LAY_L2_PACKAGE_SOURCE="$EXACT_V13_PACKAGE_SOURCE" \
    ...
    exact_v13_sidecar_sources_unchanged
    install_file_atomic \
        "$EXACT_V13_SIDECAR_SOURCE" \
        "$L2_DIR/$EXACT_V13_SIDECAR_NAME" 0644
    verify_installed_exact_v13_sidecar
