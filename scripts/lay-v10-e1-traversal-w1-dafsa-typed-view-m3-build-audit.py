#!/usr/bin/env python3
"""Independent read-only audit of the single M3 diagnostic ELF."""
...
RESULT = ROOT / (
    "docs/structural_gates/receipts/"
    "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_"
    "BUILD_AUDIT_V1_2026-08-27"
)
EXPECTED_ASSEMBLED = "56a0c3243ec8ea9fc3a4f8bf49c4b56f10e5ba38b1158026271dd34218f13aba"
EXPECTED_PREFIX = "ce9ea2d290602774e2f45444e10e6fdabce6af4a61ae2bdca54c6d2e31a53f26"
OWNER_TOKENS = ("d1_enumerate_lane_prepared", "m3_enumerate_lane_prepared_t")
...
EXPECTED_CARGO_TAIL = (
    ...
    "--lib",
    "--no-run",
    "nanda_wave::l2_field::v13_typed_peak::tests::v10_m3_dafsa_typed_view_physical",
)
...
def self_check() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-build-audit-self-check.v3",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_BUILD_AUDITOR_VERIFIED_UNRUN",
        **values,
        "elf_executions": 0,
        "remote_writes": 0,
...
def inspect_elf(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(elf_type == 3, "M3 ELF is not ET_DYN")
    ...
    for token in OWNER_TOKENS:
        ...
        need(rows, f"M3 machine owner absent: {token}")
        parsed = []
        for row in rows:
        ...
        need(parsed and all(item["size"] > 0 for item in parsed), f"M3 owner range invalid: {token}")
        owners[token] = parsed
    starts = [item["address"] for rows in owners.values() for item in rows]
    need(len(starts) == len(set(starts)), "B/T code folding or owner overlap detected")
    owner_ranges = sorted(
...
def validate(live: dict[str, Any], build: pathlib.Path) -> dict[str, Any]:
    ...
    need(provenance.get("verdict") == "M3_BUILD_CREATED_UNAUDITED", "build producer verdict drift")
...
def audit() -> dict[str, Any]:
    ...
    need(bootstrap.get("verdict") == "M3_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED", "bootstrap audit drift")
    ...
    try:
        ...
        verdict = "M3_BUILD_AUDITED_PARITY_ADMITTED" if issue is None else issue.verdict
        receipt = {
            "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-build-audit.v3",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": verdict,
            ...
            **science,
            ...
            "next_action_admitted": "one M3 parity route only" if issue is None else "terminal audit only; no rebuild",
        }
        write_json(stage / "M3_BUILD_AUDIT_RECEIPT.json", receipt)
        ...
        return {**receipt, "receipt_sha256": sha256_file(RESULT / "M3_BUILD_AUDIT_RECEIPT.json")}
...
def main() -> int:
    ...
    try:
        ...
        print(f"M3 BUILD AUDIT ERROR: {type(error).__name__}: {error}", file=sys.stderr)
        return 1
