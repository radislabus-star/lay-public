...
EXPECTED_SOURCE = {
    ...
    "src/typing_transition/proposal_admission.rs": (87_732, "b563f8a9400f9ca61d1d4bf4f31c06b146b30d6f2d78cd364e4d69636aafd3e3"),
...
def verify_source_closure(root: pathlib.Path) -> dict[str, Any]:
    ...
    need(manifest.get("schema") == "lay.v10r1-source-closure.v1", "source manifest schema drift")
...
def verify_execution_admission(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(value.get("verdict") == "V10R1_EXECUTION_ADMITTED", "execution admission verdict drift")
...
def append_state(sequence: int, state: str, **extra: Any) -> None:
    write_json(
        ...
        {
            "schema": "lay.v10r1-remote-state.v1",
            "task_id": TASK_ID,
...
def bootstrap_once(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(payload.get("schema") == "lay.v10r1-bootstrap-payload.v1", "bootstrap payload schema drift")
    ...
    try:
        ...
        receipt = {
            "schema": "lay.v10r1-bootstrap.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "V10R1_BOOTSTRAP_CREATED_UNAUDITED",
...
def verify_bootstrap_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10R1_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED", "bootstrap audit did not admit markers")
...
def create_markers(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10r1-markers.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R1_ALL_MARKERS_AVAILABLE",
        "markers": marker_inventory(),
        "marker_rows": rows,
        "markers_created": 2,
        "markers_consumed": 0,
...
def build_once(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    workspace = pathlib.Path("/home/e/.cache") / f"lay-m3-v10r1-build-{TRANSACTION_ID}"
    ...
    try:
        ...
        provenance = {
            "schema": "lay.v10r1-build.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "V10R1_BUILD_CREATED_UNAUDITED",
            "source": {
                "source_manifest_sha256": sha256_file(BOOTSTRAP / "source-closure/SOURCE_MANIFEST.json"),
        ...
        if marker is not None and stage.exists():
            failure = {
                "schema": "lay.v10r1-build-failure.v1",
                "task_id": TASK_ID,
                "transaction_id": TRANSACTION_ID,
...
def verify_build_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10R1_BUILD_AUDIT_PASS_TRACE_ADMITTED", "build audit did not admit TRACE")
...
def verify_quiet_admission(path: pathlib.Path, controller_sha: str, build_audit: Mapping[str, Any]) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10R1_QUIET_HOST_TRACE_ADMITTED", "quiet-host audit did not admit TRACE")
...
def trace_once(build_audit_path: pathlib.Path, quiet_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    fsync_dir(stage)
    marker = consume_marker("TRACE", verify_bootstrap_audit(quiet_path, controller_sha) if quiet.get("verdict") == "V10R1_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED" else quiet)
    ...
    wrapper = {
        ...
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R1_TRACE_CREATED_UNAUDITED" if producer_complete else "BLOCKED_PROVENANCE",
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10r1-status.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R1_STATUS",
...
def self_check() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10r1-remote-self-check.v1",
        "verdict": "V10R1_REMOTE_CONTROLLER_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.v10r1-remote-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
