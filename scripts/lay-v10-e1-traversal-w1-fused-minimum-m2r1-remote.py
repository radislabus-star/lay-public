#!/usr/bin/env python3
"""Remote one-shot producer for the M2R1 fused-minimum repair experiment."""
...
EXPECTED = {
    ...
    "fragment": "a6ea388d5d76f8223511fd4822cff2df9fd0c3394fc200f4fc52db956522ce5b",
    "assembled": "b2b44ed1b42e79dcdb9b7dcfb2753eec547fa7d1e26b5fb63772a706f786ccd5",
...
def assemble_source(v10: bytes, fragment: bytes) -> bytes:
    ...
    need(sha256_bytes(fragment) == EXPECTED["fragment"] and len(fragment) == 156_122, "M2R1 fragment drift")
    need(v10.endswith(b"}\n"), "V10 terminal brace drift")
    assembled = v10[:-2] + fragment + b"}\n"
    need(len(assembled) == 247_640 and sha256_bytes(assembled) == EXPECTED["assembled"], "assembled M2R1 source drift")
...
def verify_bootstrap(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(payload.get("execution_admission_verdict") == "M2R1_EXECUTION_ADMITTED", "execution admission missing")
    files = {}
...
def marker_payload(route: str, controller_sha256: str) -> bytes:
    ...
    return canonical_json_bytes({
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2r1-marker.v1",
        "task_id": TASK_ID,
...
def append_state(sequence: int, state: str, **extra: Any) -> None:
    write_new_json(
        ...
        {
            "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2r1-state.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "sequence": sequence,
            "state": state,
...
def bootstrap_once(bootstrap: pathlib.Path) -> dict[str, Any]:
    ...
    try:
        ...
        receipt = {
            "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2r1-bootstrap.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
...
def build_once(controller_sha: str) -> dict[str, Any]:
    ...
    try:
        ...
        provenance = {
            "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2r1-build.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
        ...
        if marker is not None and stage.exists():
            failure = {
                "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2r1-build-failure.v1",
                "task_id": TASK_ID,
                "transaction_id": TRANSACTION_ID,
...
def parity_once(controller_sha: str, build_audit: pathlib.Path) -> dict[str, Any]:
    ...
    wrapper = {
        "schema": "lay.v10.e1-traversal-w1-fused-minimum-m2r1-parity-wrapper.v2",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": verdict,
...
def self_check() -> dict[str, Any]:
    ...
    return {"verdict": "M2R1_REMOTE_CONTROLLER_VERIFIED_UNRUN", "task_id": TASK_ID, "transaction_id": TRANSACTION_ID, "routes": list(ROUTE_ORDER), "markers": list(MARKER_ROUTES), "cargo_invocations": 0, "perf_stat_invocations": 0, "subject_executions": 0, "runtime_authority_changed": False}
