#!/usr/bin/env python3
"""Local fail-closed orchestrator for the M3 V8R3 direct-exec proof."""
...
TASK_ID = "slice8b-v10-e1-traversal-w1-dafsa-typed-view-m3-e2e-v8r3-terminal-projection-20260827"
TRANSACTION_ID = "a33732116bdf8a1ccf1216e99958750ad33b0ccc6a3c7bbd4226454b20bfd66f"
...
REMOTE_CACHE = pathlib.PurePosixPath("/home/e/.cache") / f"lay-m3-v8r3-{TRANSACTION_ID}"

CONTROLLER = pathlib.Path(__file__).resolve()
REMOTE_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v8r3-remote.py"
AUDITOR = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v8r3-audit.py"
V8_SOURCE = ROOT / "src/nanda_wave/l2_field/v13_typed_peak.rs"
PREFLIGHT_MANIFEST = ROOT / "docs/structural_gates/preflights/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_TERMINAL_PROJECTION_V1_2026-08-27.json"
PREFLIGHT_RECEIPT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_TERMINAL_PROJECTION_V1_PREFLIGHT_2026-08-27.json"
V8R3_PAPER = ROOT / "docs/structural_gates/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_TERMINAL_PROJECTION_CORRECTION_V1_2026-08-27.md"
V8R3_ROUTE = ROOT / "docs/structural_gates/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_TERMINAL_PROJECTION_CORRECTION_V1_ROUTE_V3_2026-08-27.md"
V8R3_ROUTE_RECEIPT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_TERMINAL_PROJECTION_CORRECTION_V1_ROUTE_V3_RECEIPT_2026-08-27.json"
...
CONTROLLER_EVIDENCE = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_REMOTE_CONTROLLER_V8R3_V1_2026-08-27"
CONTROLLER_IMPLEMENTATION = CONTROLLER_EVIDENCE / "IMPLEMENTATION_RECEIPT.json"
EXECUTION_JOURNAL = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_EXECUTION_JOURNAL_V1_2026-08-27"
ADMISSION_RECEIPT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_EXECUTION_ADMISSION_V1_2026-08-27/EXECUTION_ADMISSION.json"
BOOTSTRAP_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_BOOTSTRAP_AUDIT_V1_2026-08-27/BOOTSTRAP_AUDIT.json"
QUIET_ADMISSION = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_QUIET_ADMISSION_V1_2026-08-27/QUIET_ADMISSION.json"
TERMINAL_AUDIT = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_END_TO_END_V8R3_TERMINAL_AUDIT_V1_2026-08-27/TERMINAL_AUDIT.json"
...
class V8R3ControllerError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise V8R3ControllerError(message)
...
def verify_preflight() -> dict[str, Any]:
    need(sha256_file(PREFLIGHT_MANIFEST) == "299112c9db3db24ef56d8743328bc2fc80defc7860e543c948e16e8eff0ddf2c", "preflight manifest SHA drift")
    need(sha256_file(PREFLIGHT_RECEIPT) == "ca3745efde0d2c9311b0f2d777c671ae53e4a3146d11c9ce0ef20897b84d4cb3", "preflight receipt SHA drift")
    ...
    need(receipt.get("manifest_sha256") == "b48a42ae9c90c62babd77ffc81dd778d2acb8771dd21f02589bf6361ef75e98f", "NANDA manifest identity drift")
    return receipt


def verify_fixed_inputs() -> dict[str, Any]:
    expected = {
        V8R3_PAPER: "3d4fcffdb5ae5a9878ae7c35aec243d767963f516193d44ab59303b03a27b7fd",
        V8R3_ROUTE: "fca7659fe1de100feee0ac3c10c7155cf14de1f0ea5c46c4c5451b0df7bed804",
        V8R3_ROUTE_RECEIPT: "68fc2f49f5fc7cb2097e5289acad4c9f876dc617244d7b5bd7c09b90f076aa16",
    ...
    for path in (V8R3_PAPER, V8R3_ROUTE, V8R3_ROUTE_RECEIPT, V8R2_DIAGNOSIS, V8R2_JOURNAL, V8R2_CONTROLLER):
...
def parse_python(path: pathlib.Path) -> ast.Module:
    try:
        ...
        raise V8R3ControllerError(f"invalid controller source: {path}: {error}") from error
...
def static_graph(remote_check: Mapping[str, Any], auditor_check: Mapping[str, Any]) -> dict[str, Any]:
    ...
    need(remote_check.get("actions") == ["self-check", "status", "bootstrap", "create-marker", "e2e-once"], "remote action registry drift")
    need(remote_check.get("routes") == ["E2E"] and remote_check.get("markers") == ["e2e.available"], "remote route registry drift")
    command = remote_check.get("e2e_argv")
    need(isinstance(command, list) and str(REMOTE_PARENT / "bootstrap-v1/m3-v8r3-test-elf") in command, "direct executable absent")
...
def fault_model() -> dict[str, Any]:
    rows = []
    with tempfile.TemporaryDirectory(prefix="lay-m3-v8r3-faults-") as raw:
        ...
        for test_id, source in (
            ...
            try:
                ...
                raise V8R3ControllerError(f"partial source was accepted: {test_id}")
        ...
        try:
            ...
            raise V8R3ControllerError("exclusive receipt publication fault failed")
    terminal = load_json(V8R1_TERMINAL_AUDIT)
    producer = terminal.get("live_projection", {}).get("latest_state", {})
    need(
        terminal.get("verdict") == "BLOCKED_PROVENANCE"
        and terminal.get("e2e_wrapper_sha256") == EXPECTED_V8R1_WRAPPER_SHA256
        and producer.get("state") == "E2E_CREATED_UNAUDITED"
        and producer.get("e2e_wrapper_sha256") == EXPECTED_V8R1_WRAPPER_SHA256,
        "V8R1 terminal projection pair drift",
    )
    rows.extend([
        {
            "id": "terminal-projection-parity",
            "verdict": "PASS",
            "remote_producer_state": producer["state"],
            "independent_terminal_verdict": terminal["verdict"],
            "e2e_wrapper_sha256": EXPECTED_V8R1_WRAPPER_SHA256,
        },
        {"id": "response-retention-parity", **response_retention_fault()},
        {"id": "elf-copy-parity", "verdict": "PASS", "expected_sha256": EXPECTED_ELF_SHA256, "expected_size_bytes": EXPECTED_ELF_SIZE},
        {"id": "direct-command-parity", "verdict": "PASS", "route": "E2E"},
        {"id": "marker-parity", "verdict": "PASS", "markers": ["e2e.available"]},
        {"id": "auditor-parity", "verdict": "PASS", "dispatch": "independent"},
        {"id": "namespace-parity", "verdict": "PASS", "task_ids": [V8R1_TASK_ID, V8R2_TASK_ID, TASK_ID], "all_distinct": len({V8R1_TASK_ID, V8R2_TASK_ID, TASK_ID}) == 3},
...
def response_retention_fault() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="lay-m3-v8r3-response-retention-") as raw:
        journal = pathlib.Path(raw)
        try:
            journaled(journal, 1, "synthetic", lambda: response, ["ADMITTED"])
        except V8R3ControllerError:
            pass
        else:
            raise V8R3ControllerError("non-admitted response was accepted")
...
def self_check() -> dict[str, Any]:
    ...
    need(remote.get("verdict") == "M3_V8R3_REMOTE_CONTROLLER_STATIC_PASS", "remote self-check failed")
    need(auditor.get("verdict") == "M3_V8R3_INDEPENDENT_AUDITOR_STATIC_PASS", "auditor self-check failed")
    graph = static_graph(remote, auditor)
    faults = fault_model()
    retention = next(row for row in faults["tests"] if row["id"] == "response-retention-parity")
    return {
        "schema": "lay.m3-v8r3-controller-self-check.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R3_CONTROLLER_STATIC_SELF_CHECK_PASS",
...
def seal_self_check() -> dict[str, Any]:
    ...
    check = self_check()
    receipt = {
        "schema": "lay.m3-v8r3-controller-implementation.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_V8R3_CONTROLLERS_VERIFIED_UNRUN",
        ...
        "next_action_admitted": "independent V8R3 live execution admission only",
    }
...
def prepare_bootstrap(admission: Mapping[str, Any]) -> pathlib.Path:
    staging = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v8r3-bootstrap-"))
    try:
        files = {
        ...
        payload = {
            "schema": "lay.m3-v8r3-bootstrap-payload.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            ...
            "source_elf_remote_copy_only": True,
            "files": inventory,
        }
...
def initialize_journal() -> pathlib.Path:
    need(not EXECUTION_JOURNAL.exists(), "V8R3 execution journal already exists; retry is forbidden")
    EXECUTION_JOURNAL.mkdir(parents=True, mode=0o700)
    write_new(EXECUTION_JOURNAL / "JOURNAL.json", canonical({
        "schema": "lay.m3-v8r3-execution-journal.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def append_intent(root: pathlib.Path, sequence: int, action: str) -> None:
    ...
    write_new(root / f"{sequence:02d}-{action}.intent.json", canonical({
        "schema": "lay.m3-v8r3-external-intent.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def append_completion(
    ...
    write_new(root / f"{sequence:02d}-{action}.complete.json", canonical({
        "schema": "lay.m3-v8r3-external-completion.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "sequence": sequence,
        "action": action,
...
def create_remote_cache() -> dict[str, Any]:
    ...
    return {"verdict": "M3_V8R3_REMOTE_CACHE_CREATED", "stdout_sha256": sha256_bytes(output)}
...
def upload_bootstrap(staging: pathlib.Path) -> dict[str, Any]:
    ...
    return {"verdict": "M3_V8R3_BOOTSTRAP_UPLOADED", "payload_sha256": sha256_file(staging / "PAYLOAD.json")}
...
def verify_implementation() -> dict[str, Any]:
    ...
    need(value.get("verdict") == "M3_V8R3_CONTROLLERS_VERIFIED_UNRUN", "controller implementation verdict drift")
    for key, path in {
...
def execute() -> dict[str, Any]:
    ...
    try:
        admission = journaled(journal, sequence, "live-admission", lambda: auditor_call("live-admission"), ["M3_V8R3_EXECUTION_ADMITTED"])
        sequence += 1
        staging = prepare_bootstrap(admission)
        journaled(journal, sequence, "remote-cache-create", create_remote_cache, ["M3_V8R3_REMOTE_CACHE_CREATED"])
        sequence += 1
        journaled(journal, sequence, "bootstrap-upload", lambda: upload_bootstrap(staging), ["M3_V8R3_BOOTSTRAP_UPLOADED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            ...
            ["M3_V8R3_BOOTSTRAP_CREATED_UNAUDITED"],
        )
        sequence += 1
        journaled(journal, sequence, "bootstrap-audit", lambda: auditor_call("bootstrap"), ["M3_V8R3_BOOTSTRAP_AUDIT_PASS_MARKER_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "bootstrap-audit-upload",
            lambda: upload_audit(BOOTSTRAP_AUDIT, "BOOTSTRAP_AUDIT.json", "M3_V8R3_BOOTSTRAP_AUDIT_UPLOADED"),
            ["M3_V8R3_BOOTSTRAP_AUDIT_UPLOADED"],
        )
        sequence += 1
        ...
        journaled(
            ...
            ["M3_V8R3_E2E_MARKER_AVAILABLE"],
        )
        sequence += 1
        journaled(journal, sequence, "quiet-audit", lambda: auditor_call("quiet"), ["M3_V8R3_QUIET_HOST_E2E_ADMITTED"])
        sequence += 1
        journaled(
            journal,
            sequence,
            "quiet-audit-upload",
            lambda: upload_audit(QUIET_ADMISSION, "QUIET_ADMISSION.json", "M3_V8R3_QUIET_ADMISSION_UPLOADED"),
            ["M3_V8R3_QUIET_ADMISSION_UPLOADED"],
        )
        sequence += 1
        ...
        journaled(
            ...
            sequence,
            "e2e-once",
            lambda: remote_call("e2e-once", "--quiet", quiet_remote, timeout=10_800),
            ["M3_V8R3_E2E_CREATED_UNAUDITED", "BLOCKED_PROVENANCE"],
        )
        sequence += 1
        terminal = journaled(
            journal,
        ...
        write_new(journal / "TERMINAL.json", canonical({
            "schema": "lay.m3-v8r3-controller-terminal.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
        ...
        failure = {
            "schema": "lay.m3-v8r3-controller-failure.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
        ...
        write_manifest(journal)
        seal_tree(journal)
        raise V8R3ControllerError(json.dumps(failure, sort_keys=True)) from error
    finally:
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.m3-v8r3-controller-status.v1",
        "verdict": "M3_V8R3_CONTROLLER_STATUS",
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.m3-v8r3-controller-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "BLOCKED_PROVENANCE",
            "error": f"{type(error).__name__}: {error}",
