#!/usr/bin/env python3
"""Fail-closed remote producer for the V10 admission-substage diagnostic."""
...
EXPECTED_SOURCE = {
    ...
    "src/nanda_wave/l2_field/productive_v1/live.rs": (71_012, "36aeddd5e605e67377f99343f9937606ac774d9ba4bb5710152de060bc9d183b"),
    ...
    "src/typing_transition/proposal_admission.rs": (88_326, "6169e6d89a06c9ad3d7aefd467a8147f6d094b962faa25c739ad6d94a364b3dd"),
    "src/typing_transition/decision.rs": (45_197, "ad3c6d450c01811844a49e9c714d0eb9ff80f7de7d2f03a2e8b3e290deda3691"),
}
...
class V10RemoteError(RuntimeError):
    pass


def need(condition: bool, message: str) -> None:
    if not condition:
        raise V10RemoteError(message)
...
def verify_source_closure(root: pathlib.Path) -> dict[str, Any]:
    ...
    need(manifest.get("schema") == "lay.v10r2-source-closure.v1", "source manifest schema drift")
    ...
    content_sha256 = sha256_bytes(canonical(content))
    need(manifest.get("content_sha256") == content_sha256, "source closure content digest drift")
    return {
        "files": len(rows),
...
def verify_execution_admission(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(value.get("verdict") == "V10R2_EXECUTION_ADMITTED", "execution admission verdict drift")
...
def uid_capability_probe(stage_parent: pathlib.Path) -> dict[str, Any]:
    ...
    code = (
        ...
        "f=open(a,'xb'); f.write(b'v10-uid-proof\\n'); f.flush(); os.fsync(f.fileno()); f.close(); "
        "os.rename(a,b); assert b.read_bytes()==b'v10-uid-proof\\n'; b.unlink()"
    )
    result = run(
...
def append_state(sequence: int, state: str, **extra: Any) -> None:
    write_json(
        ...
        {
            "schema": "lay.v10r2-remote-state.v1",
            "task_id": TASK_ID,
...
def bootstrap_once(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(not PARENT.exists() and not STATE.exists(), "V10 remote namespace already exists")
    payload = load_json(path / "PAYLOAD.json")
    need(payload.get("schema") == "lay.v10r2-bootstrap-payload.v1", "bootstrap payload schema drift")
    ...
    need(payload.get("source_content_sha256") == source["content_sha256"], "payload/source closure binding drift")
    ...
    try:
        ...
        write_new(state_stage / "route.lock", b"v10-route-lock\n", 0o600)
        receipt = {
            "schema": "lay.v10r2-bootstrap.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "V10R2_BOOTSTRAP_CREATED_UNAUDITED",
...
def verify_bootstrap_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10R2_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED", "bootstrap audit did not admit markers")
...
def create_markers(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    markers = STATE / "markers"
    need(not markers.exists(), "V10 marker tree already exists")
    ...
    return {
        "schema": "lay.v10r2-markers.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R2_ALL_MARKERS_AVAILABLE",
        "markers": marker_inventory(),
        "marker_rows": rows,
        "markers_created": 2,
        "markers_consumed": 0,
...
def build_once(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(not BUILD.exists() and not (PARENT / "build-failure-v1").exists(), "V10 build already exists")

    stage = PARENT / f"build-v1.stage-{os.getpid()}-{time.time_ns()}"
    workspace = pathlib.Path("/home/e/.cache") / f"lay-m3-v10r2-build-{TRANSACTION_ID}"
    need(not workspace.exists(), "V10 build workspace already exists")
    stage.mkdir(mode=0o700)
    ...
    try:
        ...
        with (stage / "cargo.log").open("wb") as log:
            ...
            log.flush()
            os.fsync(log.fileno())
        need(result.returncode == 0, f"V10 Cargo exited {result.returncode}")
        elf = find_test_elf(workspace / "target")
        candidate = stage / "v10-test-elf"
        ...
        provenance = {
            "schema": "lay.v10r2-build.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "V10R2_BUILD_CREATED_UNAUDITED",
            "source": {
                "source_manifest_sha256": sha256_file(BOOTSTRAP / "source-closure/SOURCE_MANIFEST.json"),
        ...
        if marker is not None and stage.exists():
            failure = {
                "schema": "lay.v10r2-build-failure.v1",
                "task_id": TASK_ID,
                "transaction_id": TRANSACTION_ID,
...
def verify_build_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10R2_BUILD_AUDIT_PASS_TRACE_ADMITTED", "build audit did not admit TRACE")
    ...
    need(value.get("elf_sha256") == sha256_file(BUILD / "v10-test-elf"), "build audit ELF SHA drift")
    return value


def verify_quiet_admission(path: pathlib.Path, controller_sha: str, build_audit: Mapping[str, Any]) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10R2_QUIET_HOST_TRACE_ADMITTED", "quiet-host audit did not admit TRACE")
    ...
    retained_build_audit = path.parent / "BUILD_AUDIT.json"
    need(retained_build_audit.is_file(), "retained build audit absent beside quiet admission")
    need(value.get("build_audit_sha256") == sha256_file(retained_build_audit), "quiet-host build-audit binding drift")
    need(value.get("elf_sha256") == build_audit.get("elf_sha256"), "quiet-host ELF binding drift")
    return value
...
def trace_once(build_audit_path: pathlib.Path, quiet_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    state = latest_state()
    need(state is not None and state.get("state") == "BUILD_CREATED_UNAUDITED", "TRACE state predecessor drift")
    need(not TRACE.exists() and not (PARENT / "trace-failure-v1").exists(), "V10 TRACE evidence already exists")
    ...
    fsync_dir(stage)
    marker = consume_marker("TRACE", verify_bootstrap_audit(quiet_path, controller_sha) if quiet.get("verdict") == "V10R2_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED" else quiet)
    ...
    aggregate_trace_lines = stderr.count(b"productive_v90_materialization_trace ")
    substage_trace_lines = stderr.count(b"proposal_admission_substage_trace ")
    wrapper = {
        "schema": "lay.v10r2-trace-wrapper.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R2_TRACE_CREATED_UNAUDITED" if producer_complete else "BLOCKED_PROVENANCE",
        ...
        "trace_lines": {
            "v9_aggregate": aggregate_trace_lines,
            "v10_substage": substage_trace_lines,
        },
    ...
    append_state(
        ...
        trace_wrapper_sha256=sha256_file(root / "TRACE_WRAPPER.json"),
    )
    return wrapper
...
def status() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10r2-status.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10R2_STATUS",
...
def self_check() -> dict[str, Any]:
    ...
    need(ACTIONS == ("self-check", "status", "bootstrap", "create-markers", "build-once", "trace-once"), "action registry drift")
    need(ROUTES == ("BUILD", "TRACE") and set(MARKER_NAMES) == set(ROUTES), "route registry drift")
    need("perf" not in " ".join(build_tail + trace_tail), "perf route became reachable")
    need("--pid" not in build_tail + trace_tail, "attach route became reachable")
    need(SCIENTIFIC_TEST.endswith("m3_end_to_end_physical_proof"), "scientific test drift")
    need(("ld" + "-linux") not in pathlib.Path(__file__).read_text(), "TRACE must execute the test ELF directly")
    return {
        "schema": "lay.v10r2-remote-self-check.v1",
        "verdict": "V10R2_REMOTE_CONTROLLER_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def main() -> int:
    ...
    try:
        ...
        if args.action == "self-check":
            ...
            value = with_route_lock(lambda: trace_once(args.build_audit, args.quiet, controller_sha))
        print(json.dumps(value, sort_keys=True))
        return 0
    except BaseException as error:
        print(json.dumps({
            "schema": "lay.v10r2-remote-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
