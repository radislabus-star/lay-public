#!/usr/bin/env python3
"""Independent terminal scientific audit for W1 dafsa-typed-view M3."""
...
LOCAL_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v1.py"
...
PAIR_ROUTES = {
    "B": ("B0-BYTE-VIEW", "B1-BYTE-VIEW"),
    "T": ("T0-TYPED-VIEW", "T1-TYPED-VIEW"),
}
QUERIES = 382
ROUNDS = 20
...
def fixed_inputs() -> dict[str, Any]:
    ...
    need(implementation.get("verdict") == "M3_CONTROLLER_VERIFIED_UNRUN", "implementation verdict drift")
    need(bootstrap.get("verdict") == "M3_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED", "bootstrap audit verdict drift")
    build = json.loads(BUILD_AUDIT.read_text()) if BUILD_AUDIT.is_file() else None
    if build is not None:
        need(
            build.get("verdict")
            in {"M3_BUILD_AUDITED_PARITY_ADMITTED", "BLOCKED_BUILD", "BLOCKED_PROVENANCE"},
...
def self_check() -> dict[str, Any]:
    ...
    need(SAMPLE.size == 118 and RECORDS == 7_640 and MEASURED_EDGES == 502_915_120, "M3 denominator constants drift")
    need(tuple(PAIR_ROUTES["B"] + PAIR_ROUTES["T"]) != ROUTE_ORDER, "pair registry accidentally used as physical order")
    return {
        "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-terminal-audit-self-check.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "M3_TERMINAL_AUDITOR_VERIFIED_UNRUN",
        **values,
...
ps=os.popen("ps -eo pid=,args=").read().splitlines(); active=[x.strip() for x in ps if ('v10_m3_dafsa_typed_view' in x or str(parent) in x) and 'python3 -c' not in x]
...
def validate_route(root: pathlib.Path, route: str) -> dict[str, Any]:
    ...
    try:
        ...
        expected_candidate = (
            "BYTE_VIEW_BASELINE" if route.startswith("B") else "PREVALIDATED_TYPED_VIEW"
        )
        need(receipt.get("candidate") == expected_candidate, f"candidate identity drift: {route}")
        materialization = receipt.get("typed_materialization")
        if route.startswith("T"):
            need(
                isinstance(materialization, dict)
                and materialization.get("state_record_bytes") == 12
                and materialization.get("edge_record_bytes") == 12
                and materialization.get("states") == 81_128
                and materialization.get("edges") == 226_341
                and materialization.get("payload_bytes") == 3_689_628
                and materialization.get("field_mismatches") == 0
                and materialization.get("root_identity_match") is True
                and materialization.get("symbol_digest_match") is True
                and int(materialization.get("construction_wall_ns", 0)) > 0
                and int(materialization.get("construction_thread_cpu_ns", 0)) > 0,
                f"typed materialization closure drift: {route}",
            )
        else:
            need(materialization is None, f"baseline unexpectedly materialized typed view: {route}")
    ...
    return {
        ...
        "typed_materialization": materialization,
    }


def validate_parity(wrapper: Mapping[str, Any]) -> dict[str, Any]:
    receipt = wrapper.get("subject_receipt", {})
    need(
        wrapper.get("verdict") == "PASS"
        and isinstance(receipt, dict)
        and receipt.get("verdict") == "PASS",
        "M3 parity PASS receipt absent",
    )
    parity = receipt.get("parity", {})
    materialization = parity.get("materialization", {})
    need(
        parity.get("query_mismatches") == 0
        and parity.get("candidate_registry")
        == ["BYTE_VIEW_BASELINE", "PREVALIDATED_TYPED_VIEW"],
        "M3 parity semantic closure drift",
    )
    need(
        materialization.get("state_record_bytes") == 12
        and materialization.get("edge_record_bytes") == 12
        and materialization.get("states") == 81_128
        and materialization.get("edges") == 226_341
        and materialization.get("payload_bytes") == 3_689_628
        ...
        and materialization.get("root_identity_match") is True
        and materialization.get("symbol_digest_match") is True,
        "M3 parity typed-record closure drift",
    )
    schedules = parity.get("schedules", [])
    need(
        isinstance(schedules, list)
        and [row.get("schedule") for row in schedules] == ["FORWARD", "REVERSED"]
        and all(
            row.get("queries") == QUERIES
            and row.get("byte_examined_edges") == EDGES_PER_ROUND
            and row.get("typed_examined_edges") == EDGES_PER_ROUND
            for row in schedules
        ),
        "M3 parity schedule denominator drift",
    )
    return {
        "query_mismatches": 0,
        "state_field_mismatches": 0,
        "states_checked": 81_128,
        "edges_checked": 226_341,
        "schedules": ["FORWARD", "REVERSED"],
    }
...
def decide(routes: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    pairs = {key: pair_metrics(routes, key) for key in ("B", "T")}
    ...
    candidate = {
        "candidate": "PREVALIDATED_TYPED_VIEW",
        "cpu_gain": cpu_gain,
        "cycle_gain": cycle_gain,
        "instruction_delta": instruction_change,
        "frequency_delta": frequency_delta,
        "pass": passed,
    }
    verdict = blocked or ("W1_DAFSA_TYPED_VIEW_PASS" if passed else "W1_DAFSA_TYPED_VIEW_REJECTED")
    return {
        "verdict": verdict,
        ...
        "candidate": candidate,
        "selected": candidate["candidate"] if passed else None,
    }
...
def validate_live(value: dict[str, Any], controller_sha: str, build_audit_verdict: str | None) -> dict[str, Any]:
    ...
    need(value.get("active_subjects") == [], "M3 process remains active")
    states = value.get("state_rows", [])
    need(3 <= len(states) <= 8 and [row["value"]["sequence"] for row in states] == list(range(len(states))), "state sequence drift")
    ...
    if build_terminal:
        ...
        need(blocked is not None or (executed_count == 4 and latest == "B1-BYTE-VIEW_PASS"), "terminal producer state drift")
    executed_routes = list(ROUTE_ORDER[:executed_count])
    markers = value.get("markers", [])
    need(len(markers) == 6, "terminal marker cardinality drift")
    ...
    return {
        ...
        "markers_available": 6 - consumed_count,
...
def audit() -> dict[str, Any]:
    ...
    try:
        ...
        parity_audit_error: str | None = None
        cargo_invocations = 1
        if live["build_terminal"]:
            if live["latest_state"] == "BLOCKED_BUILD":
                ...
                scientific = blocked_science("BLOCKED_BUILD", [str(failure.get("error", "M3 build failed"))])
            else:
                ...
                scientific = blocked_science(
                    ...
                    check["build_audit_failure_causes"] or ["independent M3 build audit blocked"],
                )
        else:
            verify_manifest(remote / "build-v1")
            ...
            if parity.get("verdict") == "PASS":
                try:
                    parity_audit = validate_parity(parity)
                except Exception as error:
                    parity_audit_error = f"{type(error).__name__}: {error}"
            ...
            if parity.get("verdict") == "BLOCKED_PARITY":
                scientific = blocked_science("BLOCKED_PARITY", ["M3 semantic parity failed"])
            ...
            elif parity_audit_error is not None:
                scientific = blocked_science(
                    "BLOCKED_PROVENANCE",
                    [f"independent M3 parity audit failed: {parity_audit_error}"],
                )
                ...
                need(set(routes) == set(ROUTE_ORDER), "complete M3 route set absent without blocked verdict")
                scientific = decide(routes)
        after = live_projection()
        need(after == before, "remote state changed during terminal audit")
        receipt = {
            "schema": "lay.v10.e1-traversal-w1-dafsa-typed-view-m3-terminal-audit.v3",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            ...
            "next_action_admitted": (
                ...
                if scientific["verdict"] == "W1_DAFSA_TYPED_VIEW_PASS"
                else (
                    "new architectural-frontier paper only; typed decode closed"
                    if scientific["verdict"] == "W1_DAFSA_TYPED_VIEW_REJECTED"
                    else "none; terminal M3 blocked verdict"
                )
            ),
        }
        write_json(stage / "M3_TERMINAL_AUDIT_RECEIPT.json", receipt)
        ...
        return {**receipt, "receipt_sha256": sha256_file(RESULT / "M3_TERMINAL_AUDIT_RECEIPT.json")}
...
def main() -> int:
    ...
    try:
        ...
        print(f"M3 TERMINAL AUDIT ERROR: {type(error).__name__}: {error}", file=sys.stderr)
        return 1
