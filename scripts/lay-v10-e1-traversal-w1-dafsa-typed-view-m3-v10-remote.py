...
def verify_source_closure(root: pathlib.Path) -> dict[str, Any]:
    ...
    need(manifest.get("schema") == "lay.v10-source-closure.v1", "source manifest schema drift")
...
def verify_execution_admission(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    need(value.get("verdict") == "V10_EXECUTION_ADMITTED", "execution admission verdict drift")
...
def append_state(sequence: int, state: str, **extra: Any) -> None:
    write_json(
        ...
        {
            "schema": "lay.v10-remote-state.v1",
            "task_id": TASK_ID,
...
def bootstrap_once(path: pathlib.Path) -> dict[str, Any]:
    ...
    need(payload.get("schema") == "lay.v10-bootstrap-payload.v1", "bootstrap payload schema drift")
    ...
    try:
        ...
        receipt = {
            "schema": "lay.v10-bootstrap.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "V10_BOOTSTRAP_CREATED_UNAUDITED",
...
def marker_payload(route: str, admission: Mapping[str, Any]) -> bytes:
    ...
    return canonical({
        "schema": "lay.v10-one-shot-marker.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "route": route,
...
def verify_bootstrap_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED", "bootstrap audit did not admit markers")
...
def create_markers(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10-markers.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10_ALL_MARKERS_AVAILABLE",
        "markers": marker_inventory(),
        "marker_rows": rows,
        "markers_created": 2,
        "markers_consumed": 0,
...
def build_once(audit_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    workspace = pathlib.Path("/home/e/.cache") / f"lay-m3-v10-build-{TRANSACTION_ID}"
    ...
    try:
        ...
        provenance = {
            "schema": "lay.v10-build.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "V10_BUILD_CREATED_UNAUDITED",
            "source": {
                "source_manifest_sha256": sha256_file(BOOTSTRAP / "source-closure/SOURCE_MANIFEST.json"),
        ...
        if marker is not None and stage.exists():
            failure = {
                "schema": "lay.v10-build-failure.v1",
                "task_id": TASK_ID,
                "transaction_id": TRANSACTION_ID,
...
def verify_build_audit(path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10_BUILD_AUDIT_PASS_TRACE_ADMITTED", "build audit did not admit TRACE")
...
def verify_quiet_admission(path: pathlib.Path, controller_sha: str, build_audit: Mapping[str, Any]) -> dict[str, Any]:
    value = load_json(path)
    need(value.get("verdict") == "V10_QUIET_HOST_TRACE_ADMITTED", "quiet-host audit did not admit TRACE")
...
def trace_once(build_audit_path: pathlib.Path, quiet_path: pathlib.Path, controller_sha: str) -> dict[str, Any]:
    ...
    fsync_dir(stage)
    marker = consume_marker("TRACE", verify_bootstrap_audit(quiet_path, controller_sha) if quiet.get("verdict") == "V10_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED" else quiet)
    ...
    wrapper = {
        "schema": "lay.v10-trace-wrapper.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10_TRACE_CREATED_UNAUDITED" if producer_complete else "BLOCKED_PROVENANCE",
...
def self_check() -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10-remote-self-check.v1",
        "verdict": "V10_REMOTE_CONTROLLER_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.v10-remote-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
