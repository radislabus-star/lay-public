...
LOCAL_CONTROLLER = ROOT / "scripts/lay-v10-e1-traversal-w1-dafsa-typed-view-m3-v10.py"
...
CONTROLLER_IMPLEMENTATION = ROOT / "docs/structural_gates/receipts/LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10_IMPLEMENTATION_V1_2026-08-27/IMPLEMENTATION_RECEIPT.json"

RECEIPTS = ROOT / "docs/structural_gates/receipts"
ADMISSION_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10_EXECUTION_ADMISSION_V1_2026-08-27"
BOOTSTRAP_AUDIT_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10_BOOTSTRAP_AUDIT_V1_2026-08-27"
BUILD_AUDIT_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10_BUILD_AUDIT_V1_2026-08-27"
QUIET_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10_QUIET_ADMISSION_V1_2026-08-27"
TERMINAL_ROOT = RECEIPTS / "LAY_IME_TARGET_AUTHORITY_SLICE8B_V10_E1_TRAVERSAL_W1_DAFSA_TYPED_VIEW_M3_ADMISSION_SUBSTAGE_DIAGNOSTIC_V10_TERMINAL_AUDIT_V1_2026-08-27"
...
def fixed_local() -> dict[str, Any]:
    expected = {
        ...
        CONTROLLER_PREFLIGHT: "9449c227dbd22457cb250d5499ecdfaaab7e3d04ffc873397383645ecbc217ad",
    ...
    implementation = load_sealed_receipt(
        ...
        ("V10_REMOTE_CONTROLLERS_VERIFIED_UNRUN",),
    )
    for key, path in {
...
def live_admission() -> dict[str, Any]:
    ...
    probe_path = f"/home/e/.cache/lay-m3-v10-admission-{TRANSACTION_ID}"
    ...
    receipt = {
        "schema": "lay.v10-execution-admission.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10_EXECUTION_ADMITTED",
...
def bootstrap_audit() -> dict[str, Any]:
    local = fixed_local()
    admission = load_sealed_receipt(ADMISSION_ROOT, ADMISSION_RECEIPT, ("V10_EXECUTION_ADMITTED",))
    ...
    need(receipt.get("verdict") == "V10_BOOTSTRAP_CREATED_UNAUDITED", "remote bootstrap producer verdict drift")
    ...
    receipt_out = {
        "schema": "lay.v10-bootstrap-audit.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED",
...
def build_audit() -> dict[str, Any]:
    ...
    bootstrap = load_sealed_receipt(
        ...
        ("V10_BOOTSTRAP_AUDIT_PASS_MARKERS_ADMITTED",),
    )
    snapshot = remote_snapshot()
    validate_host(snapshot)
    ...
    temporary = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v10-build-audit-"))
    try:
        if build_state == "BLOCKED_BUILD":
            ...
            need(failure.get("schema") == "lay.v10-build-failure.v1", "build failure schema drift")
        ...
        need(provenance.get("verdict") == "V10_BUILD_CREATED_UNAUDITED", "build producer verdict drift")
        ...
        receipt = {
            ...
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": "V10_BUILD_AUDIT_PASS_TRACE_ADMITTED",
            "local_controller_sha256": bootstrap["local_controller_sha256"],
            "remote_controller_sha256": bootstrap["remote_controller_sha256"],
            "auditor_sha256": bootstrap["auditor_sha256"],
            "bootstrap_audit_sha256": sha256_file(BOOTSTRAP_AUDIT_RECEIPT),
...
def quiet_admission() -> dict[str, Any]:
    ...
    build = load_sealed_receipt(
        BUILD_AUDIT_ROOT,
        BUILD_AUDIT_RECEIPT,
        ("V10_BUILD_AUDIT_PASS_TRACE_ADMITTED",),
    )
    before = remote_snapshot()
    validate_host(before)
    ...
    receipt = {
        "schema": "lay.v10-quiet-admission.v1",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10_QUIET_HOST_TRACE_ADMITTED",
...
def summarize_trace(rows: Sequence[Mapping[str, Any]], semantic_failures: Sequence[str]) -> dict[str, Any]:
    ...
    return {
        "schema": "lay.v10-admission-substage-trace.v1",
        "trace_rows": len(rows),
...
def terminal_decision(
    ...
    if wrapper.get("schema") != "lay.v10-trace-wrapper.v1" or wrapper.get("task_id") != TASK_ID or wrapper.get("transaction_id") != TRANSACTION_ID:
    ...
    if wrapper.get("verdict") not in {"V10_TRACE_CREATED_UNAUDITED", "BLOCKED_PROVENANCE"}:
    ...
    if build.get("verdict") != "V10_BUILD_AUDIT_PASS_TRACE_ADMITTED":
...
def terminal_audit() -> dict[str, Any]:
    ...
    quiet = load_sealed_receipt(QUIET_ROOT, QUIET_RECEIPT, ("V10_QUIET_HOST_TRACE_ADMITTED",))
    snapshot = remote_snapshot()
    validate_host(snapshot)
    ...
    temporary = pathlib.Path(tempfile.mkdtemp(prefix="lay-m3-v10-terminal-"))
    try:
        ...
        write_new(derived / "TRACE_SUMMARY.json", canonical({
            "schema": "lay.v10-admission-substage-derived.v1",
            "parse_failures": trace_failures,
            "summary": summary,
        }))
        receipt = {
            "schema": "lay.v10-terminal-audit.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
            "verdict": verdict,
...
def self_check() -> dict[str, Any]:
    ...
    wrapper = {
        ...
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "verdict": "V10_TRACE_CREATED_UNAUDITED",
    ...
    build = {"verdict": "V10_BUILD_AUDIT_PASS_TRACE_ADMITTED", "elf_sha256": "a" * 64}
    verdict, failures = terminal_decision(subject, wrapper, build, rows, trace_failures, summary)
    ...
    return {
        "schema": "lay.v10-independent-auditor-self-check.v1",
        "verdict": "V10_INDEPENDENT_AUDITOR_STATIC_PASS",
        "task_id": TASK_ID,
        "transaction_id": TRANSACTION_ID,
        "auditor_sha256": sha256_file(AUDITOR),
...
def status() -> dict[str, Any]:
    return {
        "schema": "lay.v10-auditor-status.v1",
        "verdict": "V10_AUDITOR_STATUS",
        "receipts": {
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({
            "schema": "lay.v10-auditor-error.v1",
            "task_id": TASK_ID,
            "transaction_id": TRANSACTION_ID,
