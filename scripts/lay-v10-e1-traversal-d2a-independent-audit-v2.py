...
EXPECTED = {
    ...
    "audit_v1_receipt": "41b6928566f577a7cd39f6ec9610a89fa4832473fbc838866a9a0626d87d6e21",
    "audit_v1_manifest": "06eaee1fcd4176ca6e2b7b3b232f6a5c15872fa93e366d5d98552265eb9d07a6",
    "audit_v1_source": "f596ff43b216a5b2a8d20d1bc04d3fbbfb38b548676a441490a1e61059c8a028",
}
...
def self_check() -> dict[str, Any]:
    ...
    v1_receipt = require_file(
        AUDIT_V1_RESULT / "D2A_INDEPENDENT_AUDIT_RECEIPT.json",
        digest=EXPECTED["audit_v1_receipt"],
        mode="0444",
    )
    v1_manifest = require_file(
        AUDIT_V1_RESULT / "SHA256SUMS",
        digest=EXPECTED["audit_v1_manifest"],
        mode="0444",
    )
    v1_source = require_file(
        AUDIT_V1_RESULT / "auditor.py",
        digest=EXPECTED["audit_v1_source"],
        mode="0444",
    )
    require(verify_sha256sums(AUDIT_V1_RESULT)["entries"] == 2, "audit V1 manifest mismatch")
    v1_value = load_json(AUDIT_V1_RESULT / "D2A_INDEPENDENT_AUDIT_RECEIPT.json")
    require(v1_value.get("verdict") == "BLOCKED_PROVENANCE", "audit V1 verdict drift")
    require(v1_value.get("build_admitted") is False, "audit V1 admitted build")
    require(v1_value.get("marker_mutations") == 0, "audit V1 mutated markers")
    probe = remote_probe_source()
    compile(probe, "<remote-read-only-projection>", "exec")
    require(probe.startswith("import pathlib\nPARENT="), "remote import sequencing drift")
    ...
    return {
        "schema": "lay.v10.e1-traversal-d2a-independent-auditor-self-check.v2",
...
def publish_result(
    ...
    *,
    ...
    try:
        ...
        receipt = {
            "schema": "lay.v10.e1-traversal-d2a-independent-audit.v2",
            "task_id": TASK_ID,
            "verdict": verdict,
            "error": error,
